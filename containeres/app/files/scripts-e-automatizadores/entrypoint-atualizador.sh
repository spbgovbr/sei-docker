...
if [ "$MODULO_GESTAODOCUMENTAL_INSTALAR" == "true" ]; then
    ...
    if [ ! -f /sei/controlador-instalacoes/instalado-modulo-gestaodocumental.ok ]; then
        ...
        if [ -z "$MODULO_GESTAODOCUMENTAL_VERSAO" ]; then
            ...
            TMPFILE_SEI=/sei/controlador-instalacoes/instalado-modulo-gestaodocumental.ok
            if ! test -f "$TMPFILE_SEI"; then

                \cp /opt/sei/web/modulos/mod-gestao-documental/scripts/sei/sei-gestao-documental-atualizar.php /opt/sei/scripts
                \cp /opt/sei/web/modulos/mod-gestao-documental/scripts/sip/sip-gestao-documental-atualizar.php /opt/sip/scripts

                php -c /etc/php.ini /opt/sei/scripts/sei-gestao-documental-atualizar.php
                php -c /etc/php.ini /opt/sip/scripts/sip-gestao-documental-atualizar.php
        ...
        if [ -z "$MODULO_LOGINUNICO_VERSAO" ] || \
           ...
           [ -z "$MODULO_LOGINUNICO_ORGAO" ]; then
            ...
            if [ -d "/opt/sei/web/modulos/loginunico" ]; then
                ...
                mv /opt/sei/web/modulos/mod-sei-loginunico /opt/sei/web/modulos/loginunico
            fi


            cd /opt/sei/web/modulos/loginunico
            ...
            php -c /etc/php.ini web/modulos/loginunico/scripts/scriptInstalacao.php
    ...
    if [ ! -f /sei/controlador-instalacoes/instalado-modulo-assinaturavancada.ok ]; then
        ...
        if [ -z "$MODULO_ASSINATURAVANCADA_VERSAO" ] || \
           ...
           [ -z "$MODULO_ASSINATURAVANCADA_SERVIDORQUALIFICADA" ]; then
            echo "Informe as seguinte variaveis de ambiente no container:"
            echo "MODULO_ASSINATURAVANCADA_VERSAO, MODULO_ASSINATURAVANCADA_CLIENTID, MODULO_LOGINUNICO_SECRET, MODULO_LOGINUNICO_URLPROVIDER, MODULO_LOGINUNICO_REDIRECTURL"
            ...
            sed -i "s|require_once DIR_SEI_WEB.'/SEI.php';|require_once '/opt/sei/web/SEI.php';|g" /opt/sei/web/modulos/mod-assinatura/int/AssinaturaAvancadaINT.php
            ...
            php -c /etc/php.ini web/modulos/mod-assinatura/scripts/scriptInstalacao.php
