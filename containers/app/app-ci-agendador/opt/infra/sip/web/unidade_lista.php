...
try {
  ...
  if ($strSiglaPesquisa!==''){
    $objUnidadeDTO->setStrSigla('%'.$strSiglaPesquisa.'%',InfraDTO::$OPER_LIKE);
  }
  
  $strDescricaoPesquisa = PaginaSip::getInstance()->recuperarCampo('txtDescricaoUnidade');
  if ($strDescricaoPesquisa!==''){
    $objUnidadeDTO->setStrDescricao('%'.$strDescricaoPesquisa.'%',InfraDTO::$OPER_LIKE);
  }

  $strIdOrigemPesquisa = PaginaSip::getInstance()->recuperarCampo('txtIdOrigemUnidade');
  if ($strIdOrigemPesquisa!==''){
    $objUnidadeDTO->setStrIdOrigem('%'.$strIdOrigemPesquisa.'%',InfraDTO::$OPER_LIKE);
  }
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td valign="top">'.PaginaSip::getInstance()->getTrCheck($i,$arrObjUnidadeDTO[$i]->getNumIdUnidade(),$arrObjUnidadeDTO[$i]->getStrSigla()).'</td>';
      }
...
<form id="frmUnidadeLista" method="post" action="<?=SessaoSip::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  ?>
  <label id="lblOrgaoUnidade" for="selOrgaoUnidade" accesskey="o" class="infraLabelOpcional">Órgã<span class="infraTeclaAtalho">o</span>:</label>
  <select id="selOrgaoUnidade" name="selOrgaoUnidade" onchange="this.form.submit();" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" >
  <?=$strItensSelOrgao?>
  </select>

  <label id="lblSiglaUnidade" for="txtSiglaUnidade" class="infraLabelOpcional">Sigla:</label>
  <input type="text" id="txtSiglaUnidade" name="txtSiglaUnidade" class="infraText" value="<?=PaginaSip::tratarHTML($strSiglaPesquisa)?>" maxlength="30" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
  
  <label id="lblDescricaoUnidade" for="txtDescricaoUnidade" class="infraLabelOpcional">Descrição:</label>
  <input type="text" id="txtDescricaoUnidade" name="txtDescricaoUnidade" class="infraText" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" value="<?=PaginaSip::tratarHTML($strDescricaoPesquisa)?>" />

  <label id="lblIdOrigemUnidade" for="txtIdOrigemUnidade" accesskey="" class="infraLabelOpcional">ID Origem:</label>
  <input type="text" id="txtIdOrigemUnidade" name="txtIdOrigemUnidade" class="infraText" value="<?=PaginaSip::tratarHTML($strIdOrigemPesquisa);?>" maxlength="50" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
