...
class DocumentoRN extends InfraRN {
    ...
    try {
      ...
      foreach($arrIdDocumentos as $dblIdDocumento){
        ...
        if ($objDocumentoDTO->getStrStaProtocoloProtocolo() == ProtocoloRN::$TP_DOCUMENTO_GERADO){
          ...
          $arrComandoExecucao[] = DocumentoRN::montarComandoGeracaoPdf('processo-'.InfraUtil::retirarFormatacao($objDocumentoDTO->getStrProtocoloProcedimentoFormatado(),false), $strArquivoHtmlTemp, $strArquivoPdfParcial, $bolEscalaCinza);
          ...
          if (InfraUtil::getStrMimeType($objAnexoDTO->getStrNome()) == 'application/pdf' || InfraUtil::getStrMimeType($objAnexoDTO->getStrNome()) == 'text/html' || InfraUtil::getStrMimeType($objAnexoDTO->getStrNome()) == 'text/plain' || $this->ehUmaExtensaoDeImagemPermitida($objAnexoDTO->getStrNome()) || $this->processarArquivoOpenOffice($objAnexoDTO->getStrNome())){
            if ($objAnexoDTO==null){
              ...
              if (InfraUtil::getStrMimeType($objAnexoDTO->getStrNome()) == 'application/pdf'){
                ...
                $strArquivoPdfParcial = DIR_SEI_TEMP.'/'.$objAnexoRN->gerarNomeArquivoTemporario('-parte'.$numParteArquivoPdf++.'.pdf');
                $arrArquivoPdfParcial[] = $strArquivoPdfParcial;
                $arrComandoExecucao[] = DocumentoRN::montarComandoGeracaoPdf('processo-'.InfraUtil::retirarFormatacao($objDocumentoDTO->getStrProtocoloProcedimentoFormatado(),false), $strCaminhoCompletoArquivoTxt, $strArquivoPdfParcial, $bolEscalaCinza);
      ...
      if (!$bolIndividual) {
        ...
        $strCaminhoCompletoArquivoPdfTotal = $arrArquivoPdfParcial[0];
  ...
  public static function montarComandoGeracaoPdf($strTitulo, $strArquivoOrigem, $strArquivoPdf, $bolEscalaCinza = false){
    ...
    try{
      ...
      return 'wkhtmltopdf '.$strOpcaoEscalaCinza.' --quiet --disable-smart-shrinking --disable-javascript --disable-local-file-access '.$strPermissaoLocal.' --title '.$strTitulo.' ' .$strArquivoOrigem.' ' .$strArquivoPdf .' 2>&1';
