    ...
    */
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoVisualizar && in_array($arrObjTipoFormularioDTO[$i]->getNumIdTipoFormulario(),$arrIdTipoFormularioComAtributos)) {
        $strResultado .= '<a href="#" onclick="infraLimparFormatarTrAcessada(this.parentNode.parentNode);infraAbrirJanela(\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=tipo_formulario_visualizar&id_tipo_formulario='.$arrObjTipoFormularioDTO[$i]->getNumIdTipoFormulario()).'\',\'janelaTipoFormulario\',700,500,\'location=0,status=1,resizable=1,scrollbars=1\',false)" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.Icone::PRE_VISUALIZAR.'" title="Visualizar Tipo de Formulário" alt="Visualizar Tipo de Formulário" class="infraImg" /></a>&nbsp;';
      }
