  ...
  switch($_GET['acao']){
    ...
    case 'editor_salvar':
      ...
      try{
        ...
        if (substr($e->__toString(), 0, strlen('COMPARACAO')) == 'COMPARACAO'){
          die($e->__toString());
        }
  ...
  <script type="text/javascript">//<![CDATA[
    ...
    function exibirAvisoEditor(){
      ...
      if (divFundo==null){
        ...
        document.getElementById('imgInfraAviso').src = INFRA_ICONE_AGUARDAR;
      }
      ...
      if (ret != ''){
        if (ret.substring(0,2) != 'OK'){
          ...
          if (ret.substring(0,prefixoComparacao.length) == prefixoComparacao){
            ...
            if (ret.substr(0, prefixoValidacao.length)==prefixoValidacao) {
    ...
    var verificaReadOnly = function (editor) {
      var range = editor.createRange();
      range.selectNodeContents( editor.document.getBody() );
      var currentNode = range.startContainer;
      var nextNode;
      while (currentNode) {
        nextNode = currentNode.getNextSourceNode(false, CKEDITOR.NODE_ELEMENT);
        if (currentNode.isReadOnly()) {
          // verificar se é link_sei, senão volta a ser editavel
          if (currentNode.getName()=='span' && nextNode.getName()=='a' && nextNode.getAttribute('class')=='ancora_sei') {
            nextNode = nextNode.getNextSourceNode(false, CKEDITOR.NODE_ELEMENT);
          } else {
    ...
    function  inicializar(){
      ...
      <?if ($objEditorDTORetorno->getStrSinAlterouVersao()=='S'){?>
