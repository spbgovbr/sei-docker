...
abstract class InfraOracle implements InfraIBanco {
    ...
    public function abrirConexao(){
        try {
            ...
            if ($this->getNumberFormat() != null) {
                $this->executarSql("ALTER SESSION SET NLS_NUMERIC_CHARACTERS = '" . $this->getNumberFormat() . "'");
            }
    ...
    public function consultarSql($sql, $arrCamposBind = null){
        if (InfraDebug::isBolProcessar()) {
            InfraDebug::getInstance()->gravarInfra('[InfraOracle->consultarSql] ' . InfraBD::formatarDetalhesSql($sql,$arrCamposBind));
        ...
        if ($resultado === FALSE) {
            throw new InfraException(oci_error($this->conexao), null, InfraBD::formatarDetalhesSql($sql,$arrCamposBind));
        }
    ...
    public function executarSql($sql, $arrCamposBind = null){
        if (InfraDebug::isBolProcessar()) {
            InfraDebug::getInstance()->gravarInfra('[InfraOracle->executarSql] ' . InfraBD::formatarDetalhesSql($sql,$arrCamposBind));
            $numSeg = InfraUtil::verificarTempoProcessamento();
        }
    ...
    * <pre>
    ...
    public function executarSqlTipado($strSql, &$arrCampos = null){
        if (InfraDebug::isBolProcessar()) {
            InfraDebug::getInstance()->gravarInfra('[InfraOracle->executarSql] ' . substr($strSql, 0, INFRA_TAM_MAX_LOG_SQL));
            $numSeg = InfraUtil::verificarTempoProcessamento();
        }
        if ($this->conexao == null) {
            throw new InfraException('Tentando executar um comando em uma conexão fechada.');
            ...
            for ($i = 0; $i < count($arrChaves); $i ++) {
                ...
                switch ($numTipo) {
                    ...
                    case OCI_B_BLOB:
                        ...
                        break;
                    default:
                        throw new InfraException('Tipo inváido no parâmetro ' . $arrChaves[$i] . ' para operações com bind no banco de dados.');
                }
                if ($bolTipoBlob) {
                    ...
                    oci_bind_by_name($resResultado, $arrChaves[$i], $arrCampos[$arrChaves[$i]][0], $numTamanho, $numTipo);
                }
            }
        }
        if (! $this->transacao) {
        ...
        if (InfraDebug::isBolProcessar()) {
            $numSeg = InfraUtil::verificarTempoProcessamento($numSeg);
            InfraDebug::getInstance()->gravarInfra('[InfraOracle->executarSqlTipado] ' . $numReg . ' registro(s) afetado(s)');
