  ...
  public function pesquisar(InfraNavegadorDTO $parObjInfraNavegadorDTO){
    try{
      ...
      if ($parObjInfraNavegadorDTO->isSetDthInicial() && $parObjInfraNavegadorDTO->isSetDthFinal()){
        $sql .= 'WHERE dth_acesso >= '.$this->getObjInfraIBanco()->formatarGravacaoDth($parObjInfraNavegadorDTO->getDthInicial()).' ';
        $sql .= 'AND dth_acesso <= '.$this->getObjInfraIBanco()->formatarGravacaoDth($parObjInfraNavegadorDTO->getDthFinal()).' ';
      }
      ...
      foreach($rs as $item){
        ...
        $objInfraNavegadorDTO->setDblTotalAcessos($this->getObjInfraIBanco()->formatarLeituraDbl($item['total_acessos']));
        $objInfraNavegadorDTO->setStrIdentificacao($this->getObjInfraIBanco()->formatarLeituraStr($item['identificacao']));
        
        if ($parObjInfraNavegadorDTO->getStrSinIgnorarVersao()=='N'){
          $objInfraNavegadorDTO->setStrVersao($this->getObjInfraIBanco()->formatarLeituraStr($item['versao']));
        }
