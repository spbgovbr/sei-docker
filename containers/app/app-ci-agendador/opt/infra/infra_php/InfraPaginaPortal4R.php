  ...
  */
  ...
  abstract class InfraPaginaPortal4R extends InfraPagina {
    ...
    public function montarStyle() {
      parent::montarStyle();
      if ($this->getNumTipoBrowser() == self::$TIPO_BROWSER_IE56 || $this->getNumTipoBrowser() == self::$TIPO_BROWSER_IE7 || $this->getNumTipoBrowser()== self::$TIPO_BROWSER_IE8) {
        echo '<link href="'.$this->getDiretorioCssLocal().'/infra-ie5678.css?'.$this->getNumVersao().'" rel="stylesheet" type="text/css" media="all" />'."\n";
      }
      if (self::isBolMobile()) {
        echo '<link href="'.$this->getDiretorioCssGlobal().'/infra-global-portal4r-mobile.css?'.$this->getNumVersao().'" rel="stylesheet" type="text/css" media="all" />'."\n";
      }
    }

    public function montarJavaScript() {
      parent::montarJavascript();
      echo '<script type="text/javascript" charset="iso-8859-1" src="'.$this->getDiretorioJavaScriptGlobal().'/InfraUtilPaginaPortal4R.js?'.$this->numURLRandomica.'\"></script>'."\n";
    }
    ...
    public function fecharBody() {
      echo '<div style="clear:both;color:#78a483;font-size:10px;font-weight:bold;text-align:center;width:100%;">'.
          ...
          '<br /><br />'.
          '<a href="'.ConfiguracaoPortal4R::getInstance()->getValor('Portal4R','URL').'trf4/controlador.php?acao=pagina_visualizar&id_pagina=937"><img src="'.ConfiguracaoPortal4R::getInstance()->getValor('Portal4R','URL').'trf4/imagens/icone_enderecos_telefones.jpg" alt="Endereços e Telefones" /></a>';
      ...
      if (preg_match('/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i',$strUserAgent) ||
