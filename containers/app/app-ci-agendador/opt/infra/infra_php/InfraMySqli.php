...
abstract class InfraMySqli extends InfraMySql {
  ...
  //SELECAO
        ...
        try{
          ...
          if (InfraDebug::isBolProcessar()) {
            ...
            if ($this->getCharset()!=null){
              if ($this->conexao->set_charset($this->getCharset())===false){
    ...
    public function consultarSql($sql, $arrCamposBind = null) {
      ...
      if (InfraDebug::isBolProcessar()) {
        InfraDebug::getInstance()->gravarInfra('[InfraMySqli->consultarSql] ' . InfraBD::formatarDetalhesSql($sql,$arrCamposBind));
      ...
      if ($arrCamposBind != null && count($arrCamposBind) > 0) {
        ...
        if (($stmt = $this->conexao->prepare($sql)) === FALSE){
          throw new InfraException($this->conexao->error, null, InfraBD::formatarDetalhesSql($sql,$arrCamposBind));
        }
        ...
        if (call_user_func_array(array($stmt, 'bind_param'), $arrParams) === FALSE){
          throw new InfraException($this->conexao->error, null, InfraBD::formatarDetalhesSql($sql,$arrCamposBind));
        }

        if (!empty($arrBlobs)){
          foreach($arrBlobs as $arrBlob){
            if ($stmt->send_long_data($arrBlob[0], $arrBlob[1]) === FALSE){
              throw new InfraException($this->conexao->error, null, InfraBD::formatarDetalhesSql($sql,$arrCamposBind));
            }
          }
        }
    ...
    public function executarSql($sql, $arrCamposBind = null) {
      ...
      if (InfraDebug::isBolProcessar()) {
        InfraDebug::getInstance()->gravarInfra('[InfraMySqli->executar] ' . InfraBD::formatarDetalhesSql($sql,$arrCamposBind));
        $numSeg = InfraUtil::verificarTempoProcessamento();
      }
      ...
      if ($arrCamposBind != null && count($arrCamposBind) > 0) {
        ...
        if ( $this->conexao->query($sql) === FALSE ) {
          throw new InfraException($this->conexao->error,null,InfraBD::formatarDetalhesSql($sql,$arrCamposBind));
        }
