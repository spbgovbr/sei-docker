...
abstract class InfraCache {
  ...
  public function __construct(){
    ...
    }else if ($this->strTipoCache == self::$REDIS){
    ...
    try {
      ...
      InfraDebug::getInstance()->gravarInfra('[InfraCache->conectar] '.$this->strTipoCache.' '.$this->getStrServidor().':'.$this->getNumPorta());
      ...
      if ($this->getObjInfraLog()!=null){
        try{
          ...
          $this->getObjInfraLog()->gravar('Servidor ['.$_SERVER['SERVER_ADDR'].'] não conseguiu acessar o serviço do '.($this->strTipoCache == self::$MEMCACHE?'Memcache':'Redis').' no endereço [' . $this->getStrServidor() . ':'.$this->getNumPorta().'].'."\n\n".InfraException::inspecionar($e));

        }catch(Exception $e){}
      }
    }
    //$this->objCache->connect('127.0.0.1', 11211);
    //$this->objCache->flush();
  }
