...
try {
  ...
  if (isset($_GET['id_usuario']) || isset($_GET['id_codigo_acesso'])) {
    ...
    PaginaSip::getInstance()->salvarCamposPost(array('selOrgaoSistemaLogin', 'selSistemaLogin','selOrgaoUsuarioLogin', 'hdnIdUsuarioLogin', 'txtUsuarioLogin','hdnSiglaUsuarioLogin','txtRemoteAddr', 'txtHttpClientIp', 'txtHttpXForwardedFor'));
 ...
 */
  ...
  $numIdOrgao = PaginaSip::getInstance()->recuperarCampo('selOrgaoSistemaLogin',SessaoSip::getInstance()->getNumIdOrgaoSistema());
  if ($numIdOrgao!==''){
    $objLoginDTO->setNumIdOrgaoSistema($numIdOrgao);
  ...
  $strRemoteAddr = PaginaSip::getInstance()->recuperarCampo('txtRemoteAddr');
  if ($strRemoteAddr!=''){
    $objLoginDTO->setStrRemoteAddr('%'.$strRemoteAddr.'%', InfraDTO::$OPER_LIKE);
  }

  $strHttpClientIp = PaginaSip::getInstance()->recuperarCampo('txtHttpClientIp');
  if ($strHttpClientIp!=''){
    $objLoginDTO->setStrHttpClientIp('%'.$strHttpClientIp.'%', InfraDTO::$OPER_LIKE);
  }

  $strHttpXForwardedFor = PaginaSip::getInstance()->recuperarCampo('txtHttpXForwardedFor');
  ...
  $strStaTipo = PaginaSip::getInstance()->recuperarCampo('rdoTipo');
  if ($strStaTipo != 'T'){
    if ($strStaTipo == 'S') {
      $objLoginDTO->setStrIdCodigoAcesso(null);
    }else if ($strStaTipo == 'E'){
 ...
 */    }else{
    ...
    */
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td valign="top">'.PaginaSip::getInstance()->getTrCheck($i,$arrObjLoginDTO[$i]->getStrIdLogin().'-'.$arrObjLoginDTO[$i]->getNumIdSistema().'-'.$arrObjLoginDTO[$i]->getNumIdUsuario(),$arrObjLoginDTO[$i]->getStrIdLogin()).'</td>';
      }
...
?>
<?if(0){?><style><?}?>
  #divInfraAreaDados {<?=$strDisplayCriterios?>}

  #fldTipo  {position:absolute;left:0%;top:0%;height:80%;width:19%;padding-top:8px;}
  div.infraDivRadio {margin:8px 0 0 15px;}

  #lblOrgaoSistemaLogin {position:absolute;left:22%;top:0%;width:20%;}
  #selOrgaoSistemaLogin {position:absolute;left:22%;top:12%;width:20%;}

  #lblSistemaLogin {position:absolute;left:22%;top:30%;width:20%;}
  #selSistemaLogin {position:absolute;left:22%;top:42%;width:20%;}

  #lblOrgaoUsuarioLogin {position:absolute;left:44%;top:0%;width:20%;}
  #selOrgaoUsuarioLogin {position:absolute;left:44%;top:12%;width:20%;}

  #lblUsuarioLogin {position:absolute;left:44%;top:30%;width:20%;}
  #txtUsuarioLogin {position:absolute;left:44%;top:42%;width:20%;}

  #lblRemoteAddr {position:absolute;left:66%;top:0%;width:20%;}
  #txtRemoteAddr {position:absolute;left:66%;top:12%;width:20%;}

  #lblHttpClientIp {position:absolute;left:66%;top:30%;width:20%;}
  #txtHttpClientIp {position:absolute;left:66%;top:42%;width:20%;}

  #lblHttpXForwardedFor {position:absolute;left:66%;top:60%;width:20%;}
  #txtHttpXForwardedFor {position:absolute;left:66%;top:72%;width:20%;}
...
<? } ?>
  ...
  function trocarOrgaoSistema(obj){
    document.getElementById('selSistemaLogin').value='null';
    obj.form.submit();
  }

  function abrirJanelaAcessoLogin(link){
    infraAbrirJanela(link,'janelaAcessoLogin',700,450,'location=0,status=1,resizable=1,scrollbars=1',true);
  }

  function abrirJanelaCodigoAcessoLogin(link){
    infraAbrirJanela(link,'janelaCodigoAcessoLogin',700,450,'location=0,status=1,resizable=1,scrollbars=1',true);
  }
...
?>
<form id="frmLoginLista" method="post" action="<?=SessaoSip::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  <?
  ...
  <fieldset id="fldTipo" class="infraFieldset">
    ...
    <div id="divSinTodos" class="infraDivRadio">
      <input type="radio" name="rdoTipo" id="optSinTodos" onchange="this.form.submit();" value="T" <?=($strStaTipo=='T'?'checked="checked"':'')?>  class="infraRadio" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
      <label id="lblSinTodos" accesskey="" for="optSinTodos" class="infraLabelRadio">Todos</label>
    </div>

    <div id="divSinSenha" class="infraDivRadio">
      <input type="radio" name="rdoTipo" id="optSinSenha" onchange="this.form.submit();" value="S" <?=($strStaTipo=='S'?'checked="checked"':'')?>  class="infraRadio" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
    ...
    <div id="divSin2Fatores" class="infraDivRadio">
      <input type="radio" name="rdoTipo" id="optSin2Fatores" onchange="this.form.submit();" value="E" <?=($strStaTipo=='E'?'checked="checked"':'')?>  class="infraRadio" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
      <label id="lblSin2Fatores" accesskey="" for="optSin2Fatores" class="infraLabelRadio">2 Fatores</label>
    </div>

  </fieldset>

  <label id="lblOrgaoSistemaLogin" for="selOrgaoSistemaLogin" accesskey="" class="infraLabelOpcional">Órgão do Sistema:</label>
  <select id="selOrgaoSistemaLogin" name="selOrgaoSistemaLogin" onchange="trocarOrgaoSistema(this);" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" >
    <?=$strItensSelOrgaoSistema?>
  </select>

  <label id="lblSistemaLogin" for="selSistemaLogin" accesskey="S" class="infraLabelObrigatorio" ><span class="infraTeclaAtalho">S</span>istema:</label>
  <select id="selSistemaLogin" name="selSistemaLogin" onchange="this.form.submit();" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$strDesabilitar?> >
    <?=$strItensSelSistema?>
  </select>

  <label id="lblOrgaoUsuarioLogin" for="selOrgaoUsuarioLogin" accesskey="" class="infraLabelObrigatorio">Órgão do Usuário:</label>
  <select id="selOrgaoUsuarioLogin" name="selOrgaoUsuarioLogin" onchange="objAjaxUsuario.limpar();this.form.submit();" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" >
  ...
  <input type="text" id="txtUsuarioLogin" name="txtUsuarioLogin" class="infraText" value="<?=PaginaSip::tratarHTML($strSiglaUsuario)?>" maxlength="100" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
