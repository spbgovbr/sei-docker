    ...
    case 'documento_escolher_tipo':
      ...
      if (SessaoSEI::getInstance()->verificarPermissao('documento_gerar')){
        ...
        if ($strFiltroSerie=='U'){
          ...
          $strImgExibir = '<img id="imgExibirSeries" onclick="exibirSeries(\'T\');" src="'.PaginaSEI::getInstance()->getIconeMais().'" title="Exibir todos os tipos" alt="Exibir todos os tipos" class="infraImg" />';
        }
        ...
        if ($strFiltroSerie=='T'){
          ...
          $strImgExibir = '<img id="imgExibirSeries" onclick="exibirSeries(\'U\');" src="'.PaginaSEI::getInstance()->getIconeMenos().'" title="Exibir apenas os tipos já utilizados pela unidade" alt="Exibir apenas os tipos já utilizados pela unidade" class="infraImg" />';
        }
      }
      ...
      if ($numRegistros) {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
          ...
          if (!isset($arrObjSerieNaoLiberados[$arrOpcoes[$i][0]])) {
            ...
            $strResultado .= '<a style="width:100%;" href="' . SessaoSEI::getInstance()->assinarLink('controlador.php?acao=' . $strAcaoDestino . '&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_procedimento=' . $_GET['id_procedimento'] . '&id_serie=' . $arrOpcoes[$i][0] . $strParametros) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '" class="ancoraOpcao">' . PaginaSEI::tratarHTML($arrOpcoes[$i][1]) . ($arrOpcoes[$i][2] == SerieRN::$TA_FORMULARIO ? '<sup>&nbsp;<span style="font-size:1.1em;">(Formulário)</span></sup>' : '') . '</a>' . "\n";
            $strResultado .= '</td>';
            $strResultado .= '</tr>';
          }
        }
