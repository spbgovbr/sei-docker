      ...
      if (count($arrTipoAssociacao)){
        ...
        if ($parObjProcedimentoDTO->getStrSinMontandoArvore()=='S' && $parObjProcedimentoDTO->getStrSinLinhaDireta()=='S'){
          ...
          $objAtividadeDTO->adicionarCriterio(
              array('IdUnidade','IdUnidadeOrigem'),
              array(InfraDTO::$OPER_IGUAL, InfraDTO::$OPER_IGUAL),
      ...
      foreach($arrObjAtividadeDTO as $objAtividadeDTO){
        ...
        if (in_array($objAtividadeDTO->getNumIdTarefa(),array(
          ...
          $objAtividadeDTO->setStrNomeTarefa(str_replace(array(
              ...
              '@HIPOTESE_LEGAL@',
              '@VISUALIZACAO@',
              '@VALIDADE@'),'',$objAtividadeDTO->getStrNomeTarefa()));
        }
      }
  ...
  protected function concluirControlado($arrObjProcedimentoDTO){
    try{
      ...
      foreach($arrObjProcedimentoDTO as $objProcedimentoDTO){
        ...
        $objAcessoExternoDTO->setDistinct(true);
        $objAcessoExternoDTO->retStrProtocoloDocumentoFormatado();
        $objAcessoExternoDTO->retStrStaTipo();
        $objAcessoExternoDTO->setStrStaTipo(array(AcessoExternoRN::$TA_ASSINATURA_EXTERNA,
        ...
        $arrObjAcessoExternoDTO = InfraArray::indexarArrInfraDTO($objAcessoExternoRN->listar($objAcessoExternoDTO),'StaTipo',true);

        if (count($arrObjAcessoExternoDTO)){
          if (isset($arrObjAcessoExternoDTO[AcessoExternoRN::$TA_USUARIO_EXTERNO])) {
            $objInfraException->adicionarValidacao('Processo '.$strProtocoloFormatado.' possui liberação de acesso externo com permissão para inclusão de documentos nesta unidade.');
          }
          if (isset($arrObjAcessoExternoDTO[AcessoExternoRN::$TA_ASSINATURA_EXTERNA])){
            $objInfraException->adicionarValidacao('Processo '.$strProtocoloFormatado.' possui liberação para assinatura externa ('.implode(', ',InfraArray::converterArrInfraDTO($arrObjAcessoExternoDTO[AcessoExternoRN::$TA_ASSINATURA_EXTERNA],'ProtocoloDocumentoFormatado')).') com permissão para inclusão de documentos nesta unidade.');
          }
        }
      }
    ...
    try{
      ...
      if (InfraArray::contar($arrObjAtividadeDTO)) {
        ...
        if (InfraArray::contar($arr)) {
          ...
          if ($arrObjAtividadeDTO[0]->getNumIdUsuarioAtribuicao() == null ||
              $arrObjAtividadeDTO[0]->getNumIdUsuarioAtribuicao() == SessaoSEI::getInstance()->getNumIdUsuario() ||
              $objProcedimentoDTO->getStrStaNivelAcessoGlobalProtocolo() == ProtocoloRN::$NA_SIGILOSO ||
              $objInfraParametro->getValor('SEI_SINALIZACAO_PROCESSO') == '0') {
      ...
      if ($objPesquisaSigilosoDTO->isSetDtaInicio() && $objPesquisaSigilosoDTO->isSetDtaFim()) {
        ...
        if (!InfraString::isBolVazia($objPesquisaSigilosoDTO->getDtaInicio())) {
          if ($objPesquisaSigilosoDTO->getDtaInicio() == $objPesquisaSigilosoDTO->getDtaFim()) {
            ...
            $objPesquisaSigilosoDTO->adicionarCriterio(array('Geracao', 'Geracao'),
                array(InfraDTO::$OPER_MAIOR_IGUAL, InfraDTO::$OPER_MENOR_IGUAL),
                array($objPesquisaSigilosoDTO->getDtaInicio(), $objPesquisaSigilosoDTO->getDtaFim()),
                InfraDTO::$OPER_LOGICO_AND);
          }
        }
      }
  ...
  protected function pesquisarAcervoSigilososUnidadeConectado(PesquisaSigilosoDTO $objPesquisaSigilosoDTO){
    try{
      ...
      if (count($arrObjProcedimentoDTO)) {
        ...
        $objAtividadeDTO->setDblIdProtocolo($arrIdProtocolo, InfraDTO::$OPER_IN);
        $objAtividadeDTO->setNumIdUnidade(SessaoSEI::getInstance()->getNumIdUnidadeAtual());
        $objAtividadeDTO->setNumIdTarefa(array_merge(TarefaRN::getArrTarefasTramitacao(),
