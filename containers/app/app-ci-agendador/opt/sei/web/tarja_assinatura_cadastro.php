...
function exibirAjuda(){
  infraAbrirJanela('<?=$strLinkAjuda?>','janelaAjudaVariaveisTarjas',800,600,'location=0,status=1,resizable=1,scrollbars=1',false);
}
...
<form id="frmUpload">
  <div id="divUpload" class="infraAreaDados" style="height:5em;">
    ...
    -->
    <label id="lblArquivo" for="filArquivo" accesskey="" class="infraLabelOpcional">Logotipo:</label>
<? if ($_GET['acao']=='tarja_assinatura_alterar') { ?>    
    <input type="file" id="filArquivo" accept="image/gif,image/png" name="filArquivo" size="50" onchange="objUpload.executar();" /><br />
<? }
?>
  </div>
