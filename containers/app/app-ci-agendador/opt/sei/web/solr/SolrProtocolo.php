...
class SolrProtocolo {
  ...
  public static function executar(PesquisaProtocoloSolrDTO $objPesquisaProtocoloSolrDTO)
  {
    ...
    if ($objPesquisaProtocoloSolrDTO->getStrNumero() != null) {
      ...
      $partialfields .= '(numero:*' . InfraSolrUtil::formatarCaracteresEspeciais($objPesquisaProtocoloSolrDTO->getStrNumero()) . '*)';
    }
    if ($objPesquisaProtocoloSolrDTO->getStrNomeArvore() != null) {
      if ($partialfields != '') {
        $partialfields .= ' AND ';
      }
      $partialfields .= '(nome_arvore:*' . InfraSolrUtil::formatarCaracteresEspeciais($objPesquisaProtocoloSolrDTO->getStrNomeArvore()) . '*)';
    }
    ...
    if ($itens == 0) {
      ...
      if ($numRegistros) {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
          ...
          if ($objObjProtocoloDTOProcesso->getNumCodigoAcesso() < 0 && $objUnidadeDTOAtual->getStrSinProtocolo()=='N'){
            continue;
          }
          ...
          if ($strStaTipoData == 'I' ) {
            $strMetaTagData = 'Data de Inclusão';
          }else{
            $strMetaTagData = ($arrRegistros[$i]['id_doc'] == null) ? 'Data do Processo' : 'Data do Documento';
          }
          ...
          $arrMetatags = array();
          $arrMetatags['Unidade Geradora'] = '<a alt="'.PaginaSEI::tratarHTML($strDescricaoUnidadeGeradora).'" title="'.PaginaSEI::tratarHTML($strDescricaoUnidadeGeradora).'" class="ancoraSigla" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'">'.PaginaSEI::tratarHTML($strSiglaUnidadeGeradora).'</a>';
          ...
          if (count($arrMetatags)) {
            ...
            foreach ($arrMetatags as $nomeMetaTag => $valorMetaTag) {
              $html .= "<td class=\"pesquisaMetatag\"><b>".$nomeMetaTag.":</b> ".$valorMetaTag."</td>\n";
            }
            $html .= "</tr>\n";
          }
        }
