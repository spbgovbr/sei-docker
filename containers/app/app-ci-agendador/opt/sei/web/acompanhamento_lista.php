...
try {
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      $strResultado .= '<td valign="top" class="tdAcompanhamento">'.PaginaSEI::getInstance()->getTrCheck($i,$arrObjAcompanhamentoDTO[$i]->getNumIdAcompanhamento(), ProcedimentoINT::formatarProtocoloTipoRI0200($objProcedimentoDTO->getStrProtocoloProcedimentoFormatado(),$objProcedimentoDTO->getStrNomeTipoProcedimento()),'N','Infra','',false ).'</td>';
      ...
      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=acompanhamento_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_acompanhamento='.$arrObjAcompanhamentoDTO[$i]->getNumIdAcompanhamento()).'" ><img src="'.PaginaSEI::getInstance()->getIconeConsultar().'" title="Consultar Acompanhamento" alt="Consultar Acompanhamento" class="infraImg" /></a>&nbsp;';
      }
