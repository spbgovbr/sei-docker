...
try {
  ...
  $strTitulo = 'Geraзгo de ZIP de Processo do SEI Federaзгo na Instalaзгo '.SessaoSEIFederacao::getInstance()->getStrSiglaInstalacaoFederacaoLocal();
  ...
  foreach($arrIdProtocoloFederacao as $strIdProtocoloFederacao) {
    ...
    if ($objDocumentoDTO->getStrSinZip()=='N'){
      $objInfraException->adicionarValidacao('Documento '.$objDocumentoDTO->getStrProtocoloDocumentoFormatado().' nгo estб disponнvel para geraзгo de ZIP.');
    }else {
  ...
  SeiINT::download(null, null, $objAnexoDTO->getStrNome(), 'SEI-'.$objProcedimentoDTO->getStrProtocoloProcedimentoFormatado().'.zip', 'attachment');
