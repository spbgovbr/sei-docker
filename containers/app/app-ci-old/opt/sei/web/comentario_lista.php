    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($arrObjComentarioDTO[$i]->getDblIdRelProtocoloProtocolo() == null) {
        ...
        if ($objProtocoloDTO->getStrStaProtocolo()==ProtocoloRN::$TP_DOCUMENTO_GERADO || $objProtocoloDTO->getStrStaProtocolo()==ProtocoloRN::$TP_DOCUMENTO_RECEBIDO){
          ...
          $strLinkProtocolo = '<a href="javascript:void(0);" class="ancoraPadraoAzul" onclick="infraLimparFormatarTrAcessada(this.parentNode.parentNode);window.open(\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=documento_visualizar&acao_origem='.$_GET['acao'].'&id_documento='.$objProtocoloDTO->getDblIdProtocolo()).'\');" title="'.PaginaSEI::tratarHTML($strTipoProtocolo).'">'.$strProtocoloFormatado.'</a>';
        }else{
          ...
          $strLinkProtocolo = '<a href="javascript:void(0);" class="ancoraPadraoAzul" onclick="infraLimparFormatarTrAcessada(this.parentNode.parentNode);window.open(\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=procedimento_trabalhar&acao_origem='.$_GET['acao'].'&id_procedimento='.$objProtocoloDTO->getDblIdProtocolo()).'\');" title="'.PaginaSEI::tratarHTML($strTipoProtocolo).'">'.$strProtocoloFormatado.'</a>';
          SessaoSEI::getInstance()->setArrParametrosRepasseLink($arrParametrosRepasseLink);
        }
      }

      if ($i){
        $strResultado .= '<tr class="trSeparador"><td colspan="6">&nbsp;</td></tr>';
      }
      //$strCssTr = ($strCssTr=='<tr class="infraTrClara">')?'<tr class="infraTrEscura">':'<tr class="infraTrClara">';
      $strResultado .= '<tr class="infraTrEscura">';

      $strResultado .= '<td align="center" valign="top" style="display:none">'.PaginaSEI::getInstance()->getTrCheck($i,$arrObjComentarioDTO[$i]->getNumIdComentario(),$arrObjComentarioDTO[$i]->getNumIdComentario()).'</td>';
      $strResultado .= '<td  align="center" valign="top">'.PaginaSEI::tratarHTML($arrObjComentarioDTO[$i]->getDthComentario()).'</td>';

      if ($strVisualizacao=='T') {
        $strResultado .= '<td  align="center" valign="top">'.$strLinkProtocolo.'</td>';
        $strResultado .= '<td  align="center" valign="top">'.$strTipoProtocolo.'</td>';
      }

      $strResultado .= '<td align="center" valign="top"><a alt="'.PaginaSEI::tratarHTML($arrObjComentarioDTO[$i]->getStrNomeUnidade()).'" title="'.PaginaSEI::tratarHTML($arrObjComentarioDTO[$i]->getStrNomeUnidade()).'" class="ancoraSigla">'.PaginaSEI::tratarHTML($arrObjComentarioDTO[$i]->getStrSiglaUnidade()).'</a></td>';
      $strResultado .= '<td align="center" valign="top"><a alt="'.PaginaSEI::tratarHTML($arrObjComentarioDTO[$i]->getStrNomeUsuario()).'" title="'.PaginaSEI::tratarHTML($arrObjComentarioDTO[$i]->getStrNomeUsuario()).'" class="ancoraSigla">'.PaginaSEI::tratarHTML($arrObjComentarioDTO[$i]->getStrSiglaUsuario()).'</a></td>';
