    ...
    try {
      ...
      if ($parObjAcessoExternoDTO->isSetNumIdAcessoExterno()) {
        ...
        if (count($arrIdLiberados)) {
          ...
          $objProtocoloDTO_Pesquisa = new ProtocoloDTO();
          $objProtocoloDTO_Pesquisa->retDblIdProtocolo();
          $objProtocoloDTO_Pesquisa->setNumIdUsuarioGerador(SessaoSEIExterna::getInstance()->getNumIdUsuarioExterno());
          $objProtocoloDTO_Pesquisa->setNumIdUnidadeGeradora($objAcessoExternoDTO->getNumIdUnidade());

          $objProtocoloRN = new ProtocoloRN();
          $arrObjProtocoloDTO = $objProtocoloRN->listarRN0668($objProtocoloDTO_Pesquisa);

          if (count($arrObjProtocoloDTO) > 0) {
            foreach ($arrObjProtocoloDTO as $objProtocoloDTO) {
              $arrIdLiberados[] = $objProtocoloDTO->getDblIdProtocolo();
            }
          }
          $bolTodosLiberados = false;
        } else {
 ...
 */
  ...
  protected function incluirDocumentoControlado(AcessoExternoDTO $objAcessoExternoDTO){
    try {
      ...
      $objDocumentoDTO->getObjProtocoloDTO()->setStrStaNivelAcessoLocal($objProtocoloDTO_Consulta->getStrStaNivelAcessoLocal());
      //atributos necessarios para validacao da insercao de protocolo
      $objDocumentoDTO->getObjProtocoloDTO()->setArrObjObservacaoDTO(array());
      $objDocumentoDTO->getObjProtocoloDTO()->setStrDescricao(null);
      $objDocumentoDTO->getObjProtocoloDTO()->setNumIdUnidadeGeradora($objAcessoExternoDTO_Pesquisa->getNumIdUnidade());
      //retorna o anexo, que é o documento... apesar do atributo no ProtocoloDTO ser um array, terá sempre apenas um objeto
      $objAnexoDTO = $objDocumentoDTO->getObjProtocoloDTO()->getArrObjAnexoDTO()[0];
      //valida se é pdf
      //$this->validarStrNome($objAnexoDTO, $objInfraException);
      //atributos necessarios para o protocolo referente ao documento
      $objDocumentoDTO->getObjProtocoloDTO()->setDtaGeracao($objAnexoDTO->getDthInclusao());
      //apesar do protocolo conter um array de anexos, nesse caso, esse array terá sempre um elemento apenas
      $objDocumentoDTO->getObjProtocoloDTO()->setArrObjParticipanteDTO(array($objParticipanteDTO_Interessado,$objParticipanteDTO_Destinatario));
