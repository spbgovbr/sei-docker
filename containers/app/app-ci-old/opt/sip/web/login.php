...
try {
  ...
  foreach($_GET as $key => $item){
    ...
    }else if ($key == 'acao' && !is_numeric($item)){
      die('Link inválido.');
    }
  }
  ...
  $_SESSION['SIP_CAPTCHA'] = InfraCaptcha::gerar($strCodigoParaGeracaoCaptcha);

  $strChaveToken = md5($strSiglaSistema.'/'.$strSiglaOrgaoSistema);
  $strToken = (!isset($_SESSION['SIP_TOKEN_'.$strChaveToken])) ? null : $_SESSION['SIP_TOKEN_'.$strChaveToken];
  $_SESSION['SIP_TOKEN_'.$strChaveToken] = md5(uniqid(mt_rand()));
  ...
  }else if (isset($_GET['acao']) && trim($_GET['acao']!='')){
    $numAcao = $_GET['acao'];
  }
  ...
  switch ($numAcao) {
    ...
    case LoginINT::$ACAO_LOGAR_CONFIGURAR_2_FATORES:
      ...
      if (isset($_POST['hdnToken'.$strChaveToken])) {
        try {

          if ($strToken == null || $strToken != $_POST['hdnToken'.$strChaveToken]) {
            $objInfraException->lancarValidacao('Erro processando dados do formulário.');
          }

          if ($_SESSION['SIP_NUM_FALHA_LOGIN'] >= $numLoginSemCaptcha && trim($_POST['txtCaptchaConfirmacao']) != $strCaptchaPesquisa) {
...
?>
  ...
  <? if ($objSistemaDTO->getStrLogo()!=null && !$bolConfigurar2Fatores){ ?>
  #divLogo {
    overflow:hidden;
    margin:auto;
    width:150px;
...
function validarCampos(){
  ...
  <? if ($_SESSION['SIP_NUM_FALHA_LOGIN'] >= $numLoginSemCaptcha){ ?>
  if (infraTrim(document.getElementById('txtCaptchaConfirmacao').value)=='') {
    alert('Informe o código de confirmação.');
    document.getElementById('txtCaptchaConfirmacao').focus();
    return false;
  }
  <? } ?>
...
function acaoLogin(acao){
  ...
  <? if ($bolConfigurar2Fatores){ ?>
    if (!confirm('Se você já efetuou a leitura do QR Code com o aplicativo clique OK para continar.')){
      return false;
    }
  <?}?>
...
?>
  <form id="frmLogin" name="frmLogin" class="h-100" action="login.php?<?=$strParametrosAction?>" method="post" onsubmit="return acaoLogin(<?=$numAcaoSubmit?>);">
    ...
    <div class="d-flex   justify-content-center align-items-center h-100">
      <div id="area-cards-login" class="align-self-center col-xs-6 col-sm-6 col-md-6 col-lg-4 col-xl-4">
              ...
              <div class="col-xl-9">
                ...
                <div  >
                  <div id="divUsuario" class="input-group mb-3 d-flex">
                    ...
                    <input type="text" autofocus="" id="txtUsuario" name="txtUsuario" placeholder="Usuário" class="form-control"    value="<?=PaginaLogin::tratarHTML($_POST['txtUsuario'])?>" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>" maxlength="100" >
                  </div>
                  ...
                  <div id="divSenha" class="mb-3 d-flex">
                    ...
                    <input type="password" autofocus="" id="pwdSenha" name="pwdSenha" placeholder="Senha" class="form-control" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>"  value="" autocomplete="off">
                  </div>
                  ...
                  <div id="divOrgao" class="input-group mb-3 d-flex" style="<?=$strDisplayOrgao?>">
                    ...
                    <select id="selOrgao" name="selOrgao" class="form-control" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>">
                      <?=$strItensSelOrgao?>
                    </select>
                  </div>
                  ...
                  <div class="md-form d-flex media">
                    <button type="submit" id="Acessar" name="Acessar" class="btn text-white  flex-grow-1 infraCorBarraSuperior" style="border: none;" accesskey="e" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>">
                      ACESSAR
                    </button>
                  </div>
                  ...
                  <div class="text-right" style="margin-top:5px">
                    <a href="#" class="pl-1 linkLogin" title="Adicione maior segurança no seu acesso ao sistema." alt="Adicione maior segurança no seu acesso ao sistema." onclick="acaoLogin(<?=LoginINT::$ACAO_LOGAR_CONFIGURAR_2_FATORES?>)" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>">Autenticação em dois fatores</a>
                  </div>
                  <? } ?>

                  <? if ($bolCaptcha){
                    echo LoginINT::montarDivCaptchaBootstrap($strCodigoParaGeracaoCaptcha);
                  } ?>


                  <?} else if ($bolCodigoAcesso){?>
                  <div id="divValidacao2FA">
                    <p class="text-justify text-secondary">Informe o código de 6 números gerado pelo aplicativo de autenticação em 2 fatores:</p>
                  </div>
                  <div id="divCodigoAcesso" class="input-group mb-3 d-flex">
                    <span class="input-group-prepend">
                        <span class="input-group-text"><img src="svg/cadeado.svg"></span>
                    </span>
                    <input type="text" autofocus="" id="txtCodigoAcesso" name="txtCodigoAcesso" placeholder="Código de Acesso" class="form-control"  onkeypress="return infraMascaraNumero(this,event,6)"  value="" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>" maxlength="6" >
                  </div>


                  <div class="d-flex flex-md-row flex-column mb-3">
                    <div  class="input-group flex-grow-1 pb-md-0 pb-2 ">
                      <input type="checkbox" id="chkLiberar" name="chkLiberar" <?=($strSinLiberar=='S'?'checked="checked"':'')?> class="infraCheckbox" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>" />
                      <label id="lblLiberar" for="chkLiberar" class="infraLabelCheckbox">Não usar o 2FA neste dispositivo e navegador</label>
                    </div>
                  </div>

                  <div class="d-flex flex-md-row flex-column mb-3">
                    <a target="_blank" href="ajuda/ajuda_2_fatores.htm" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>" class="linkLogin">Instruções</a>
                  </div>

                  <div class="d-flex flex-md-row flex-column" >
                    <div class="d-flex pb-md-0 pb-2 mr-md-1 mr-0  w-md-50 w-100">
                      <button type="submit"   value="Validar" class="btn text-white w-100 infraCorBarraSuperior  " style="border: none;" accesskey="p"  tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>">
                        Validar
                      </button>
                    </div>
                    <div class="d-flex  mr-ml-1 ml-0  w-md-50 w-100">
                      <button type="button" class="btn text-white w-100 infraCorBarraSuperior" onclick="acaoLogin(<?=LoginINT::$ACAO_DESATIVAR_2_FATORES?>)" value="Desativar 2FA" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>"  style="border: none;" accesskey="d">
                        Desativar 2FA
                      </button>
                    </div>
                  </div>
                  ...
                  <?} else if ($bolAviso2Fatores){?>

                    <div >
                      <p class="text-justify text-secondary">A autenticação em 2 fatores já está ativada para sua conta no sistema.</p>
                    </div>
                    ...
                    <div class="d-flex flex-md-row flex-column" >
                      ...
                      <div class="d-flex mr-ml-1 ml-0 w-md-50 w-100">
                        <button type="button"  class="btn text-white w-100 infraCorBarraSuperior" onclick="location.href='login.php?sigla_sistema=<?=PaginaLogin::tratarHTML($objSistemaDTO->getStrSigla())?>&sigla_orgao_sistema=<?=PaginaLogin::tratarHTML($objSistemaDTO->getStrSiglaOrgao())?>';" value="Voltar" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>"  style="border: none;" accesskey="v">
                          Voltar
                        </button>
                      </div>
                    </div>
                  ...
                  <?} else if ($bolInstrucoes2Fatores){?>

                  <div id="divIntroducao2FA">
                      <p class="text-justify text-secondary">A autenticação em 2 fatores é um recurso para adicionar maior segurança no seu acesso ao sistema. Ao ativá-la, qualquer tentativa de login em dispositivos não liberados irá requerer também um código numérico gerado por um aplicativo.</p>
                      <p class="text-justify  text-secondary">Antes de prosseguir, leia as instruções <a class="linkLogin"  target="_blank" href="ajuda/ajuda_2_fatores.htm" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>" >aqui</a>.</p>
                  </div>

                  <div class="d-flex flex-md-row flex-column" >
                    <div class="d-flex pb-md-0 pb-2 mr-md-1 mr-0 w-md-50 w-100">
                      <button type="button" onclick="acaoLogin(<?=LoginINT::$ACAO_GERAR_2_FATORES?>)" value="Prosseguir" class="btn text-white w-100 infraCorBarraSuperior" style="border: none;" accesskey="p"  tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>">
                        Prosseguir
                      </button>
                    </div>

                    <div class="d-flex  mr-ml-1 ml-0 w-md-50 w-100">
                      <button type="button"  class="btn text-white w-100 infraCorBarraSuperior" onclick="location.href='login.php?sigla_sistema=<?=PaginaLogin::tratarHTML($objSistemaDTO->getStrSigla())?>&sigla_orgao_sistema=<?=PaginaLogin::tratarHTML($objSistemaDTO->getStrSiglaOrgao())?>';" value="Cancelar" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>"  style="border: none;" accesskey="c">
                        Cancelar
                      </button>
                    </div>
                  </div>
                  ...
                  <?} else if ($bolConfigurar2Fatores){?>
                    ...
                    <div class="mb-3 w-100" style="position: relative;">
                      <img alt="Autenticação de 2 Fatores" title="Autenticacao de 2 Fatores" src="data:image/png;base64,<?=$strQrCode?>" class="mx-auto d-block">
                      <a class="linkLogin" style="position: absolute;right: 0;bottom: 4px;" target="_blank" href="ajuda/ajuda_2_fatores.htm" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>" >Instruções</a>
                    ...
                    <span class="input-group-prepend">
                      ...
                      <input type="text" autofocus="" id="txtEmail" name="txtEmail" placeholder="E-mail pessoal" class="form-control"  value="<?=PaginaSip::tratarHTML($_POST['txtEmail']);?>" tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>" >
                    </div>

                    <div class="d-flex flex-md-row flex-column" >
                      <div class="d-flex pb-md-0 pb-2 mr-md-1 mr-0  w-md-50 w-100">
                        <button type="button"   value="Enviar" onclick="acaoLogin(<?=LoginINT::$ACAO_ATIVAR_2_FATORES?>)" class="btn text-white w-100  infraCorBarraSuperior" style="border: none;" accesskey="e"  tabindex="<?=PaginaLogin::getInstance()->getProxTabDados()?>">
                          Enviar
                        </button>
                      </div>
                      <div class="d-flex  mr-ml-1 ml-0  w-md-50 w-100">
    ...
    <input type="hidden" id="hdnToken<?=$strChaveToken?>" name="hdnToken<?=$strChaveToken?>" value="<?=$_SESSION['SIP_TOKEN_'.$strChaveToken]?>"/>
