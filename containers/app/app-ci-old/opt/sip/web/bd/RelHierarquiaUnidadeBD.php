...
class RelHierarquiaUnidadeBD extends InfraBD {
  ...
  public function listarUnidadesNovas(RelHierarquiaUnidadeDTO $objRelHierarquiaUnidadeDTO){
    try{
      ...
      $sql = ' SELECT a.id_unidade as id_unidade, a.sigla as sigla_unidade '.
             ' FROM unidade a '.
             ' WHERE NOT EXISTS (SELECT b.id_unidade '.
                               ' FROM rel_hierarquia_unidade b '.
                               ' WHERE a.id_unidade=b.id_unidade and b.id_hierarquia='.$objInfraBanco->formatarGravacaoNum($objRelHierarquiaUnidadeDTO->getNumIdHierarquia()).') '.
             ' AND sin_global='.$objInfraBanco->formatarGravacaoStr('N').
             ' AND sin_ativo='.$objInfraBanco->formatarGravacaoStr('S').
             ' AND a.id_orgao='.$objInfraBanco->formatarGravacaoNum($objRelHierarquiaUnidadeDTO->getNumIdOrgaoUnidade()).
