...
try {
  ...
  if ($numIdSistema != '' && $numIdUsuario != ''){
    ...
    if ($numIdSistema==$objInfraParametro->getValor('ID_SISTEMA_SIP')) {
      $arrPerfisReservados = $objInfraParametro->listarValores(array('ID_PERFIL_SIP_ADMINISTRADOR_SISTEMA',
...
?>
<form id="frmCoordenadorPerfilCadastro" method="post" onsubmit="return OnSubmitForm();" action="<?=SessaoSip::getInstance()->assinarLink('coordenador_perfil_cadastro.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
<?
...
?>
  ...
  <select id="selOrgaoSistema" name="selOrgaoSistema" onchange="this.form.submit();" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$strDesabilitar?>>
