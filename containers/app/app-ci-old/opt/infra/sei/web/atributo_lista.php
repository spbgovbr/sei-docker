...
try {
  ...
  if ($_GET['acao'] == 'atributo_listar' && $numRegistros){
    ...
    if ($bolAcaoTipoFormularioVisualizar){
      $arrComandos[] = '<button type="button" accesskey="V" id="btnVisualizar" value="Visualizar" onclick="infraAbrirJanela(\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=tipo_formulario_visualizar&id_tipo_formulario='.$_GET['id_tipo_formulario']).'\',\'janelaTipoFormulario\',700,500,\'location=0,status=1,resizable=1,scrollbars=1\',false)" class="infraButton"><span class="infraTeclaAtalho">V</span>isualizar</button>';
    }
  }
