...
<form id="frmNovidadeMostrar" method="post" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  <? if ($numRegistros){ ?>
  <input type="checkbox" id="chkNovidade" name="chkNovidade" class="infraCheckbox" onclick="gravarUltimaNovidade(this);" <?=$strOcultar?> />
  <label id="lblNovidade" for="chkNovidade" class="infraLabelCheckbox" <?=$strOcultar?> ><?=$numRegistros==1?'Não exibir novamente esta novidade':'Não exibir novamente estas novidades'?></label>
  <? } ?>
