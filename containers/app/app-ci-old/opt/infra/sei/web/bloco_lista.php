...
try {
  ...
  if(($_GET['acao']=='bloco_assinatura_listar')){
    ...
    $objBlocoDTOPesquisa->retNumDocumentos();
    $objBlocoDTOPesquisa->retNumAssinados();
  ...
  if (!PaginaSEI::getInstance()->isBolPaginaSelecao()){
    ...
    if ($bolAcaoCadastrar){
      ...
      $arrComandos[] = '<button type="button" accesskey="N" id="btnNovo" value="Novo" onclick="location.href=\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$strAcaoNovo.'&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao']).'\'" class="infraButton d-none d-md-inline-block" style="width:5em;"><span class="infraTeclaAtalho">N</span>ovo</button>';
    }
  }else{
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if (PaginaSEI::getInstance()->isBolPaginaSelecao()){
        ...
        $strResultado .= '<td align="center"><a href="javascript:void(0);" alt="'.$objBlocoDTO->getStrDescricaoUnidade().'" title="'.$objBlocoDTO->getStrDescricaoUnidade().'" class="ancoraSigla">'.$objBlocoDTO->getStrSiglaUnidade().'</a></td>';
      ...
      $strResultado .= '<td>'.PaginaSEI::tratarHTML($objBlocoDTO->getStrDescricao()).'</td>';



      //if ($objBlocoDTOPesquisa->getStrStaTipo() == BlocoRN::$TB_ASSINATURA){
      //  $strResultado .= '<td align="center">'.InfraUtil::formatarMilhares($objBlocoDTO->getNumDocumentos()).'</td>'."\n";
      //  $strResultado .= '<td align="center">'.InfraUtil::formatarMilhares($objBlocoDTO->getNumDocumentos()-$objBlocoDTO->getNumAssinados()).'</th>'."\n";
      //}
