...
class InfraCronograma {
  ...
  public function getArrCores(){
    return array('#3399CC','#999966','#FFCC66','#CC3333',
        '#CCCC99','#99CCCC' , '#666633',
        '#CC9933', '#996600',
        '#FF9900', '#FF6600', '#FF9966',
        '#CC6633', '#993300', '#FF9999',
        '#FFCCCC', '#CC9999',
        '#996666', '#0066CC', '#990000',
        '#808080', '#FFFF00', '#6666FF',
        '#990099', '#00FF80', '#FF00FF',
        '#FF9933', '#FCBA8C', '#808000',
        '#E6E6E6', '#CCCCCC', '#666666',
        '#66CCFF',  '#006699',
        '#669999', '#336666',
        '#008000',
    );
  ...
  public function gerarCronogramaSimples($arrDados, $varCores = null) {
    ...
    $numExtensaoTabela = $numTamanhoUmaPaginaPaisagem*(InfraData::compararDatas($strMenorData,$strMaiorData)/$numDiasAno);    
    ...
    $strGrafico = '<div id="divGrafico" class="infraAreaDados" style="height:' .(((count($arrDados))*$numAlturaBarras)+$numEspacoFinalTabela+$numDiferencaAlturaRotuloPontas).'px">';
    ...
    $strValorInicialEscala = $numExtensaoUmMes*((InfraData::obterUltimoDiaMes($arrMenorData[InfraData::$MES],$arrMenorData[InfraData::$ANO])-$arrMenorData[InfraData::$DIA])/$numTamanhoMedioMes);    
    $strValorFinalEscala = $numExtensaoTabela+($numExtensaoUmMes*(($arrMaiorData[InfraData::$DIA]-1)/$numTamanhoMedioMes));  
          
    foreach($arrDados as $i => $dado){
      
      $strGrafico .= '<div style="left: 0px; top: '.(($numAlturaBarras*($i))+$numEspacoNoTopoParaRotuloMeses-$numTantoQueMarcacaoRotuloSaiPraForaDaPrimeiraBarra) .'px; height: '.($i < (count($arrDados)-1) ? $numAlturaBarras-2 : $numAlturaBarras+$numTantoQueMarcacaoRotuloSaiPraForaDaPrimeiraBarra).'px; border-right: 2px; border-right-style: dotted; position:absolute; z-index:1;"></div>';
      $strGrafico .= '<div style="left: '.($numExtensaoTabela-1).'px; top: '.(($numAlturaBarras*($i))+$numEspacoNoTopoParaRotuloMeses-$numTantoQueMarcacaoRotuloSaiPraForaDaPrimeiraBarra) .'px; height: '.($i < (count($arrDados)-1) ? $numAlturaBarras-2 : $numAlturaBarras+$numTantoQueMarcacaoRotuloSaiPraForaDaPrimeiraBarra).'px; border-right: 2px; border-right-style: dotted; position:absolute; z-index:1;"></div>';
            
      $strValorAtualEscala = $strValorInicialEscala;
      $strDataAtualEscala = InfraData::calcularData(1,InfraData::$UNIDADE_MESES,InfraData::$SENTIDO_ADIANTE,$strMenorData);
      ...
      while ($strValorAtualEscala < $strValorFinalEscala){                      
        ...
        $strGrafico .= '<div style="left: ' .(($strValorAtualEscala)-(0.008*$numExtensaoTabela)) .'px; top: '.$numDiferencaAlturaRotuloPontas.'px; position:absolute;">'.InfraData::obterMesSiglaBR($arrDataAtualEscala[InfraData::$MES]).'/'.$arrDataAtualEscala[InfraData::$ANO].'</div>';                        
        $strGrafico .= '<div style="left: ' .($strValorAtualEscala) .'px; top: '.(($numAlturaBarras*($i))+$numEspacoNoTopoParaRotuloMeses+$numDiferencaAlturaRotuloPontas-$numTantoQueMarcacaoRotuloSaiPraForaDaPrimeiraBarra) .'px; height: '.($i < (count($arrDados)-1) ? $numAlturaBarras-2 : $numAlturaBarras+$numTantoQueMarcacaoRotuloSaiPraForaDaPrimeiraBarra-$numDiferencaAlturaRotuloPontas) .'px; border-right: 1px; border-right-style: dashed; position:absolute; z-index:1"></div>';
          
        $strValorAtualEscala += $numExtensaoUmMes;
        $strDataAtualEscala = InfraData::calcularData(1,InfraData::$UNIDADE_MESES,InfraData::$SENTIDO_ADIANTE,$strDataAtualEscala);
        $arrDataAtualEscala = InfraData::decomporData($strDataAtualEscala);
      }
      ...
      if ($arrFator[1] == "D") {
        if ($strMenorData == $dado['dataInicial']){
          ...
          $numInicioBarra =  ($numExtensaoUmMes*(InfraData::compararDatas($strMenorData,$dado['dataInicial'])/$numTamanhoMedioMes))/$arrFator[0];
        }
      ...
      $strGrafico .= '<div onmouseover="return infraTooltipMostrar(\''.$dado['dataInicial'] .' a ' .$dado['dataFinal'] .' (' .InfraData::formatarTimestamp($dado[1]*86400) .') ' .$dado[2] .'\',null,200)"  onmouseout="return infraTooltipOcultar();" 
