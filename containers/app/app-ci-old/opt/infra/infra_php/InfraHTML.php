...
class InfraHTML{
  ...
  public static function comparar($str1,$str2)
  {
    ...
    $diff->build();
    $ret=utf8_decode($diff->getDifference());
...
//    $arrayErrosPermitidos[]=201; //namespace not defined
  ...
  {
    return preg_replace_callback(self::$REGEXP_IMAGEM, 'self::md5Imagem', $strHtml);
  }
  /**
...
class HtmlDiff
{
  ...
  private function purifyHtml($html, $tags = null)
  {
    ...
    $doc = new DOMDocument();
    libxml_use_internal_errors(true);
    $doc->loadHTML(str_replace($tags, '', $html),LIBXML_NOWARNING|LIBXML_NOERROR|LIBXML_HTML_NODEFDTD|LIBXML_NOENT);
    libxml_clear_errors();
    $html = $doc->saveHTML($doc->getElementsByTagName('body')[0]);
    $html = $this->getStringBetween($html, '<body>','</body>');
    */
      ...
      switch ($mode) {
        case self::$MODE_CHARACTER: //61864
          if ($character==='<') { //911
            ...
            $current_word = '<';
            $mode = self::$MODE_TAG;
          } else if (preg_match('[^\s]', $character)) { //9342
            ...
            if (preg_match("/^[a-zA-Z0-9\pL]+$/u", $character) && (''===$current_word || preg_match('/[\p{L}\p{N}]+/u', $current_word))) { //47256
        ...
        case self::$MODE_WHITESPACE://22318
          if ($character==='<') {//2381
            ...
            $current_word = '<';
            $mode = self::$MODE_TAG;
          } else if (preg_match('/\s/u', $character)) {//9684
            $current_word .= $character;
  ...
  private function InsertTag($tag, $cssClass, &$words)
  {
    while (true) {
      ...
      if (count($nonTags) !== 0) {
        ...
        } else if (in_array($words[0], $this->specialCaseClosingTags,false) !== false) {
      ...
      if ((empty($words) || !is_array($words) || count($words) === 0) &&
          (empty($specialCaseTagInjection) || !is_array($specialCaseTagInjection) || count($specialCaseTagInjection) === 0)) {
        break;
      }
  ...
  {
    return $condition === 'tag' ? preg_match(InfraHTML::$REGEX_IS_TAG, $word) : !preg_match(InfraHTML::$REGEX_IS_TAG, $word);
  }
...
//  }
  ...
  private function FindMatch($startInOld, $endInOld, $startInNew, $endInNew)
  {
    ...
    if($this->oldWords[$startInOld]===$this->newWords[$startInNew]){
      ...
      return new Match($startInOld,$startInNew,$bestMatchSize);
    }
    ...
    if($this->oldWords[$endInOld-1]===$this->newWords[$endInNew-1]){
      ...
      return new Match($endInOld-$bestMatchSize,$endInNew-$bestMatchSize,$bestMatchSize);
    }
    ...
    if ($bestMatchSize===1 && $this->oldWords[$bestMatchInOld]===' ') return null;
