...
class InfraCasObject
{
    ...
    private function connect($http, $http_arguments, &$headers) {
        try {
            if (!isset($http_arguments))
                return $this->setError(CASTOR_LOG_LEVEL_ERROR, 0, CASTOR_ERROR_UNSPECIFIED_ERROR, "Erro para inicializar argumentos");
            ...
            if($error=="") {
                ...
                if($error=="") {
                    ...
                    if ( ($this->error_code<200) || ($this->error_code>=300) )
                        return $this->setError(CASTOR_LOG_LEVEL_INFO, $this->error_code, "Retorno de HTTP com erro: ".$http->response_message);
    ...
    private function __writeObjectSuccess(&$headers) {
        if (!$this->parseReadHeaders($headers))
            return $this->setError(CASTOR_LOG_LEVEL_ERROR, CASTOR_ERROR_UNAVAILABLE_NODES_ERROR, "Erro para interpretar cabeçalhos de retorno", $this);

        return true;
    }
    private function __readObjectSuccess($http, &$headers) {
        $this->parseReadHeaders($headers);

        if (isset($this->bodyfilename)) {
            $stream = fopen($this->bodyfilename, "w+");
            if ( $stream == false )
                return $this->setError(CASTOR_LOG_LEVEL_ERROR, CASTOR_ERROR_STREAM_ERROR, "Erro para abrir ".$this->bodyfilename." dados usando stream.");
            for(;;)
            {
                ...
                if(!@fwrite($stream, $body)) {
                    return $this->setError(CASTOR_LOG_LEVEL_ERROR, CASTOR_ERROR_STREAM_ERROR, "Erro para gravar dados usando stream.");
                }
            }
            fclose($stream);
        } else
        ...
        try {
            ...
            switch($operation) {
                ...
                case CASTOR_OPER_WRITE:
                    ...
                    if (!$result)
                        return $this->setError(CASTOR_LOG_LEVEL_ERROR, CASTOR_ERROR_UNSPECIFIED_ERROR, "Erro para inicializar argumentos de HTTP." );
                    break;
            ...
            if ($result) {
                switch($operation) {
                    ...
                    case CASTOR_OPER_DELETE:
                    case CASTOR_OPER_INFO:
                        $result=$this->__writeObjectSuccess($headers);
                        break;
                    case CASTOR_OPER_READ:
                        $result=$this->__readObjectSuccess($http, $headers);
                        break;
                }
                $http->Close(1);
                return $result;
