 ...
 */
  ...
  class InfraCodigoBarras {
  	
  	public static $TIPO_STD25 = 'std25';
  	public static $TIPO_INT25 = 'int25';
  	public static $TIPO_EAN8 = 'ean8';
  	public static $TIPO_EAN13 = 'ean13';
  	public static $TIPO_CODE11 = 'code11';
  	public static $TIPO_CODE39 = 'code39';
  	public static $TIPO_CODE93 = 'code93';
  	public static $TIPO_CODE128 = 'code128';
  	public static $TIPO_CODEBAR = 'codabar';
  	public static $TIPO_MSI = 'msi';
  	public static $TIPO_DATAMATRIX = 'datamatrix';
  
  	public static $COR_AZUL = 'A';
  	public static $COR_BRANCO = 'B';
  	public static $COR_PRETO = 'P';
  	public static $COR_VERMELHO = 'V';
  	
  	public static $FORMATO_GIF = 'gif';
  	public static $FORMATO_PNG = 'png';
  	...
  	static public function listarTiposDisponiveis(){
  		$arrTipos = array();
  		$arrTipos[] = InfraCodigoBarras::$TIPO_STD25;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_INT25;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_EAN8;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_EAN13;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_CODE11;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_CODE39;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_CODE93;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_CODE128;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_CODEBAR;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_MSI;
  		$arrTipos[] = InfraCodigoBarras::$TIPO_DATAMATRIX;
  		return $arrTipos;
  	}
  	  	  	
    static public function gerar($dados, $diretorio, $tipo = 'code39', $cor = 'P', $largura = 1, $altura = 25, $angulo = 0, $largura_imagem=350, $altura_imagem=50, $formato = 'gif'){
    	
    	//$largura_imagem = 350;
    	//$altura_imagem = 50;
    	
  		self::$imagem = imagecreatetruecolor($largura_imagem, $altura_imagem);
		  self::$black = ImageColorAllocate(self::$imagem,0x00,0x00,0x00);
		  self::$white = ImageColorAllocate(self::$imagem,0xff,0xff,0xff);
		  self::$red = ImageColorAllocate(self::$imagem,0xff,0x00,0x00);
		  self::$blue = ImageColorAllocate(self::$imagem,0x00,0x00,0xff);
		  imagefilledrectangle(self::$imagem, 0, 0, $largura_imagem, $altura_imagem, self::$white);
		  
    	switch ($cor) {
		  	case InfraCodigoBarras::$COR_AZUL:
		  		$cor = self::$blue;
		  		break;
		  	case InfraCodigoBarras::$COR_BRANCO:
		  		$cor = self::$white;
		  		break;
		  	case InfraCodigoBarras::$COR_PRETO:
		  		$cor = self::$black;
		  		break;
		  	case InfraCodigoBarras::$COR_VERMELHO:
		  		$cor = self::$red;
		  		break;		  
		  	default:
		  		$cor = self::$black;
		  ...
		  $centro_horizontal = bcdiv($largura_imagem, 2);
		  $centro_vertical = bcdiv($altura_imagem, 2);
		  
      self::_draw(self::$imagem, $cor, $centro_horizontal, $centro_vertical, $angulo, $tipo, $dados, $largura, $altura);
      
      //var_dump($_SERVER);die;
      $nome_arq = $tipo.'_'.InfraUtil::formatarNomeArquivo($dados).'.'.$formato;
      
      try{
        if ($formato==self::$FORMATO_GIF){
		  	  imagegif(self::$imagem, $diretorio.'/'.$nome_arq);
        }else if ($formato==self::$FORMATO_PNG){
          imagepng(self::$imagem, $diretorio.'/'.$nome_arq);
        }else{
          throw new InfraException('Formato ['.$formato.'] inválido.');
        }
      }catch(Exception $e){
      	throw new InfraException('Erro gerando código de barras.',$e);
			}
    ...
    static private function _draw($res, $color, $x, $y, $angle, $type, $datas, $width, $height){
      ...
      switch($type){
        case 'std25':
        case 'int25':
          $digit = InfraBarcodeI25::getDigit($code, $crc, $type);
          $hri = InfraBarcodeI25::compute($code, $crc, $type);
        break;
        case 'ean8':
        case 'ean13':
          $digit = InfraBarcodeEAN::getDigit($code, $type);
          $hri = InfraBarcodeEAN::compute($code, $type);
        break;
        case 'code11':
          $digit = InfraBarcode11::getDigit($code);
          $hri = $code;
        break;
        case 'code39':
          $digit = InfraBarcode39::getDigit($code);
          $hri = $code;
        break;
        case 'code93':
          $digit = InfraBarcode93::getDigit($code, $crc);
          $hri = $code;
        break;
        case 'code128':
          $digit = InfraBarcode128::getDigit($code);
          $hri = $code;
        break;
        case 'codabar':
          $digit = InfraBarcodeCodabar::getDigit($code);
          $hri = $code;
        break;
        case 'msi':
          $digit = InfraBarcodeMSI::getDigit($code, $crc);
          $hri = InfraBarcodeMSI::compute($code, $crc);
        break;
        case 'datamatrix':   
          $digit = InfraBarcodeDatamatrix::getDigit($code, $rect);
          $hri = $code;
          $b2d = true;
        break;
      }
