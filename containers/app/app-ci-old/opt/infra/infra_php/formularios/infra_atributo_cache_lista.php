...
try {
  ...
  if ($numRegistros > 0){
    ...
    if ($bolAcaoExcluir){
      ...
      $strLinkExcluir = SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_atributo_cache_excluir&acao_origem='.$_GET['acao']);
    }
    ...
    foreach($arrAtributos as $nome){
      ...
      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_atributo_cache_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&nome='.$nome).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getIconeConsultar().'" title="Consultar Atributo de Memória" alt="Consultar Atributo de Memória" class="infraImg" /></a>&nbsp;';
      }
      ...
      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaInfra::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getIconeExcluir().'" title="Excluir Atributo de Memória" alt="Excluir Atributo de Memória" class="infraImg" /></a>&nbsp;';
      }
...
?>
  <form id="frmInfraAtributoCacheLista" method="post"  onsubmit="return validarForm();"  action="<?=SessaoInfra::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
    <?
    ...
    <input type="text" id="txtAtributo" name="txtAtributo" class="infraText" value="<?=PaginaInfra::getInstance()->tratarHTML($strAtributo)?>" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />

    <label id="lblLimite" for="txtLimite" class="infraLabelOpcional">Máx. Retorno:</label>
    <input type="text" id="txtLimite" name="txtLimite" class="infraText" onkeypress="return infraMascaraNumero(this,event,5);" value="<?=PaginaInfra::getInstance()->tratarHTML($numLimite)?>" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />
