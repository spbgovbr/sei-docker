  ...
  try {
    ...
    switch($_GET['acao']){
      case 'infra_sessao_rest_selecionar':
        $strTitulo = PaginaInfra::getInstance()->getTituloSelecao('Selecionar Infra Sessão REST','Selecionar Infra Sessão REST');
    ...
    if ($numRegistros > 0){
      ...
      if ($bolAcaoExcluir){
        ...
        $strLinkExcluir = SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_sessao_rest_excluir&acao_origem='.$_GET['acao']);
      }
      ...
      $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSessaoRestDTO,'Sigla do Usuário','SiglaUsuario',$arrObjInfraSessaoRestDTO).'</th>'."\n";
      ...
      $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSessaoRestDTO,'Sigla do Órgão','SiglaOrgao',$arrObjInfraSessaoRestDTO).'</th>'."\n";
      $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSessaoRestDTO,'Data do Login','Login',$arrObjInfraSessaoRestDTO).'</th>'."\n";
      $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSessaoRestDTO,'Data do Último Acesso','Acesso',$arrObjInfraSessaoRestDTO).'</th>'."\n";
      $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSessaoRestDTO,'Data do Logout','Logout',$arrObjInfraSessaoRestDTO).'</th>'."\n";
      $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSessaoRestDTO,'User Agent','UserAgent',$arrObjInfraSessaoRestDTO).'</th>'."\n";
      ...
      for ($i = 0;$i < $numRegistros; $i++) {
        ...
        if ($bolCheck){
          $strResultado .= '<td valign="top">'.PaginaInfra::getInstance()->getTrCheck($i,$arrObjInfraSessaoRestDTO[$i]->getStrIdInfraSessaoRest(),$arrObjInfraSessaoRestDTO[$i]->getNumIdUsuario()).'</td>';
        }
        //$strResultado .= '<td>'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSessaoRestDTO[$i]->getNumIdUsuario()).'</td>';
        $strResultado .= '<td align="center">'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSessaoRestDTO[$i]->getStrSiglaUsuario()).'</td>';
        //$strResultado .= '<td>'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSessaoRestDTO[$i]->getNumIdOrgao()).'</td>';
        $strResultado .= '<td align="center">'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSessaoRestDTO[$i]->getStrSiglaOrgao()).'</td>';
        $strResultado .= '<td align="center">'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSessaoRestDTO[$i]->getDthLogin()).'</td>';
        $strResultado .= '<td align="center">'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSessaoRestDTO[$i]->getDthAcesso()).'</td>';
        $strResultado .= '<td align="center">'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSessaoRestDTO[$i]->getDthLogout()).'</td>';
        $strResultado .= '<td>'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSessaoRestDTO[$i]->getStrUserAgent()).'</td>';
        ...
        $strResultado .= PaginaInfra::getInstance()->getAcaoTransportarItem($i,$arrObjInfraSessaoRestDTO[$i]->getStrIdInfraSessaoRest());

        if ($bolAcaoConsultar){
          $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_sessao_rest_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_infra_sessao_rest='.$arrObjInfraSessaoRestDTO[$i]->getStrIdInfraSessaoRest()).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getIconeConsultar().'" title="Consultar Infra Sessão Rest" alt="Consultar Infra Sessão Rest" class="infraImg" /></a>&nbsp;';
        }
...
?>
<form id="frmInfraSessaoRestLista" method="post" action="<?=SessaoInfra::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
<?
...
?>
  <fieldset class="infraFieldset">
    ...
    <label class="infraLabelOpcional">Sigla do Usuário: </label><input type="text" size="10" name="txtSiglaUsuario" value="<?=$_POST['txtSiglaUsuario'];?>" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />
    &nbsp;&nbsp;&nbsp;
    <label class="infraLabelOpcional">Sigla do Órgão: </label><input type="text" size="10" name="txtSiglaOrgao" value="<?=$_POST['txtSiglaOrgao'];?>" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />
    &nbsp;&nbsp;&nbsp;
    <label class="infraLabelOpcional">Data do Login entre </label><input type="text" size="10" id="txtDataInicialLogin" name="txtDataInicialLogin" value="<?=$_POST['txtDataInicialLogin'];?>" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />
    <img src="src="<?=PaginaInfra::getInstance()->getIconeCalendario()?>" id="imgCalDataInicio" title="Selecionar Data Inicial" alt="Selecionar Data Inicial" class="infraImg" onclick="infraCalendario('txtDataInicialLogin',this);" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />
    <label class="infraLabelOpcional"> e </label><input type="text" size="10" id="txtDataFinalLogin" name="txtDataFinalLogin" value="<?=$_POST['txtDataFinalLogin'];?>" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />
    <img src="<?=PaginaInfra::getInstance()->getIconeCalendario()?>" id="imgCalDataFim" title="Selecionar Data Final" alt="Selecionar Data Final" class="infraImg" onclick="infraCalendario('txtDataFinalLogin',this);" tabindex="<?=PaginaInfra::getInstance()->getProxTabDados()?>" />
  </fieldset>
<?
