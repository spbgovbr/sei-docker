	...
	private function montarBarraSuperior(){
	  if ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA || $this->getTipoPagina()==self::$TIPO_PAGINA_SEM_MENU){
  	  ...
  	  if ($this->getStrTextoBarraSuperior()!=null){
  	    echo '<label>'.self::tratarHTML(InfraString::transformarCaixaAlta($this->getStrTextoBarraSuperior())).'</label>'."\n";
  	  }else{
  	    if ($this->getObjInfraSessao()!=null){
  	      echo '<label>'.self::tratarHTML(InfraString::transformarCaixaAlta($this->getObjInfraSessao()->getStrDescricaoOrgaoSistema())).'</label>'."\n";
  	    }
 	  	}
      echo '</div>'."\n";
	  }
	...
	private function montarBarraSistema() {
      ...
      if ($this->getStrTextoBarraSistema()!=null){
        echo '<label>'.self::tratarHTML($this->getStrTextoBarraSistema()).'</label>';
          ...
          if ($this->getObjInfraSessao()!=null){
            echo '<label>'.self::tratarHTML($this->getObjInfraSessao()->getStrSiglaSistema()).'</label>';
          }
        }
      }
	  	  echo '</div>'."\n".
