...
try {
  ...
  //}
	...
	PaginaSip::getInstance()->prepararOrdenacao($objRegraAuditoriaDTO, 'IdRegraAuditoria', InfraDTO::$TIPO_ORDENACAO_ASC);			
  ...
  PaginaSip::getInstance()->processarPaginacao($objRegraAuditoriaDTO);
    ...
    if ($_GET['acao']=='regra_auditoria_reativar'){
      ...
      if ($bolCheck){
				$strResultado .= '<td valign="top">'.PaginaSip::getInstance()->getTrCheck($i,$arrObjRegraAuditoriaDTO[$i]->getNumIdRegraAuditoria(),$arrObjRegraAuditoriaDTO[$i]->getNumIdRegraAuditoria()).'</td>';
			}
