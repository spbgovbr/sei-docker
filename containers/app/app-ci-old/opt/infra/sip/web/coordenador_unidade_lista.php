  ...
  switch($_GET['acao']){
	...
	if ($numRegistros > 0){
		...
		for($i = 0;$i < $numRegistros; $i++){
			...
			if ($bolCheck){
				$strResultado .= '<td valign="top">'.PaginaSip::getInstance()->getTrCheck($i,$arrObjCoordenadorUnidadeDTO[$i]->getNumIdSistema().'#'.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdUsuario().'#'.$arrObjCoordenadorUnidadeDTO[$i]->getNumIdUnidade(),'').'</td>';
			}
...
<form id="frmCoordenadorUnidadeLista" method="post" action="<?=SessaoSip::getInstance()->assinarLink('coordenador_unidade_lista.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  <select id="selOrgaoUnidade" name="selOrgaoUnidade" onchange="trocarOrgaoUnidade(this);" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" >
