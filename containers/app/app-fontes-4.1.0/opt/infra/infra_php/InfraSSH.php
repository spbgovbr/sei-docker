 ...
 * Esta classe deve ser extendida por cada aplicação.
 *
...
abstract class InfraSSH implements InfraIProtocoloComunicacao
{
    ...
    /**
     * Abrir conexão com servidor SSH.
     *
     * @param boolean $bolConexaoSegura Não utilizado por esta implementação.
     * @param string $strIdConexao ID da conexão
     *
     */
        ...
        try {
            ...
            if (!$objConexao) {
                throw new InfraException(
                    'Não foi possível abrir conexão com o servidor ' . $this->getServidor(
                        $strIdConexao
                    ) . ':' . $this->getPorta($strIdConexao) . '].'
                );
            } else {
                $objValidacao = ssh2_auth_password(
                    $objConexao,
                    $this->getUsuario($strIdConexao),
                    $this->getSenha($strIdConexao)
                );
                if (!$objValidacao) {
                    throw new InfraException(
                        'Combinação usuário/senha inválida [Conexão ' . $this->getServidor($strIdConexao) . '].'
                    );
                } else {
     ...
     * A implementação atual pode não retornar o tamanho correto do arquivo se este tiver mais de 2GB.
     *
    ...
    function enviarArquivo($strArquivoLocal, $strArquivoRemoto)
    {
        try {
            ...
            // ssh2_scp_send tem um bug nesta versão, pode não copiar pedaços do arquivo
