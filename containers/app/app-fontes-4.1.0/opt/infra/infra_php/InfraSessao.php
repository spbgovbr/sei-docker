...
abstract class InfraSessao
{
    ...
    public function getStrPrefixoCookie()
    {
        return str_replace(
            ' ',
            '_',
            $this->getStrSiglaOrgaoSistema() . '_' . $this->getStrSiglaSistema() . '_' . $this->getStrSiglaUsuario()
        );
    }
    ...
    public function trocarUnidadeAtual()
    {
        if ($this->isBolHabilitada() && $this->getObjInfraSessaoDTO() !== null) {
            ...
            if (isset($_POST['selInfraUnidades'])) {
            ...
            } elseif ($this->getNumIdUnidadeAtual() === null) {
                ...
                if ($this->isBolHabilitada() && $this->getObjInfraIBanco() != null) {
                    try {
                        $objInfraDadoUsuario = new InfraDadoUsuario($this);
                        $this->getObjInfraSessaoDTO()->setNumIdUnidadeAtual(
                            $objInfraDadoUsuario->getValor('INFRA_UNIDADE_ATUAL')
                        );
                    } catch (Exception $e) {
                    }
                }
                ...
                if ($this->getNumIdUnidadeAtual() == null) {
                    ...
                    if ($this->isBolHabilitada() && $this->getNumIdUnidadeAtual() != null && $this->getObjInfraIBanco(
                        ) != null) {
                        try {
                            $objInfraDadoUsuario = new InfraDadoUsuario($this);
                            $objInfraDadoUsuario->setValor('INFRA_UNIDADE_ATUAL', $this->getNumIdUnidadeAtual());
    ...
    private function carregarSessao()
    {
        ...
        if (($obj = $objInfraSip->validarLogin(
                $_GET['id_login'],
                $_GET['id_sistema'],
                $_GET['id_usuario'],
                $this->gerarHashAgente()
            )) == null) {
            return false;
        }
        //}catch(Exception $e){
        //if ($this->getObjInfraLog()!=null){
        //  $this->getObjInfraLog()->gravar(InfraException::inspecionar($e));
        //}
        //throw $e;
        //}
    ...
    private function gerarObjInfraSessaoDTO($obj)
    {
        ...
        if (InfraDebug::isBolProcessar()) {
            InfraDebug::getInstance()->gravarInfra(
                'ID do Órgão do Sistema: ' . $objInfraSessaoDTO->getNumIdOrgaoSistema()
            );
            InfraDebug::getInstance()->gravarInfra(
                'Sigla do Órgão do Sistema: ' . $objInfraSessaoDTO->getStrSiglaOrgaoSistema()
            );
            InfraDebug::getInstance()->gravarInfra(
                'Descrição do Órgão do Sistema: ' . $objInfraSessaoDTO->getStrDescricaoOrgaoSistema()
            );
            ...
            InfraDebug::getInstance()->gravarInfra(
                'ID do Órgão do Usuário: ' . $objInfraSessaoDTO->getNumIdOrgaoUsuario()
            );
            InfraDebug::getInstance()->gravarInfra(
                'Sigla do Órgão do Usuário: ' . $objInfraSessaoDTO->getStrSiglaOrgaoUsuario()
            );
            InfraDebug::getInstance()->gravarInfra(
                'Descrição do Órgão do Usuário: ' . $objInfraSessaoDTO->getStrDescricaoOrgaoUsuario()
            );
            InfraDebug::getInstance()->gravarInfra(
                'ID do Contexto do Usuário: ' . $objInfraSessaoDTO->getNumIdContextoUsuario()
            );
            ...
            InfraDebug::getInstance()->gravarInfra(
                'Nome Registro Civil do Usuário: ' . $objInfraSessaoDTO->getStrNomeRegistroCivilUsuario()
            );
            InfraDebug::getInstance()->gravarInfra(
                'Nome Social do Usuário: ' . $objInfraSessaoDTO->getStrNomeSocialUsuario()
            );
            InfraDebug::getInstance()->gravarInfra('ID Origem Usuário: ' . $objInfraSessaoDTO->getStrIdOrigemUsuario());
            InfraDebug::getInstance()->gravarInfra(
                'ID do Órgão do Usuário Emulador: ' . $objInfraSessaoDTO->getNumIdOrgaoUsuarioEmulador()
            );
            InfraDebug::getInstance()->gravarInfra(
                'Sigla do Órgão do Usuário Emulador: ' . $objInfraSessaoDTO->getStrSiglaOrgaoUsuarioEmulador()
            );
            InfraDebug::getInstance()->gravarInfra(
                'Descrição do Órgão do Usuário Emulador: ' . $objInfraSessaoDTO->getStrDescricaoOrgaoUsuarioEmulador()
            );
            InfraDebug::getInstance()->gravarInfra(
                'ID do Usuário Emulador: ' . $objInfraSessaoDTO->getNumIdUsuarioEmulador()
            );
            InfraDebug::getInstance()->gravarInfra(
                'Sigla do Usuário Emulador: ' . $objInfraSessaoDTO->getStrSiglaUsuarioEmulador()
            );
            InfraDebug::getInstance()->gravarInfra(
                'Nome do Usuário Emulador: ' . $objInfraSessaoDTO->getStrNomeUsuarioEmulador()
            );
            ...
            InfraDebug::getInstance()->gravarInfra(
                'Versão SIP: ' . $objInfraSessaoDTO->getNumVersaoSip(
                ) . ' (InfraPHP ' . $objInfraSessaoDTO->getNumVersaoInfraSip() . ')'
            );
        }
    ...
    public function setObjInfraSessaoDTO($objInfraSessaoDTO)
    {
        ...
        $_SESSION['INFRA_SESSAO'][$objInfraSessaoDTO->getStrSiglaOrgaoSistema()][$objInfraSessaoDTO->getStrSiglaSistema(
        )] = $objInfraSessaoDTO;
    ...
    public function getObjInfraSessaoDTO()
    {
        if (self::$objInfraSessaoDTO == null) {
            if (isset($_SESSION['INFRA_SESSAO'][$this->getStrSiglaOrgaoSistema()][$this->getStrSiglaSistema()])) {
                self::$objInfraSessaoDTO = $_SESSION['INFRA_SESSAO'][$this->getStrSiglaOrgaoSistema(
                )][$this->getStrSiglaSistema()];
            }
        }
    ...
    public function assinarLink($strLink)
    {
        ...
        if ($numIdSistema != null) {
            ...
            if ($numPosParam === false) {
                $strLink .= '?infra_sistema=' . $numIdSistema;
            } else {
                $strLink .= '&infra_sistema=' . $numIdSistema;
            }
        }

        $numIdUnidadeAtual = $this->getNumIdUnidadeAtual();
        if ($numIdUnidadeAtual != null) {
            $numPosParam = strpos($strLink, '?');
            if ($numPosParam === false) {
                $strLink .= '?infra_unidade_atual=' . $numIdUnidadeAtual;
            } else {
                $strLink .= '&infra_unidade_atual=' . $numIdUnidadeAtual;
            }
        }
        ...
        if ($numPosParam !== false) {
            ...
            $strLink .= '&infra_hash=' . $this->gerarHashExterno($strParam) . ((substr(
                        $strLink,
                        0,
                        4
                    ) == 'http') ? $this->gerarHashUsuario() : '');
        }
    ...
    public function validarLink($strLink = null)
    {
        ...
        if (isset($_GET['infra_sistema']) && isset($_GET['infra_unidade_atual']) && $_GET['infra_sistema'] == $this->getNumIdSistema(
            ) && $_GET['infra_unidade_atual'] != $this->getNumIdUnidadeAtual()) {
            ...
            $strMsg = $this->getStrMsgDetectadaTrocaUnidade(
                $this->getStrSiglaUnidade($_GET['infra_unidade_atual']),
                $this->getStrSiglaUnidadeAtual()
            );
    ...
    public function gerarLinkLogin()
    {
        ...
        return $this->getStrPaginaLogin() . $strSeparador . 'sigla_orgao_sistema=' . $this->getStrSiglaOrgaoSistema(
            ) . '&sigla_sistema=' . $this->getStrSiglaSistema();
    }

    public function setPropriedade($strChave, $strPropriedade, $strValor)
    {
        $this->getObjInfraSessaoDTO()->setPropriedade(
            $this->getStrSiglaSistema(),
            $strChave,
            $strPropriedade,
            $strValor
    ...
    public function validarAuditarPermissao($strNomeRecurso, $strOperacao = null, $varParametro = null)
    {
        ...
        return $this->getObjInfraAuditoria()->auditar($strNomeRecurso, $strOperacao, $varParametro);
    }
        ...
        if (!$this->verificarPermissao($strNomeRecurso)) {
            ...
            if ($numIdUnidade !== null) {
                throw new InfraException(
                    'Acesso negado a este recurso nesta unidade (' . $strNomeRecurso . ' / ' . $strSiglaUnidade . ').'
                );
            } else {
    ...
    public function getStrSiglaOrgaoUnidadeAtual()
    {
        if (($objInfraSessaoDTO = $this->getObjInfraSessaoDTO()) != null) {
            ...
            return $arrOrgaos[$arrUnidades[$objInfraSessaoDTO->getNumIdUnidadeAtual(
            )][InfraSip::$WS_LOGIN_PERMISSAO_UNIDADES_ID_ORGAO]][InfraSip::$WS_LOGIN_ORGAO_SIGLA];
        }
        return null;
    }
    ...
    public function getStrDescricaoOrgaoUnidadeAtual()
    {
        if (($objInfraSessaoDTO = $this->getObjInfraSessaoDTO()) != null) {
            ...
            $arrOrgaos = $objInfraSessaoDTO->getArrOrgaos();
            return $arrOrgaos[$arrUnidades[$objInfraSessaoDTO->getNumIdUnidadeAtual(
            )][InfraSip::$WS_LOGIN_PERMISSAO_UNIDADES_ID_ORGAO]][InfraSip::$WS_LOGIN_ORGAO_DESCRICAO];
        }
        return null;
    }
