...
class InfraData
{
    ...
    {
        if ($strDataHoraIni === null || $strDataHoraFim === null || trim($strDataHoraIni) === '' || trim(
                $strDataHoraFim
            ) === '') {
            return null;
        }
    ...
    public static function formatarExtenso4($strDataHora)
    {
        ...
        return $strDiaSemana . ', ' . substr($strDataHora, 0, 2) . ' de ' . $strMes . ' às ' . substr(
                $strDataHora,
                11,
                5
            ) . '.';
    ...
    public static function calcularData($quantidade, $unidade, $sentido, $strData = null)
    {
        ...
        } elseif (!self::validarData($strData)) {
    ...
    public static function gerarIntervaloMes($numMes, $numAno)
    {
        ...
        if (($numAno >= 1800) && ($numAno <= 2100)) {
            ...
            return array(
                'DtaInicial' => ('01/' . $numMes . '/' . $numAno),
                'DtaFinal' => ($numDia . '/' . $numMes . '/' . $numAno)
            );
        } else {
            return null;
        }
    ...
    public static function obterUltimoDiaMes($numMes, $numAno)
    {
        ...
        return substr(self::calcularData(1, self::$UNIDADE_DIAS, self::$SENTIDO_ATRAS, '01/' . $numMes . '/' . $numAno), 0, 2);
    }
    ...
    {
        if (self::compararDatasSimples($dtaDataInicio1, $dtaDataFim1) < 0 || self::compararDatasSimples(
                $dtaDataInicio2,
                $dtaDataFim2
            ) < 0) {
        ...
        if (self::compararDatasSimples($dtaDataInicio1, $dtaDataFim2) < 0 || self::compararDatasSimples(
                $dtaDataFim1,
                $dtaDataInicio2
            ) > 0) {
        ...
        if (self::compararDatasSimples($dtaDataInicio1, $dtaDataInicio2) <= 0 && self::compararDatasSimples(
                $dtaDataFim1,
                $dtaDataFim2
            ) >= 0) {
    ...
    public static function converterDataEmExcelTimestamp($strData)
    {
        ...
        $strData2 = substr($strData1, 6, 4) . '-' . substr($strData1, 3, 2) . '-' . substr(
                $strData1,
                0,
                2
            ) . ' ' . substr($strData1, 11);
     ...
     */
    public static function validarPeriodo(
        $dtaInicial,
        $dtaFinal,
        InfraException $objInfraException,
        $bolDtaInicialFutura = false,
        $bolDtaFinalFutura = false
    ) {
        if (!InfraString::isBolVazia($dtaInicial) || !InfraString::isBolVazia($dtaFinal)) {
            if (InfraString::isBolVazia($dtaInicial) || InfraString::isBolVazia($dtaFinal)) {
                ...
                if (!$bolValidaDataInicial || !$bolValidaDataFinal) {
                    ...
                    $bolFuturoDataInicial = !$bolDtaInicialFutura && (self::compararDatas(
                                self::getStrDataAtual(),
                                $dtaInicial
                            ) > 0);
                    $bolFuturoDataFinal = !$bolDtaFinalFutura && (self::compararDatas(
                                self::getStrDataAtual(),
                                $dtaFinal
                            ) > 0);
    ...
    public static function buscarDomingoPascoa($numAno)
    {
        ...
        $arrValoresReferencia = array(
            array('AnoInicio' => 1582, 'AnoFim' => 1599, 'X' => 22, 'Y' => 2),
