...
/** Funções utilitárias para a manipulação de certificados digitais */
        ...
        while (strlen($data) > 1) {
            ...
            switch ($class) {
                ...
                case 0x03:
                    ...
                    $result[] = array(
                        'bit string (' . $len . ')',
                        'UnsedBits:' . ord($bitstring_data[0]) . ':' . ord(
                            $bitstring_data[1]
                        )
                    );
                    break;
...
# Recupera dados da oid passada como parametro.....
        ...
        if (count($ret)) {
            ...
            foreach ($ret as $group) {
                if (substr(
                        ...
                        0,
                        17
                    ) == 'Context Especific')  // primeiro indice tem de ter o valor  'Context Especific' ...
                {
                    $oid = explode(':', $group[1][0][0][0]);   //  Pega o numero da oid.
                    $dados = array_merge(
                        self::parse(trim($oid[1]), $group[1][0][1][1][0][1]),
                        $dados
                    ); // Passa a oid e o seu valor para ser parseado....
                }
        ...
        if ($AUX[0][$i][1][1]) {
            if (is_array($AUX[0][$i][1][1])) {
                ...
                foreach ($AUX[0][$i][1][1] as $crl) {
                    if (substr($crl[1][0][1][0][1][0][1], 0, 4) == 'http' || substr(
                            $crl[1][0][1][0][1][0][1],
                            0,
                            4
                        ) == 'ldap') {
    ...
    private static function BASICCONSTRAINTS($xx, $certificado_digital_formato_der)
    {
        $dados = array();
        if (isset($certificado_digital_formato_der)) {
            $AUX = self::recuperarDadosOID(
                $certificado_digital_formato_der,
                '2.5.29.19'
            );  // busca oid do BasicConstraints
            if (count($AUX) > 0) {
    ...
    private static function recuperarDadosDER($cert_der)
    {
        ...
        foreach ($funcoes as $funcao => $parametro) {
            $dados = array_merge(
                $dados,
                call_user_func(array('InfraCertificadoDigital', $funcao), $cert_data, $parametro)
            );
        }
