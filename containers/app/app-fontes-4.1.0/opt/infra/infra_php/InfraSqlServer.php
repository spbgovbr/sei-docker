...
abstract class InfraSqlServer implements InfraIBanco
{
    ...
    {
        $rs = $this->consultarSql(
            'INSERT INTO ' . $sequencia . ' OUTPUT CAST(INSERTED.id as VARCHAR) as \'id\' VALUES (null);'
        );
        return $rs[0]['id'];
    }
    ...
    public function formatarPesquisaStr($strTabela, $strCampo, $strValor, $strOperador, $bolCaseInsensitive, $strBind)
    {
        if ($bolCaseInsensitive) {
            ...
            if ($strBind == null) {
                return $strCampo . ' ' . $strOperador . ' \'' . str_replace(
                        "'",
                        '\'\'',
                        str_replace("\'", '\'', $strValor)
                    ) . '\' ';
            } else {
        ...
        try {
            ...
            if ($this->numTipoInstalacao == self::$TI_SQLSRV) {
                ...
                $connectionInfo = array(
                    "Database" => $this->getBanco(),
                    "UID" => $this->getUsuario(),
                    "PWD" => $this->getSenha(),
                    'MultipleActiveResultSets' => false,
                    'CharacterSet' => SQLSRV_ENC_CHAR
                );
                if (($this->conexao = sqlsrv_connect(
            ...
            } elseif ($this->numTipoInstalacao == self::$TI_FREETDS) {
                $this->conexao = mssql_connect(
                    $this->getServidor() . ':' . $this->getPorta(),
                    $this->getUsuario(),
                    $this->getSenha()
                );
                ...
                $this->conexao = new PDO(
                    'dblib:host=' . $this->getServidor() . ':' . $this->getPorta() . ';dbname=' . $this->getBanco(),
                    $this->getUsuario(),
                    $this->getSenha()
                );
            }
    ...
    public function consultarSql($sql, $arrCamposBind = null)
    {
        if (InfraDebug::isBolProcessar()) {
            InfraDebug::getInstance()->gravarInfra(
                '[InfraSqlServer->consultarSql] ' . InfraBD::formatarDetalhesSql($sql, $arrCamposBind)
            );
        ...
        if (InfraDebug::isBolProcessar()) {
            ...
            InfraDebug::getInstance()->gravarInfra(
                '[InfraSqlServer->consultarSql] ' . InfraUtil::formatarMilhares(
        ...
        if (strpos(strtoupper($select), 'DISTINCT') === false) {
            ...
            $arrSelect = explode(
                ...
                str_replace(
                    ',',
                    ' ',
                    str_replace('CAST(', '', str_replace(' as varchar)', '', $select))
                )
            );
            ...
            for ($i = 0; $i < count($arrOrder); $i++) {
                ...
                for ($j = 0; $j < count($arrSelect); $j++) {
                    if ($arrSelect[$j] == $arrOrder[$i] && isset($arrSelect[$j + 1]) && strtoupper(
                            $arrSelect[$j + 1]
                        ) == 'AS' && isset($arrSelect[$j + 2])) {
                        $order .= $arrSelect[$j + 2];
                        break;
                    }
                }
    ...
    public function executarSql($sql, $arrCamposBind = null)
    {
        if (InfraDebug::isBolProcessar()) {
            InfraDebug::getInstance()->gravarInfra(
                '[InfraSqlServer->executar] ' . InfraBD::formatarDetalhesSql($sql, $arrCamposBind)
            );
            $numSeg = InfraUtil::verificarTempoProcessamento();
        }
  ...
  ;mssql.datetimeconvert = On*/
    ...
    public function formatarPesquisaFTS($strPalavras)
    {
        ...
        for ($i = 0; $i < count($arrDados); $i++) {
            if (strpos($arrDados[$i], " ") !== false || strpos($arrDados[$i], ",") !== false || (strpos(
                        $arrDados[$i],
                        "*"
                    ) !== false && strpos($arrDados[$i], "\"") === false)) {
        ...
        for ($i = 0; $i < count($arrDados); $i++) {
            ...
            if ($i > 0) {
                if (!in_array($arrDados[$i - 1], array('and', 'or', 'and not', 'near', '(')) && !in_array(
                        $arrDados[$i],
                        array('and', 'or', 'and not', 'near', ')')
                    )) {
    ...
    public function criarSequencialNativa($strSequencia, $numInicial)
    {
        ...
        $this->executarSql(
            'create table ' . $strSequencia . ' (id int identity(' . $numInicial . ',1), campo char(1) null)'
        );
    }
