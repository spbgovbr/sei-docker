...
class PHPExcel_Calculation_Engineering {
	...
	public static function _parseComplex($complexNumber) {
		...
		if (strlen($workString) > 0) {
            $current = $workString[0];
			$leadingSign = $current == '+' || $current == '-' ? 1 : 0;
		}
		$power = '';
	...
	private static function _cleanComplex($complexNumber) {
        $caracter = $complexNumber[0];
		if ($caracter == '+') $complexNumber = substr($complexNumber,1);
		if ($caracter == '0') $complexNumber = substr($complexNumber,1);
		if ($caracter == '.') $complexNumber = '0'.$complexNumber;
