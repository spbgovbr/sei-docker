...
class InfraBarraProgresso2
{
    ...
    private static $bolPreparar = false;
    ...
    public static function newInstance($strNome, $arrParametros)
    {
        ...
        if (!self::$bolPreparar) {
            $obj->numStatus = self::STATUS_CLOSED_CLIENT;
        } else {
    ...
    {
        if ($this->numStatus == self::STATUS_CLOSED_CLIENT || $this->numStatus == self::STATUS_SERVER_CLOSED) {
            return false;
        }
    ...
    private function getData()
    {
        ...
        if (self::$bolFecharJanela != null) {
            $ret['fechar'] = self::$bolFecharJanela;
        }

        if (self::$strUrlRedirecionamento != null) {
            $ret['redirect'] = self::$strUrlRedirecionamento;
        }
    ...
    {
        if ($this->numStatus == self::STATUS_ABORTED_CLIENT) {
            return true;
        }
    ...
    {
        try {
            if (is_file($this->strArquivo)) {
                unlink($this->strArquivo);
            }
        } catch (Exception $e) {
        }
        return '';
