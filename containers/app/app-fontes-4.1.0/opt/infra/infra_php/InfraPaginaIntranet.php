...
abstract class InfraPaginaIntranet extends InfraPagina
{
    ...
    function montarAreaAbas()
    {
        ...
        if ($this->getLayoutIntranet()) {
            ...
            echo "<img src=\"" . $this->getDiretorioImagens(
                ) . "cabecalho_sistema.jpg\" alt=\"Cabeçalho do Sistema\"/>";
        }
        echo "</div>\n";
    }
        ...
        for ($i = 0; $i < count($arrObjAvisoDTO); $i++) {
            ...
            if (($i % 2) == 0) {
                ...
                if ($arrObjAvisoDTO[$i]->getStrTitulo() != " ") {
                    echo "<strong style=\"color: rgb(255, 255, 255);\">" . $arrObjAvisoDTO[$i]->getStrTitulo(
                        ) . "</strong><br/><br/>";
                }
                if ($strLink == "") {
                    echo $arrObjAvisoDTO[$i]->getStrConteudo();
                } else {
                    echo "<a target=\"_blank\" href=\"" . $strLink . "\">" . $arrObjAvisoDTO[$i]->getStrConteudo(
                        ) . "</a>";
                }
                echo "</div>";
    ...
    function fecharBody($mostrarEndereco = 0)
    {
        ...
        if ($this->bolExibirMensagens) {
            ...
            if (isset($_GET['msg'])) {
                if ($_GET['msg'] != '') {
                    $strAlert .= 'alert(\'' . str_replace(
                            '\&quot;',
                            '"',
                            InfraString::formatarJavaScript($_GET['msg'])
                        ) . '\');';
                }
            }
            if ($this->getStrMensagens() != '') {
                $strAlert .= 'alert(\'' . str_replace(
                        '\&quot;',
                        '"',
                        InfraString::formatarJavaScript($this->getStrMensagens())
                    ) . '\');';
            }
            if ($strAlert != '') {
