...
abstract class InfraFTP implements InfraIProtocoloComunicacao
{
    ...
    /**
     * Abrir conexão com servidor FTP.
     *
     * @param boolean $bolConexaoSegura Parametro que define a utilização de SSL ou não na conexão.
        ...
        try {
            ...
            if (!$objConexao) {
                throw new InfraException(
                    'Não foi possível abrir a conexão FTP [(SSL: ' . $bolConexaoSegura . ') ' . $this->getServidor(
                        $strIdConexao
                    ) . ':' . $this->getPorta($strIdConexao) . '].'
                );
            } else {
                $objValidacao = ftp_login(
                    $objConexao,
