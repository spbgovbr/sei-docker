...
try {
    ...
    switch ($_GET['acao']) {
        case 'infra_banco_comparar':
            ...
            $arrComandos[] = '<button type="submit" name="sbmVerificarComparacaoBancoDireta" value="Comparação Direta" class="infraButton">Comparação Direta</button>';
            $arrComandos[] = '<button type="submit" name="sbmVerificarComparacaoBancoInversa" value="Comparação Inversa" class="infraButton">Comparação Inversa</button>';
            //$arrComandos[] = '<button type="button" name="btnCancelar" value="Cancelar" onclick="location.href=\''.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=sistema_listar')).'\';" class="infraButton" />';
            ...
            $objInfraComparacaoBancoDTO->setStrSinComparaQtdeRegistrosTabela(
                PaginaInfra::getInstance()->getCheckbox($_POST['chkSinComparaQtdeRegistrosTabela'])
            );
            $objInfraComparacaoBancoDTO->setStrSinComparaTipoColunasTabela(
                PaginaInfra::getInstance()->getCheckbox($_POST['chkSinComparaTipoColunasTabela'])
            );
            $objInfraComparacaoBancoDTO->setStrSinComparaMaxIdTabela(
                PaginaInfra::getInstance()->getCheckbox($_POST['chkSinComparaMaxIdTabela'])
            );
            $objInfraComparacaoBancoDTO->setStrSinExibirSomenteDiferencas(
                PaginaInfra::getInstance()->getCheckbox($_POST['chkSinExibirSomenteDiferencas'])
            );
            ...
            if (isset($_POST['sbmVerificarComparacaoBancoDireta']) || isset($_POST['sbmVerificarComparacaoBancoInversa'])) {
                try {
                    ...
                    $arrObjInfraComparacaoBancoDTO_Tabela = $objInfraComparacaoBancoRN->compararTabelas(
                        $objInfraComparacaoBancoDTO
                    );

                    $arrObjInfraComparacaoBancoDTO_Constraint = $objInfraComparacaoBancoRN->compararConstraints(
                        $objInfraComparacaoBancoDTO
                    );

                    $arrObjInfraComparacaoBancoDTO_Indice = $objInfraComparacaoBancoRN->compararIndices(
                        $objInfraComparacaoBancoDTO
                    );

                    if ($objInfraComparacaoBancoDTO->getStrSinComparaMaxIdTabela() == 'S') {
                        $arrObjInfraComparacaoBancoDTO_Sequencia = $objInfraComparacaoBancoRN->compararSequencias(
                            $objInfraComparacaoBancoDTO
                        );
                    }
                } catch (Exception $e) {
    ...
    if ($numRegistros > 0) {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            if ($arrObjInfraComparacaoBancoDTO_Tabela[$i]->getStrSinColunasTabelaDestinoOK() == 'S'
                && ($objInfraComparacaoBancoDTO->getStrSinComparaQtdeRegistrosTabela(
                    ) == 'N' || $arrObjInfraComparacaoBancoDTO_Tabela[$i]->getStrSinQtdeRegistrosTabelaDestinoOK(
                    ) == 'S')) {
            ...
            $strResultadoTabela .= '<td>' . $arrObjInfraComparacaoBancoDTO_Tabela[$i]->getStrNomeTabelaOrigem(
                ) . '</td>';
            $strResultadoTabela .= '<td>' . $arrObjInfraComparacaoBancoDTO_Tabela[$i]->getStrColunasTabelaOrigem(
                ) . '</td>';
            ...
            if ($objInfraComparacaoBancoDTO->getStrSinComparaQtdeRegistrosTabela() == 'S') {
                ...
                if ($arrObjInfraComparacaoBancoDTO_Tabela[$i]->getNumQtdeRegistrosTabelaOrigem(
                    ) != $arrObjInfraComparacaoBancoDTO_Tabela[$i]->getNumQtdeRegistrosTabelaDestino()) {
        ...
        if ($numRegistrosTabela) {
            $strCabecalhoTabela = '<table width="99%" class="infraTable" summary="' . $strSumarioTabela . '">' . "\n" .
                $strCabecalhoTabela .= '<caption class="infraCaption">' . PaginaInfra::getInstance(
                    )->gerarCaptionTabela($strCaptionTabela, $numRegistrosTabela) . '</caption>';
    ...
    if ($numRegistros > 0) {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            $strResultadoConstraint .= '<td>' . $arrObjInfraComparacaoBancoDTO_Constraint[$i]->getStrNomeTabelaOrigem(
                ) . '</td>';
            $strResultadoConstraint .= '<td>' . $arrObjInfraComparacaoBancoDTO_Constraint[$i]->getStrNomeConstraintOrigem(
                ) . '</td>';
            $strResultadoConstraint .= '<td>' . $arrObjInfraComparacaoBancoDTO_Constraint[$i]->getStrNomeColunasConstraintOrigem(
                ) . '</td>';
        ...
        if ($numRegistrosConstraint) {
            ...
            $strCabecalhoConstraint .= '<caption class="infraCaption">' . PaginaInfra::getInstance(
                )->gerarCaptionTabela($strCaptionTabela, $numRegistrosConstraint) . '</caption>';
    ...
    if ($numRegistros > 0) {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            $strResultadoIndice .= '<td>' . $arrObjInfraComparacaoBancoDTO_Indice[$i]->getStrNomeTabelaOrigem(
                ) . '</td>';
            $strResultadoIndice .= '<td>' . PaginaInfra::getInstance()->tratarHTML(
                    $arrObjInfraComparacaoBancoDTO_Indice[$i]->getStrNomeIndiceOrigem()
                ) . '</td>';
            $strResultadoIndice .= '<td align="center">' . $arrObjInfraComparacaoBancoDTO_Indice[$i]->getStrUnique(
                ) . '</td>';
            $strResultadoIndice .= '<td>' . $arrObjInfraComparacaoBancoDTO_Indice[$i]->getStrColunasIndiceOrigem(
                ) . '</td>';
        ...
        if ($numRegistrosIndice) {
            ...
            $strCabecalhoIndice .= '<caption class="infraCaption">' . PaginaInfra::getInstance()->gerarCaptionTabela(
                    $strCaptionTabela,
                    $numRegistrosIndice
                ) . '</caption>';
    ...
    if ($objInfraComparacaoBancoDTO->getStrSinComparaMaxIdTabela() == 'S') {
        ...
        if ($numRegistros > 0) {
            ...
            for ($i = 0; $i < $numRegistros; $i++) {
                if ($arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaSequenciaOrigem(
                    ) == $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaSequenciaDestino() &&
                    $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaSequenciaOrigem() != '[erro]' &&
                    $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaOrigem(
                    ) == $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaDestino() &&
                    ($arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaSequenciaOrigem(
                        ) == null || $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaOrigem(
                        ) <= $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaSequenciaOrigem()) &&
                    ($arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaSequenciaDestino(
                        ) == null || $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaDestino(
                        ) <= $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaSequenciaDestino())) {
                    if ($objInfraComparacaoBancoDTO->getStrSinExibirSomenteDiferencas() == 'S') {
                        continue;
                    }
                ...
                $strResultadoSequencia .= '<td>' . $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getStrNomeTabelaOrigem(
                    ) . '</td>';
                $strResultadoSequencia .= '<td align="center">' . $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaOrigem(
                    ) . '</td>';
                $strResultadoSequencia .= '<td align="center">' . $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaDestino(
                    ) . '</td>';
                $strResultadoSequencia .= '<td align="center">' . $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaSequenciaOrigem(
                    ) . '</td>';
                $strResultadoSequencia .= '<td align="center">' . $arrObjInfraComparacaoBancoDTO_Sequencia[$i]->getNumMaxIdTabelaSequenciaDestino(
                    ) . '</td>';
            ...
            if ($numRegistrosSequencia) {
                ...
                $strCabecalhoSequencia .= '<caption class="infraCaption">' . PaginaInfra::getInstance(
                    )->gerarCaptionTabela($strCaptionTabela, $numRegistrosSequencia) . '</caption>';
    ...
    $strItensSelTipoBancoDadosOrigem = InfraComparacaoBancoINT::montarSelectTipoBancoDados(
        'null',
        '',
        $objInfraComparacaoBancoDTO->getStrBancoDadosOrigem()
    );
    $strItensSelTipoBancoDadosDestino = InfraComparacaoBancoINT::montarSelectTipoBancoDados(
        'null',
        '',
        $objInfraComparacaoBancoDTO->getStrBancoDadosDestino()
    );
...
?>
    <form id="frmComparacaoBanco" method="post" onsubmit="return OnSubmitForm();"
        ...
        <fieldset id="fldDadosOrigem" class="infraFieldset">
            ...
            <label id="lblServidorOrigem" for="txtServidorOrigem" accesskey=""
                   class="infraLabelObrigatorio">Servidor:</label>
            <input type="text" id="txtServidorOrigem" name="txtServidorOrigem" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrServidorOrigem()
            ...
            <input type="text" id="txtPortaOrigem" name="txtPortaOrigem" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrPortaOrigem()
                   ); ?>" tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

            <label id="lblBancoOrigem" for="txtBancoOrigem" accesskey="" class="infraLabelObrigatorio">Banco:</label>
            <input type="text" id="txtBancoOrigem" name="txtBancoOrigem" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrBancoOrigem()
                   ); ?>" tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

            <label id="lblUsuarioOrigem" for="txtUsuarioOrigem" accesskey=""
                   class="infraLabelObrigatorio">Usuário:</label>
            <input type="text" id="txtUsuarioOrigem" name="txtUsuarioOrigem" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrUsuarioOrigem()
                   ); ?>" tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

            <label id="lblSenhaOrigem" for="txtSenhaOrigem" accesskey="" class="infraLabelObrigatorio">Senha:</label>
            <input type="password" id="txtSenhaOrigem" name="txtSenhaOrigem" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrSenhaOrigem()
                   ); ?>" tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

            <label id="lblTipoBancoDadosOrigem" for="selTipoBancoDadosOrigem"
                   class="infraLabelObrigatorio">Tipo:</label>
            <select id="selTipoBancoDadosOrigem" name="selTipoBancoDadosOrigem"
                    onchange="habilitarOuDesabilitarOpcaoCompararColunasTabela()" class="infraSelect"
                    tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>">
        ...
        <fieldset id="fldDadosDestino" class="infraFieldset">
            ...
            <label id="lblServidorDestino" for="txtServidorDestino" accesskey=""
                   class="infraLabelObrigatorio">Servidor:</label>
            <input type="text" id="txtServidorDestino" name="txtServidorDestino" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrServidorDestino()
                   ); ?>" tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

            <label id="lblPortaDestino" for="txtPortaDestino" accesskey="" class="infraLabelObrigatorio">Porta:</label>
            <input type="text" id="txtPortaDestino" name="txtPortaDestino" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrPortaDestino()
                   ); ?>" tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

            <label id="lblBancoDestino" for="txtBancoDestino" accesskey="" class="infraLabelObrigatorio">Banco:</label>
            <input type="text" id="txtBancoDestino" name="txtBancoDestino" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrBancoDestino()
                   ); ?>" tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

            <label id="lblUsuarioDestino" for="txtUsuarioDestino" accesskey=""
                   class="infraLabelObrigatorio">Usuário:</label>
            <input type="text" id="txtUsuarioDestino" name="txtUsuarioDestino" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrUsuarioDestino()
                   ); ?>" tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

            <label id="lblSenhaDestino" for="txtSenhaDestino" accesskey="" class="infraLabelObrigatorio">Senha:</label>
            <input type="password" id="txtSenhaDestino" name="txtSenhaDestino" class="infraText"
                   value="<?= PaginaInfra::getInstance()->tratarHTML(
                       $objInfraComparacaoBancoDTO->getStrSenhaDestino()
                   ); ?>" tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

            <label id="lblTipoBancoDadosDestino" for="selTipoBancoDadosDestino" accesskey=""
                   class="infraLabelObrigatorio">Tipo:</label>
            <select id="selTipoBancoDadosDestino" name="selTipoBancoDadosDestino"
                    onchange="habilitarOuDesabilitarOpcaoCompararColunasTabela()" class="infraSelect"
                    tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>">
        ...
        <fieldset id="fldTabelasIgnorar" class="infraFieldset">
            ...
            <label id="lblTabelasIgnorar" for="txaTabelasIgnorar" class="infraLabelOpcional">Informar uma tabela por
                linha:</label>
            <textarea id="txaTabelasIgnorar" name="txaTabelasIgnorar" class="infraTextarea"
                      rows="15"><?= PaginaInfra::getInstance()->tratarHTML($_POST['txaTabelasIgnorar']) ?></textarea>
        </fieldset>

        <div id="divSinComparaTipoColunasTabela" class="infraDivCheckbox">
            <input type="checkbox" id="chkSinComparaTipoColunasTabela" name="chkSinComparaTipoColunasTabela"
                   class="infraCheckbox" <?= PaginaInfra::getInstance()->setCheckbox(
                $objInfraComparacaoBancoDTO->getStrSinComparaTipoColunasTabela()
            ) ?> tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>
            <label id="lblSinComparaTipoColunasTabela" for="chkSinComparaTipoColunasTabela" accesskey=""
                   class="infraLabelCheckbox">Comparar tipos das colunas</label>
        </div>

        <div id="divSinComparaQtdeRegistrosTabela" class="infraDivCheckbox">
            <input type="checkbox" id="chkSinComparaQtdeRegistrosTabela" name="chkSinComparaQtdeRegistrosTabela"
                   class="infraCheckbox" <?= PaginaInfra::getInstance()->setCheckbox(
                $objInfraComparacaoBancoDTO->getStrSinComparaQtdeRegistrosTabela()
            ) ?> tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>
            <label id="lblSinComparaQtdeRegistrosTabela" for="chkSinComparaQtdeRegistrosTabela" accesskey=""
                   class="infraLabelCheckbox">Comparar quantidade de registros das tabelas</label>
        </div>

        <div id="divSinComparaMaxIdTabela" class="infraDivCheckbox">
            <input type="checkbox" id="chkSinComparaMaxIdTabela" name="chkSinComparaMaxIdTabela"
                   class="infraCheckbox" <?= PaginaInfra::getInstance()->setCheckbox(
                $objInfraComparacaoBancoDTO->getStrSinComparaMaxIdTabela()
            ) ?> tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>
            <label id="lblSinComparaMaxIdTabela" for="chkSinComparaMaxIdTabela" accesskey="" class="infraLabelCheckbox">Comparar
                máximo valor dos IDs</label>
        </div>

        <div id="divSinExibirSomenteDiferencas" class="infraDivCheckbox">
            <input type="checkbox" id="chkSinExibirSomenteDiferencas" name="chkSinExibirSomenteDiferencas"
                   class="infraCheckbox" <?= PaginaInfra::getInstance()->setCheckbox(
                $objInfraComparacaoBancoDTO->getStrSinExibirSomenteDiferencas()
            ) ?> tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>
            <label id="lblSinExibirSomenteDiferencas" for="chkSinExibirSomenteDiferencas" accesskey=""
                   class="infraLabelCheckbox">Exibir somente diferenças</label>
        </div>

        <input type="hidden" id="hdnFlag" name="hdnFlag" value="1"/>
        <?
        PaginaInfra::getInstance()->fecharAreaDados();
        PaginaInfra::getInstance()->montarAreaTabela(
            $strResultado,
            $numRegistrosTabela + $numRegistrosConstraint + $numRegistrosIndice + $numRegistrosSequencia
        );
