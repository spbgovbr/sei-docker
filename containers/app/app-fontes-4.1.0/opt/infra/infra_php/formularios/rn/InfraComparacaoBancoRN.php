...
class InfraComparacaoBancoRN
{
        ...
        try {
            ...
            switch ($parObjInfraComparacaoBancoDTO->getStrBancoDadosOrigem()) {
                case self::$TBD_SQLSERVER:
                    $this->objInfraMetaBancoOrigem = new InfraMetaBD(
                        InfraBancoSqlServer::newInstance(
                            $parObjInfraComparacaoBancoDTO->getStrServidorOrigem(),
                ...
                case self::$TBD_POSTGRESQL:
                    $this->objInfraMetaBancoOrigem = new InfraMetaBD(
                        InfraBancoPostgreSql::newInstance(
                            $parObjInfraComparacaoBancoDTO->getStrServidorOrigem(),
                            $parObjInfraComparacaoBancoDTO->getStrPortaOrigem(),
                            $parObjInfraComparacaoBancoDTO->getStrBancoOrigem(),
                            $parObjInfraComparacaoBancoDTO->getStrUsuarioOrigem(),
                ...
                default:
                    throw new InfraException(
                        'Tipo do banco de dados origem [' . $parObjInfraComparacaoBancoDTO->getStrBancoDadosOrigem(
                        ) . '] inválido.'
                    );
            }
            ...
            switch ($parObjInfraComparacaoBancoDTO->getStrBancoDadosDestino()) {
                case self::$TBD_SQLSERVER:
                    $this->objInfraMetaBancoDestino = new InfraMetaBD(
                        InfraBancoSqlServer::newInstance(
                            $parObjInfraComparacaoBancoDTO->getStrServidorDestino(),
                ...
                default:
                    throw new InfraException(
                        'Tipo do banco de dados destino [' . $parObjInfraComparacaoBancoDTO->getStrBancoDadosDestino(
                        ) . '] inválido.'
                    );
            }
        } catch (Exception $e) {
    ...
    public function compararTabelas(InfraComparacaoBancoDTO $parObjInfraComparacaoBancoDTO)
    {
        try {
            ...
            $rsNomeTabelaOrigem = $this->objInfraMetaBancoOrigem->obterTabelas(
                null,
                $parObjInfraComparacaoBancoDTO->getArrTabelasIgnorar()
            );
            ...
            for ($i = 0; $i < count($rsNomeTabelaOrigem); $i++) {
                ...
                $rsColunasTabelaOrigem = $this->objInfraMetaBancoOrigem->obterColunasTabela(
                    $rsNomeTabelaOrigem[$i]['table_name']
                );

                $rsNomeTabelaDestino = $this->objInfraMetaBancoDestino->obterTabelas(
                    $rsNomeTabelaOrigem[$i]['table_name']
                );
                ...
                if (count($rsNomeTabelaDestino) == 0) {
                    ...
                    $rsColunasTabelaDestino = $this->objInfraMetaBancoDestino->obterColunasTabela(
                        $rsNomeTabelaOrigem[$i]['table_name']
                    );
                        ...
                        for ($k = 0; $k < count($rsColunasTabelaDestino); $k++) {
                            ...
                            if ($rsColunasTabelaOrigem[$j]['column_name'] == $rsColunasTabelaDestino[$k]['column_name']) {
                                if ($parObjInfraComparacaoBancoDTO->getStrSinComparaTipoColunasTabela() == 'S') {
                                    ...
                                    if (!$this->compararTipos(
                                            $this->objInfraMetaBancoOrigem->getObjInfraIBanco(),
                                            ...
                                            $rsColunasTabelaDestino[$k]['numeric_scale']
                                        ) || $rsColunasTabelaOrigem[$j]['is_nullable'] <> $rsColunasTabelaDestino[$k]['is_nullable']) {
                    ...
                    if ($parObjInfraComparacaoBancoDTO->getStrSinComparaQtdeRegistrosTabela() == 'S') {
                        $rsQtdeRegistrosTabelaOrigem = $this->objInfraMetaBancoOrigem->obterRegistrosTabela(
                            $rsNomeTabelaOrigem[$i]['table_name']
                        );
                        $objInfraComparacaoBancoDTO->setNumQtdeRegistrosTabelaOrigem(
                            $rsQtdeRegistrosTabelaOrigem[0]['total']
                        );
                        //$parObjInfraComparacaoBancoDTO->setStrNomeTabelaOrigem($rsNomeTabelaOrigem[$i][name]);
                        $rsQtdeRegistrosTabelaDestino = $this->objInfraMetaBancoDestino->obterRegistrosTabela(
                            $rsNomeTabelaOrigem[$i]['table_name']
                        );

                        $objInfraComparacaoBancoDTO->setNumQtdeRegistrosTabelaDestino(
                            $rsQtdeRegistrosTabelaDestino[0]['total']
                        );
    ...
    public function compararConstraints(InfraComparacaoBancoDTO $parObjInfraComparacaoBancoDTO)
    {
        try {
            ...
            $rsNomeConstraintOrigem = $this->objInfraMetaBancoOrigem->obterConstraints(
                null,
                $parObjInfraComparacaoBancoDTO->getArrTabelasIgnorar()
            );
            ...
            for ($i = 0; $i < count($rsNomeConstraintOrigem); $i++) {
                ...
                if ($rsNomeConstraintOrigem[$i]['constraint_type'] == 'primary key') {
                    if ($this->objInfraMetaBancoOrigem->getObjInfraIBanco(
                        ) instanceof InfraMySql && !$this->objInfraMetaBancoDestino->getObjInfraIBanco(
                            ) instanceof InfraMySql) {
                    ...
                    } elseif (!$this->objInfraMetaBancoOrigem->getObjInfraIBanco(
                            ) instanceof InfraMySql && $this->objInfraMetaBancoDestino->getObjInfraIBanco(
                        ) instanceof InfraMySql) {
    ...
    public function compararSequencias(InfraComparacaoBancoDTO $parObjInfraComparacaoBancoDTO)
    {
        try {
            ...
            $rsNomeTabelaSequenciaOrigem = $this->objInfraMetaBancoOrigem->obterSequencias(
                $parObjInfraComparacaoBancoDTO->getArrTabelasIgnorar()
            );
            $rsNomeTabelaDestino = $this->objInfraMetaBancoDestino->obterSequencias(
                $parObjInfraComparacaoBancoDTO->getArrTabelasIgnorar()
            );
            ...
            for ($i = 0; $i < count($rsNomeTabelaSequenciaOrigem); $i++) {
                for ($j = 0; $j < count($rsNomeTabelaDestino); $j++) {
                    if ($rsNomeTabelaSequenciaOrigem[$i]['table_name'] == $rsNomeTabelaDestino[$j]['table_name']) {
                        $objInfraComparacaoBancoDTO = new InfraComparacaoBancoDTO();
                        $objInfraComparacaoBancoDTO->setStrNomeTabelaOrigem(
                            $rsNomeTabelaSequenciaOrigem[$i]['table_name']
                        );
                        ...
                        $rsMaxIdTabelaOrigem = $this->objInfraMetaBancoOrigem->obterMaxIdTabela(
                            str_replace('seq_', '', $rsNomeTabelaSequenciaOrigem[$i]['table_name'])
                        );
                        if (count($rsMaxIdTabelaOrigem)) {
                            $objInfraComparacaoBancoDTO->setNumMaxIdTabelaOrigem($rsMaxIdTabelaOrigem[0]['maximo']);
                        }

                        $rsMaxIdTabelaDestino = $this->objInfraMetaBancoDestino->obterMaxIdTabela(
                            str_replace('seq_', '', $rsNomeTabelaSequenciaOrigem[$i]['table_name'])
                        );
                        ...
                        $numMaxIdTabelaSequenciaOrigem = $this->objInfraMetaBancoOrigem->obterMaxIdTabelaSequencia(
                            $rsNomeTabelaSequenciaOrigem[$i]['table_name']
                        );
                        if ($numMaxIdTabelaSequenciaOrigem != null) {
                            $objInfraComparacaoBancoDTO->setNumMaxIdTabelaSequenciaOrigem(
                                $numMaxIdTabelaSequenciaOrigem
                            );
                        }

                        $numMaxIdTabelaSequenciaDestino = $this->objInfraMetaBancoDestino->obterMaxIdTabelaSequencia(
                            $rsNomeTabelaSequenciaOrigem[$i]['table_name']
                        );
                        if ($numMaxIdTabelaSequenciaDestino != null) {
                            $objInfraComparacaoBancoDTO->setNumMaxIdTabelaSequenciaDestino(
                                $numMaxIdTabelaSequenciaDestino
                            );
                        }
    ...
    public function compararIndices(InfraComparacaoBancoDTO $parObjInfraComparacaoBancoDTO)
    {
        try {
            ...
            $rsIndicesOrigemUnique = $this->objInfraMetaBancoOrigem->obterIndices(
                $parObjInfraComparacaoBancoDTO->getArrTabelasIgnorar(),
                null,
                true
            );
            $rsIndicesDestinoUnique = $this->objInfraMetaBancoDestino->obterIndices(
                $parObjInfraComparacaoBancoDTO->getArrTabelasIgnorar(),
                null,
                true
            );

            $rsIndicesOrigem = $this->objInfraMetaBancoOrigem->obterIndices(
                $parObjInfraComparacaoBancoDTO->getArrTabelasIgnorar()
            );
            $rsIndicesDestino = $this->objInfraMetaBancoDestino->obterIndices(
                $parObjInfraComparacaoBancoDTO->getArrTabelasIgnorar()
            );
    ...
    private function compararTipos(
        $objBancoOrigem,
        $strTipoOrigem,
        $numTamanhoOrigem,
        $numPrecisaoOrigem,
