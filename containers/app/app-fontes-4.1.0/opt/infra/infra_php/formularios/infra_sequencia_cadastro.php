            ...
            if (isset($_POST['sbmCadastrarInfraSequencia'])) {
                try {
                    ...
                    PaginaInfra::getInstance()->setStrMensagem(
                        'Sequência "' . $objInfraSequenciaDTO->getStrNome() . '" cadastrada com sucesso.'
                    );
                    header(
                        'Location: ' . SessaoInfra::getInstance()->assinarLink(
                            'controlador.php?acao=' . PaginaInfra::getInstance()->getAcaoRetorno(
                            ) . '&acao_origem=' . $_GET['acao'] . '&nome=' . $objInfraSequenciaDTO->getStrNome(
                            ) . PaginaInfra::getInstance()->montarAncora($objInfraSequenciaDTO->getStrNome())
                        )
                    );
                    die;
                } catch (Exception $e) {
        ...
        case 'infra_sequencia_alterar':
            ...
            $arrComandos[] = '<button type="button" accesskey="C" name="btnCancelar" id="btnCancelar" value="Cancelar" onclick="location.href=\'' . SessaoInfra::getInstance(
                    ...
                    'controlador.php?acao=' . PaginaInfra::getInstance()->getAcaoRetorno(
                        ...
                        $objInfraSequenciaDTO->getStrNome()
                    )
                ) . '\';" class="infraButton"><span class="infraTeclaAtalho">C</span>ancelar</button>';
            ...
            if (isset($_POST['sbmAlterarInfraSequencia'])) {
                try {
                    ...
                    PaginaInfra::getInstance()->setStrMensagem(
                        'Sequência "' . $objInfraSequenciaDTO->getStrNome() . '" alterada com sucesso.'
                    );
                    header(
                        'Location: ' . SessaoInfra::getInstance()->assinarLink(
                            'controlador.php?acao=' . PaginaInfra::getInstance()->getAcaoRetorno(
...
?>
    ...
    } elseif ('<?= $_GET['acao'] ?>'=='infra_sequencia_alterar'){
    document.getElementById('txtQtdIncremento').focus();
    } elseif ('<?= $_GET['acao'] ?>'=='infra_sequencia_consultar'){
...
?>
    <form id="frmInfraSequenciaCadastro" method="post" onsubmit="return OnSubmitForm();"
        ...
        <input type="text" id="txtNome" name="txtNome" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($objInfraSequenciaDTO->getStrNome()); ?>"
               onkeypress="return infraMascaraTexto(this,event);"
               tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>" <?= $strDesabilitar ?> />

        <label id="lblQtdIncremento" for="txtQtdIncremento" accesskey="I" class="infraLabelObrigatorio"><span
                class="infraTeclaAtalho">I</span>ncremento:</label>
        <input type="text" id="txtQtdIncremento" name="txtQtdIncremento" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($objInfraSequenciaDTO->getDblQtdIncremento()); ?>"
               onkeypress="return infraMascaraNumero(this,event);"
               tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

        <label id="lblNumAtual" for="txtNumAtual" accesskey="A" class="infraLabelObrigatorio">V<span
                class="infraTeclaAtalho">a</span>lor Atual:</label>
        <input type="text" id="txtNumAtual" name="txtNumAtual" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($objInfraSequenciaDTO->getDblNumAtual()); ?>"
               onkeypress="return infraMascaraNumero(this,event);"
               tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

        <label id="lblNumMaximo" for="txtNumMaximo" accesskey="M" class="infraLabelObrigatorio">Valor <span
                class="infraTeclaAtalho">M</span>áximo:</label>
        <input type="text" id="txtNumMaximo" name="txtNumMaximo" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($objInfraSequenciaDTO->getDblNumMaximo()); ?>"
               onkeypress="return infraMascaraNumero(this,event);"
               tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>
