...
try {
    ...
    switch ($_GET['acao']) {
        case 'infra_acesso_usuario_listar':
            $strTitulo = 'Últimos Acessos de ' . SessaoInfra::getInstance()->getStrSiglaUsuario(
                ) . ' no Sistema ' . SessaoInfra::getInstance()->getStrSiglaSistema();
            break;
    ...
    $arrAcessos = $objInfraSip->listarAcessos(
        SessaoInfra::getInstance()->getNumIdSistema(),
        SessaoInfra::getInstance()->getNumIdUsuario()
    );
    ...
    if ($numRegistros > 0) {
        ...
        foreach ($arrAcessos as $acesso) {
            $acesso = (object)($acesso);

            $strCssTr = ($strCssTr == '<tr class="infraTrClara">') ? '<tr class="infraTrEscura">' : '<tr class="infraTrClara">';
            $strResultado .= $strCssTr;
            $strResultado .= '<td align="center">' . PaginaInfra::getInstance()->tratarHTML(
                    InfraData::formatarExtenso4($acesso->DataHora)
                ) . '</td>';
            $strResultado .= '<td align="center">' . PaginaInfra::getInstance()->tratarHTML(
                    $acesso->Navegador
                ) . '</td>';
            $strResultado .= '<td align="center">' . PaginaInfra::getInstance()->tratarHTML($acesso->IP) . '</td>';
...
?>
    <form id="frmInfraAcessoUsuarioLista" method="post" onsubmit="return validarForm();"
