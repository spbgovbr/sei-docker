    ...
    switch ($_GET['acao']) {
                ...
                try {
                    ...
                    if (isset($_SESSION['INFRA_ATRIBUTOS'][$strSiglaOrgaoSistema][$strSiglaSistema][$nome])) {
                        ...
                        PaginaInfra::getInstance()->setStrMensagem(
                            'Atributo de Sessão "' . $nome . '" cadastrado com sucesso.'
                        );
                        header(
                            'Location: ' . SessaoInfra::getInstance()->assinarLink(
                                'controlador.php?acao=' . PaginaInfra::getInstance()->getAcaoRetorno(
                                ) . '&acao_origem=' . $_GET['acao'] . '&nome=' . $nome . PaginaInfra::getInstance(
                                )->montarAncora($nome)
                            )
                        );
                        die;
        ...
        case 'infra_atributo_sessao_alterar':
            ...
            $arrComandos[] = '<button type="button" accesskey="C" name="btnCancelar" id="btnCancelar" value="Cancelar" onclick="location.href=\'' . SessaoInfra::getInstance(
                    ...
                    'controlador.php?acao=' . PaginaInfra::getInstance()->getAcaoRetorno(
                    ) . '&acao_origem=' . $_GET['acao'] . PaginaInfra::getInstance()->montarAncora($nome)
                ) . '\';" class="infraButton"><span class="infraTeclaAtalho">C</span>ancelar</button>';
            ...
            if (isset($_POST['sbmAlterarInfraAtributoSessao'])) {
                try {
                    ...
                    PaginaInfra::getInstance()->setStrMensagem(
                        'Atributo de Sessão "' . $nome . '" alterado com sucesso.'
                    );
                    header(
                        'Location: ' . SessaoInfra::getInstance()->assinarLink(
                            'controlador.php?acao=' . PaginaInfra::getInstance()->getAcaoRetorno(
...
?>
    ...
    } elseif ('<?= $_GET['acao'] ?>'=='infra_atributo_sessao_alterar'){
...
?>
    <form id="frmInfraAtributoSessaoCadastro" method="post" onsubmit="return OnSubmitForm();"
        ...
        <input type="text" id="txtNome" name="txtNome" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($nome); ?>"
        ...
        <input type="text" id="txtValor" name="txtValor" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($valor); ?>"
               onkeypress="return infraMascaraTexto(this,event);"
