...
try {
        ...
        case 'infra_log_excluir':
            ...
            header(
                'Location: ' . SessaoInfra::getInstance()->assinarLink(
                    'controlador.php?acao=' . $_GET['acao_origem'] . '&acao_origem=' . $_GET['acao'] . $strParametros
                )
            );
            die;
    ...
    if ($numRegistros > 0) {
        ...
        if ($bolAcaoExcluir) {
            ...
            $strLinkExcluir = SessaoInfra::getInstance()->assinarLink(
                'controlador.php?acao=infra_log_excluir&acao_origem=' . $_GET['acao'] . $strParametros
            );
        }
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            if ($bolCheck) {
                $strResultado .= '<td valign="top" align="center">' . PaginaInfra::getInstance()->getTrCheck(
                        $i,
                        $arrObjInfraLogDTO[$i]->getNumIdInfraLog(),
                        $arrObjInfraLogDTO[$i]->getDthLog()
                    ) . '</td>';
            }

            $strResultado .= '<td valign="top" align="left"><b>Data:</b><br />' . PaginaInfra::getInstance(
                )->tratarHTML($arrObjInfraLogDTO[$i]->getDthLog()) . '<br/><br />';

            if ($bolTipos) {
                $strResultado .= '<b>Tipo:</b><br />' . ($arrObjInfraLogDTO[$i]->getStrStaTipo(
                    ) != null ? $arrTiposInfraLog[$arrObjInfraLogDTO[$i]->getStrStaTipo()] : '&nbsp;') . '<br/><br />';
            }

            if (!InfraString::isBolVazia($arrObjInfraLogDTO[$i]->getStrIp())) {
                $strResultado .= '<b>IP:</b><br />' . PaginaInfra::getInstance()->tratarHTML(
                        $arrObjInfraLogDTO[$i]->getStrIp()
                    ) . '</td>';
            }
            ...
            if ($bolAcaoConsultar) {
                $strResultado .= '<a href="' . SessaoInfra::getInstance()->assinarLink(
                        'controlador.php?acao=infra_log_consultar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_infra_log=' . $arrObjInfraLogDTO[$i]->getNumIdInfraLog(
                        )
                    ) . '" tabindex="' . PaginaInfra::getInstance()->getProxTabTabela(
                    ) . '"><img src="' . PaginaInfra::getInstance()->getIconeConsultar(
                    ) . '" title="Consultar Log" alt="Consultar Log" class="infraImg" /></a> ';
            }

            if ($bolAcaoAlterar) {
                $strResultado .= '<a href="' . SessaoInfra::getInstance()->assinarLink(
                        'controlador.php?acao=infra_log_alterar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_infra_log=' . $arrObjInfraLogDTO[$i]->getNumIdInfraLog(
                        )
                    ) . '" tabindex="' . PaginaInfra::getInstance()->getProxTabTabela(
                    ) . '"><img src="' . PaginaInfra::getInstance()->getIconeAlterar(
                    ) . '" title="Alterar Log" alt="Alterar Log" class="infraImg" /></a> ';
            }
            ...
            if ($bolAcaoExcluir) {
                $strResultado .= '<a onclick="acaoExcluir(\'' . $strId . '\',\'' . $strDescricao . '\');" tabindex="' . PaginaInfra::getInstance(
                    )->getProxTabTabela() . '"><img src="' . PaginaInfra::getInstance()->getIconeExcluir(
                    ) . '" title="Excluir Log" alt="Excluir Log" class="infraImg" /></a> ';
            }
    ...
    if ($bolTipos) {
        ...
        foreach ($arrTiposInfraLog as $strStaTipo => $strDescricaoTipo) {
            $strOptionsTipo .= '<option value="' . $strStaTipo . '"' . (in_array(
                    $strStaTipo,
                    $arrTipos
                ) ? ' selected="selected"' : '') . '>' . InfraPagina::tratarHTML(
                    $strDescricaoTipo
                ) . '</option>' . "\n";
        }
    }
...
PaginaInfra::getInstance()->montarTitle(
    PaginaInfra::getInstance()->getStrNomeSistema() . ' - ' . $strTitulo . (is_numeric(
        $objInfraLogDTO->getNumTotalRegistros()
    ) ? ' (' . $objInfraLogDTO->getNumTotalRegistros() . ')' : '')
);
    ...
    <style><?}?>
        ...
        function configurarAutoAtualizar() {
            if (document.getElementById('chkSinAutoAtualizar').checked) {
                window.location = '<?=SessaoInfra::getInstance()->assinarLink(
                    'controlador.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'] . '&auto_atualizar=S'
                )?>';
            } else {
                window.location = '<?=SessaoInfra::getInstance()->assinarLink(
                    'controlador.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'] . '&auto_atualizar=N'
                )?>';
            }
        }
...
?>
    <form id="frmInfraLogLista" method="post" onsubmit="return validarForm();"
        ...
        <? if ($bolTipos) { ?>
            ...
            <select style="display: none" multiple id="selTipos" name="selTipos[]" class="infraSelect multipleSelect"
        ...
        <input type="text" id="txtDthInicialLog" name="txtDthInicialLog"
        ...
        <img src="<?= PaginaInfra::getInstance()->getIconeCalendario() ?>" id="imgCalDthInicial"
             ...
             onclick="infraCalendario('txtDthInicialLog',this,true,'<?= InfraData::getStrDataAtual() . ' 00:00' ?>');"
             tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

        <label id="lblDthFinal" for="txtDthFinalLog" accesskey="" class="infraLabelOpcional">a</label>
        <input type="text" id="txtDthFinalLog" name="txtDthFinalLog"
               onkeypress="return infraMascara(this, event,'##/##/#### ##:##')" class="infraText"
        ...
        <img src="<?= PaginaInfra::getInstance()->getIconeCalendario() ?>" id="imgCalDthFinal"
             ...
             onclick="infraCalendario('txtDthFinalLog',this,true,'<?= InfraData::getStrDataAtual() . ' 23:59' ?>');"
             tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

        <label id="lblTextoLog" for="txtTextoLog" class="infraLabelOpcional">Texto:</label>
        <input type="text" id="txtTextoLog" name="txtTextoLog" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($strTextoLog) ?>"
               tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

        <label id="lblIp" for="txtIpLog" accesskey="" class="infraLabelOpcional">IP:</label>
        <input type="text" id="txtIpLog" name="txtIpLog" class="infraText"
        ...
        <div id="divSinAutoAtualizar" class="infraDivCheckbox">
            <input type="checkbox" id="chkSinAutoAtualizar" name="chkSinAutoAtualizar" class="infraCheckbox"
                   onclick="configurarAutoAtualizar();"
                   tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>
            <label id="lblSinAutoAtualizar" for="chkSinAutoAtualizar" accesskey="" class="infraLabelCheckbox">Atualizar
                automaticamente a cada minuto</label>
        </div>
