...
try {
    ...
    PaginaInfra::getInstance()->salvarCamposPost(
        array('selInfraAuditoriaUsuario', 'selInfraAuditoriaUsuarioEmulador', 'selInfraAuditoriaUnidade')
    );
        ...
        case 'infra_auditoria_cadastrar':
            ...
            if (isset($_POST['sbmCadastrarInfraAuditoria'])) {
                try {
                    ...
                    PaginaInfra::getInstance()->adicionarMensagem(
                        'Dado de Auditoria "' . $objInfraAuditoriaDTO->getNumIdInfraAuditoria(
                        ) . '" cadastrado com sucesso.'
                    );
                    header(
                        'Location: ' . SessaoInfra::getInstance()->assinarLink(
                            'controlador.php?acao=' . PaginaInfra::getInstance()->getAcaoRetorno(
                            ) . '&acao_origem=' . $_GET['acao'] . '&id_infra_auditoria=' . $objInfraAuditoriaDTO->getNumIdInfraAuditoria(
                            ) . PaginaInfra::getInstance()->montarAncora(
                                $objInfraAuditoriaDTO->getNumIdInfraAuditoria()
                            )
                        )
                    );
                    die;
    ...
    $strItensSelInfraAuditoriaUsuario = InfraAuditoriaUsuarioINT::montarSelectSigla(
        'null',
        '&nbsp;',
        $objInfraAuditoriaDTO->getNumIdUsuario()
    );
    $strItensSelInfraAuditoriaUsuarioEmulador = InfraAuditoriaUsuarioINT::montarSelectSigla(
        'null',
        '&nbsp;',
        $objInfraAuditoriaDTO->getNumIdUsuarioEmulador()
    );
    $strItensSelInfraAuditoriaUnidade = InfraAuditoriaUnidadeINT::montarSelectSigla(
        'null',
        '&nbsp;',
        $objInfraAuditoriaDTO->getNumIdUnidade()
    );
} catch (Exception $e) {
    PaginaInfra::getInstance()->processarExcecao($e);
}
...
?>
    ...
    } elseif ('<?= $_GET['acao'] ?>'=='infra_auditoria_consultar'){
...
?>
    <form id="frmInfraAuditoriaCadastro" method="post" onsubmit="return OnSubmitForm();"
        ...
        <input type="text" id="txtRecurso" name="txtRecurso" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($objInfraAuditoriaDTO->getStrRecurso()); ?>"
        ...
        <input type="text" id="txtDados" name="txtDados" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($objInfraAuditoriaDTO->getStrDados()); ?>"
        ...
        <input type="text" id="txtAcesso" name="txtAcesso" onkeypress="return infraMascaraDataHora(this, event)"
               class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($objInfraAuditoriaDTO->getDthAcesso()); ?>"
               tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>
        <img id="imgCalAcesso" title="Selecionar Data/Hora" alt="Selecionar Data/Hora"
             src="<?= PaginaInfra::getInstance()->getIconeCalendario() ?>" class="infraImg"
             onclick="infraCalendario('txtAcesso',this);"/>
        ...
        <input type="text" id="txtIp" name="txtIp" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($objInfraAuditoriaDTO->getStrIp()); ?>"
               onkeypress="return infraMascaraTexto(this,event,15);" maxlength="15"
               tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>

        <label id="lblInfraAuditoriaUsuario" for="selInfraAuditoriaUsuario" accesskey="" class="infraLabelObrigatorio">Usuário:</label>
        <select id="selInfraAuditoriaUsuario" name="selInfraAuditoriaUsuario" class="infraSelect"
                tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>">
            <?= $strItensSelInfraAuditoriaUsuario ?>
        </select>
        <label id="lblInfraAuditoriaUsuarioEmulador" for="selInfraAuditoriaUsuarioEmulador" accesskey=""
               class="infraLabelOpcional">Usuário Emulador:</label>
        <select id="selInfraAuditoriaUsuarioEmulador" name="selInfraAuditoriaUsuarioEmulador" class="infraSelect"
                tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>">
        ...
        <select id="selInfraAuditoriaUnidade" name="selInfraAuditoriaUnidade" class="infraSelect"
                tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>">
            <?= $strItensSelInfraAuditoriaUnidade ?>
        </select>
        <input type="hidden" id="hdnIdInfraAuditoria" name="hdnIdInfraAuditoria"
               value="<?= $objInfraAuditoriaDTO->getNumIdInfraAuditoria(); ?>"/>
        <?
