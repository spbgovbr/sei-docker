...
try {
    ...
    if ($bolAcaoCadastrar) {
        $arrComandos[] = '<button type="button" accesskey="N" id="btnNovo" value="Novo" onclick="location.href=\'' . SessaoInfra::getInstance(
                ...
                'controlador.php?acao=infra_navegador_cadastrar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao']
    ...
    $objInfraNavegadorDTO->setStrSinIgnorarVersao(
        PaginaInfra::getInstance()->getCheckbox(PaginaInfra::getInstance()->recuperarCampo('chkSinIgnorarVersao', 'N'))
    );
    ...
    PaginaInfra::getInstance()->prepararOrdenacao(
        $objInfraNavegadorDTO,
        'TotalAcessos',
        InfraDTO::$TIPO_ORDENACAO_DESC
    );
    ...
    if ($numRegistros > 0) {
        ...
        if ($_GET['acao'] == 'infra_navegador_selecionar') {
            ...
            $bolCheck = true;
            /*     }elseif ($_GET['acao']=='infra_navegador_reativar'){
        ...
        */
        ...
        $strResultado .= '<th class="infraTh" width="20%">' . PaginaInfra::getInstance()->getThOrdenacao(
                $objInfraNavegadorDTO,
                'Acessos',
                'TotalAcessos',
                $arrObjInfraNavegadorDTO
            ) . '</th>' . "\n";
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            $strResultado .= '<td align="center">' . PaginaInfra::getInstance()->tratarHTML(
                    $arrObjInfraNavegadorDTO[$i]->getStrTotalFormatado()
                ) . '</td>';
            $strResultado .= '<td>' . PaginaInfra::getInstance()->tratarHTML(
                    $arrObjInfraNavegadorDTO[$i]->getStrIdentificacao()
                ) . '</td>';

            if ($objInfraNavegadorDTO->getStrSinIgnorarVersao() == 'N') {
                $strResultado .= '<td align="center">' . PaginaInfra::getInstance()->tratarHTML(
                        $arrObjInfraNavegadorDTO[$i]->getStrVersao()
                    ) . '</td>';
            }
...
?>
    <form id="frmInfraNavegadorLista" method="post" onsubmit="return validarForm();"
        ...
        <input type="text" id="txtDthInicialNavegadores" name="txtDthInicialNavegadores"
        ...
        <img src="<?= PaginaInfra::getInstance()->getIconeCalendario() ?>" id="imgCalDthInicialNavegadores"
             title="Selecionar Data/Hora Inicial" alt="Selecionar Data/Hora Inicial" class="infraImg"
             onclick="infraCalendario('txtDthInicialNavegadores', this, true,'<?= InfraData::getStrDataAtual(
             ) . ' 00:00' ?>');"/>

        <label id="lblDthFinalNavegadores" for="txtDthFinalNavegadores" accesskey=""
               class="infraLabelOpcional">a</label>
        <input type="text" id="txtDthFinalNavegadores" name="txtDthFinalNavegadores"
               onkeypress="return infraMascaraDataHora(this, event)" class="infraText"
               value="<?= PaginaInfra::getInstance()->tratarHTML($dthFinal) ?>"
               tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>
        <img src="<?= PaginaInfra::getInstance()->getIconeCalendario() ?>" id="imgCalDthFinalNavegadores"
             title="Selecionar Data/Hora Final" alt="Selecionar Data/Hora Final" class="infraImg"
             onclick="infraCalendario('txtDthFinalNavegadores', this, true, '<?= InfraData::getStrDataAtual(
             ) . ' 23:59' ?>');"/>

        <div id="divSinIgnorarVersao" class="infraDivCheckbox">
            <input type="checkbox" id="chkSinIgnorarVersao" name="chkSinIgnorarVersao"
                   class="infraCheckbox" <?= PaginaInfra::getInstance()->setCheckbox(
                $objInfraNavegadorDTO->getStrSinIgnorarVersao()
            ) ?> tabindex="<?= PaginaInfra::getInstance()->getProxTabDados() ?>"/>
            <label id="lblSinIgnorarVersao" for="chkSinIgnorarVersao" accesskey="" class="infraLabelCheckbox">Ignorar
                Versão</label>
        </div>
