...
try {
    ...
    if ($bolAcaoCadastrar) {
        $arrComandos[] = '<button type="button" accesskey="N" id="btnNovo" value="Novo" onclick="location.href=\'' . SessaoInfra::getInstance(
                ...
                'controlador.php?acao=infra_atributo_sessao_cadastrar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao']
    ...
    if ($numRegistros > 0) {
        ...
        if ($bolAcaoExcluir) {
            ...
            $strLinkExcluir = SessaoInfra::getInstance()->assinarLink(
                'controlador.php?acao=infra_atributo_sessao_excluir&acao_origem=' . $_GET['acao']
            );
        }
        ...
        foreach (array_keys($arrAtributos) as $nome) {
            ...
            if (is_object($arrAtributos[$nome])) {
                $strResultado .= nl2br(PaginaInfra::getInstance()->tratarHTML($arrAtributos[$nome]->__toString()));
            }else if (is_array($arrAtributos[$nome])) {
            ...
            if ($bolAcaoAlterar) {
                $strResultado .= '<a href="' . SessaoInfra::getInstance()->assinarLink(
                        'controlador.php?acao=infra_atributo_sessao_alterar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&nome=' . $nome
                    ) . '" tabindex="' . PaginaInfra::getInstance()->getProxTabTabela(
                    ) . '"><img src="' . PaginaInfra::getInstance()->getIconeAlterar(
                    ) . '" title="Alterar Atributo de Sessão" alt="Alterar Atributo de Sessão" class="infraImg" /></a>&nbsp;';
            }
            ...
            if ($bolAcaoExcluir) {
                $strResultado .= '<a href="' . PaginaInfra::getInstance()->montarAncora(
                    ...
                    ) . '" title="Excluir Atributo de Sessão" alt="Excluir Atributo de Sessão" class="infraImg" /></a>&nbsp;';
            }
...
?>
    <form id="frmInfraAtributoSessaoLista" method="post" action="<?= SessaoInfra::getInstance()->assinarLink(
