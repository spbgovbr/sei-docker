...
abstract class InfraBD
{
    ...
    public function cadastrar($varObjInfraDTO, $bolRetornarSql = false)
    {
        try {
                ...
                foreach ($arrKeys as $key) {
                    ...
                    if ($atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] !== null && $atributo[InfraDTO::$POS_ATRIBUTO_TAB_ORIGEM] === null) {
                        if (isset($arrPK[$key]) && $arrPK[$key] == InfraDTO::$TIPO_PK_SEQUENCIAL) {
                            ...
                            call_user_func(
                                array($objInfraDTO, 'set' . $atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] . $key),
                                $numProxSeq
                            );
                            ...
                            $strSeparador = ',';
                        } elseif (isset($arrPK[$key]) && $arrPK[$key] == InfraDTO::$TIPO_PK_NATIVA) {
                            $numProxSeq = $this->getObjInfraIBanco()->getValorSequencia(
                                $objInfraDTO->getStrNomeSequenciaNativa()
                            );
                            call_user_func(
                                array($objInfraDTO, 'set' . $atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] . $key),
                                $numProxSeq
                            ...
                            $strSeparador = ',';
                        } elseif ($atributo[InfraDTO::$POS_ATRIBUTO_FLAGS] & InfraDTO::$FLAG_SET) {
                            if ($bolPrimeiro) {
                                $strCampos .= $strSeparador . $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL];
                            }

                            $strValues .= $strSeparador . $this->adicionarBind(
                                    $atributo,
                                    $atributo[InfraDTO::$POS_ATRIBUTO_VALOR]
                                );
                            $strSeparador = ',';
                        }
                    }
            ...
            throw new InfraException(
                'Erro cadastrando registro em ' . $objInfraDTO->getStrNomeTabela() . '.',
                $e,
                self::formatarDetalhesSql($sql, $this->arrBind)
            );
        }
        return null;
    }
    ...
    public function alterar($objInfraDTO, $bolRetornarSql = false)
    {
        try {
            ...
            foreach ($arrAtributos as $atributo) {
                ...
                if ($atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] !== null && $atributo[InfraDTO::$POS_ATRIBUTO_TAB_ORIGEM] === null) {
                    ...
                    if (!isset($arrPK[$atributo[InfraDTO::$POS_ATRIBUTO_NOME]]) && ($atributo[InfraDTO::$POS_ATRIBUTO_FLAGS] & InfraDTO::$FLAG_SET)) {
                        $sql .= $strSeparador . $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] . '=' . $this->adicionarBind(
                                $atributo,
                                $atributo[InfraDTO::$POS_ATRIBUTO_VALOR]
                            );
                        $strSeparador = ',';
            ...
            if ($numReg == 0) {
                throw new InfraException(
                    'Registro não encontrado em ' . $objInfraDTO->getStrNomeTabela() . '.',
                    null,
                    self::formatarDetalhesSql($sql, $this->arrBind)
                );
            }
            ...
            throw new InfraException(
                'Erro alterando registro de ' . $objInfraDTO->getStrNomeTabela() . '.',
                $e,
                self::formatarDetalhesSql($sql, $this->arrBind)
            );
        }
    ...
    public function excluir($objInfraDTO, $bolRetornarSql = false)
    {
        try {
            ...
            throw new InfraException(
                'Erro excluindo registro em ' . $objInfraDTO->getStrNomeTabela() . '.',
                $e,
                self::formatarDetalhesSql($sql, $this->arrBind)
            );
        }
    ...
    public function consultar($objInfraDTO, $bolRetornarSql = false)
    {
        try {
            ...
            if ($numRegistros === 1) {
                ...
                throw new InfraException(
                    'Consulta retornou mais de um registro de ' . strtoupper($objInfraDTO->getStrNomeTabela()) . '.',
                    null,
                    self::formatarDetalhesSql($this->strSql, $this->arrBind)
                );
            }
            return $objDTO;
        } catch (Exception $e) {
            throw new InfraException(
                'Erro consultando registro de ' . $objInfraDTO->getStrNomeTabela() . '.',
                $e,
                self::formatarDetalhesSql($this->strSql, $this->arrBind)
            );
        }
    ...
    public function desativar($objInfraDTO, $bolRetornarSql = false)
    {
        try {
            ...
            if ($numReg == 0) {
                throw new InfraException(
                    'Registro de ' . $objInfraDTO->getStrNomeTabela() . ' não encontrado para desativação.',
                    null,
                    self::formatarDetalhesSql($sql, $this->arrBind)
                );
            }
            ...
            throw new InfraException(
                'Erro desativando registro de ' . $objInfraDTO->getStrNomeTabela() . '.',
                $e,
                self::formatarDetalhesSql($sql, $this->arrBind)
            );
        }
    ...
    public function reativar($objInfraDTO, $bolRetornarSql = false)
    {
        try {
            ...
            if ($numReg == 0) {
                throw new InfraException(
                    'Registro de ' . $objInfraDTO->getStrNomeTabela() . ' não encontrado para reativação.',
                    null,
                    self::formatarDetalhesSql($sql, $this->arrBind)
                );
            }
            ...
            throw new InfraException(
                'Erro reativando registro de ' . $objInfraDTO->getStrNomeTabela() . '.',
                $e,
                self::formatarDetalhesSql($sql, $this->arrBind)
            );
        }
    ...
    public function contar($objInfraDTO, $bolRetornarSql = false)
    {
        try {
            ...
            if ($objInfraDTO->getDistinct()) {
                ...
                if ($strRet != '') {
                    $sql = 'SELECT COUNT(*) AS total FROM (SELECT DISTINCT ' . $strRet . ' ' . $this->montarFromJoinWhere(
                            $objInfraDTO
                        ) . ') Temp';
                }
            }
            ...
            if (!isset($rs[0]['total'])) {
                throw new InfraException(
                    'Contagem não retornou nenhum valor.',
                    null,
                    self::formatarDetalhesSql($sql, $this->arrBind)
                );
            }
            ...
            if (!is_numeric($rs[0]['total'])) {
                throw new InfraException(
                    'Contagem não retornou valor numérico.',
                    null,
                    self::formatarDetalhesSql($sql, $this->arrBind)
                );
            }
            ...
            throw new InfraException(
                'Erro contando registros de ' . $objInfraDTO->getStrNomeTabela() . '.',
                $e,
                self::formatarDetalhesSql($sql, $this->arrBind)
            );
        }
        ...
        try {
            ...
            if ($objInfraDTO->getNumPaginaAtual() === null) {
                if ($objInfraDTO->getNumMaxRegistrosRetorno() === null) {
                    ...
                    $rs = $this->getObjInfraIBanco()->limitarSql(
                        $sql,
                        $objInfraDTO->getNumMaxRegistrosRetorno(),
                        $this->arrBind
                    );
                }
                //Com paginação
            } else {
                $pag = $this->getObjInfraIBanco()->paginarSql(
                    $sql,
                    $objInfraDTO->getNumPaginaAtual() * $objInfraDTO->getNumMaxRegistrosRetorno(),
                    $objInfraDTO->getNumMaxRegistrosRetorno(),
                    $this->arrBind
                );
            ...
            */
            ...
            foreach ($rs as $item) {
                ...
                foreach ($arrKeysRet as $key) {
                    $valor = $this->lerCampo(
                        $arrAtributos[$key][InfraDTO::$POS_ATRIBUTO_PREFIXO],
                        $item[$this->arrAlias[$key]]
                    );
            ...
            throw new InfraException(
                'Erro listando registros de ' . $objInfraDTO->getStrNomeTabela() . '.',
                $e,
                self::formatarDetalhesSql($sql, $this->arrBind)
            );
        }
    ...
    public function bloquear($objInfraDTO, $bolRetornarSql = false)
    {
        try {
            ...
            foreach ($arrAtributos as $atributo) {
                if ($atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] !== null && $atributo[InfraDTO::$POS_ATRIBUTO_FLAGS] & InfraDTO::$FLAG_RET) {
                    if ($atributo[InfraDTO::$POS_ATRIBUTO_TAB_ORIGEM] !== null) {
                        throw new InfraException(
                            'Método bloquear aceita retorno apenas de campos da tabela principal.'
                        );
                    }
                    ...
                    call_user_func(
                        array(
                            $dto,
                            'ret' . $atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] . $atributo[InfraDTO::$POS_ATRIBUTO_NOME]
                        )
                    );
                    $bolRetTodos = false;
                }
            ...
            if ($arrPK != null) {
                ...
                foreach ($arrKeys as $key) {
                    $valor = call_user_func(
                        array($objInfraDTO, 'get' . $arrAtributos[$key][InfraDTO::$POS_ATRIBUTO_PREFIXO] . $key)
                    );
                    call_user_func(array($dto, 'set' . $arrAtributos[$key][InfraDTO::$POS_ATRIBUTO_PREFIXO] . $key),
                        $valor);
                }
            }
            ...
            if ($numRegistros === 1) {
                ...
                $dto = null;
            } elseif ($numRegistros > 1) {
                throw new InfraException(
                    'Mais de um registro encontrado para bloqueio.',
                    null,
                    self::formatarDetalhesSql($sql, $this->arrBind)
                );
            }
            ...
            throw new InfraException(
                'Erro bloqueando registro em ' . $objInfraDTO->getStrNomeTabela() . '.',
                $e,
                self::formatarDetalhesSql($sql, $this->arrBind)
            );
        }
            ...
            foreach ($arrNomesTabelasFK as $nomeTabelaFK) {
                if ($tab2 === $nomeTabelaFK) {
                    ...
                    foreach ($arrNomesAtributosFK as $nomeAtributoFK) {
                        ...
                        $strOn .= $this->montarPrefixoCampo(
                                $objInfraDTO,
                                $arrAtributos[$nomeAtributoFK]
                            ) . $arrAtributos[$nomeAtributoFK][InfraDTO::$POS_ATRIBUTO_CAMPO_SQL];
                        ...
                        foreach ($arrAtributos as $atributo) {
                            if ($atributo[InfraDTO::$POS_ATRIBUTO_TAB_ORIGEM] == $nomeTabelaFK) {
                                if ($atributo[InfraDTO::$POS_ATRIBUTO_FLAGS] & InfraDTO::$FLAG_SET) {
                                    $strOn .= $strSeparador;
                                    $strOn .= $this->montarCondicao(
                                        $objInfraDTO,
                                        $atributo,
                                        $objInfraDTO->getOperador($atributo),
                                        $atributo[InfraDTO::$POS_ATRIBUTO_VALOR]
                                    );
                                    $strSeparador = ' AND ';
                                }
    ...
    */
    ...
    protected function montarFromJoinWhere($objInfraDTO)
    {
        ...
        */
        ...
        if ($objInfraDTO->isBolConfigurouExclusaoLogica() && $objInfraDTO->isBolExclusaoLogica()) {
            ...
            $strWhere .= $strSeparador . $this->montarCondicao(
                    $objInfraDTO,
                    $arrAtributos['SinAtivo'],
                    InfraDTO::$OPER_IGUAL,
                    'S'
                );
            $strSeparador = ' AND ';
    ...
    private function montarCondicao($objInfraDTO, $atributo, $operador, $valor)
    {
        ...
        if ($valor === null) {
            ...
            } elseif ($operador == InfraDTO::$OPER_DIFERENTE) {
                $strRet .= $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] . ' IS NOT NULL';
            } else {
                throw new InfraException(
                    'Comparações com valores nulos somente são permitidas com os operadores de igualdade ou diferença.'
                );
            }
        } else {
            //Testa operação in
            if ($operador == InfraDTO::$OPER_IN || $operador == InfraDTO::$OPER_NOT_IN) {
                ...
                if ($operador == InfraDTO::$OPER_LIKE || $operador == InfraDTO::$OPER_NOT_LIKE) {
                    ...
                    if ($atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] == InfraDTO::$PREFIXO_STR) {
                        ...
                        //valor numerico nao precisa fazer upper
                        ...
                        $strCampo = $this->montarPrefixoCampo(
                                $objInfraDTO,
                                $atributo
                            ) . $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL];
                        $strBind = $this->adicionarBind($atributo, $valor);
                    } elseif ($atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] == InfraDTO::$PREFIXO_NUM || $atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] == InfraDTO::$PREFIXO_DBL || $atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] == InfraDTO::$PREFIXO_DIN) {
                        $strCampo = $this->getObjInfraIBanco()->converterStr(
                            $this->obterPrefixoCampo($objInfraDTO, $atributo),
                            $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL]
                        );
                        $strBind = $this->adicionarBind($atributo, $valor);
                    } else {
                        throw new InfraException(
                            'Operador LIKE não pode ser utilizado com atributos do tipo ' . $atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] . '.'
                        );
                    }
                    ...
                    $strRet .= $this->getObjInfraIBanco()->formatarPesquisaStr(
                        $strTabela,
                        $strCampo,
                        $valor,
                        $operador,
                ...
                } elseif ($operador == InfraDTO::$OPER_BIT_AND) {
                    if ($this->getObjInfraIBanco() instanceof InfraOracle) {
                        $strRet .= 'BITAND(' . $this->montarPrefixoCampo(
                                $objInfraDTO,
                                $atributo
                            ) . $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] . ', ' . $valor . ') = ' . $valor;
                    } else {
                        $strRet .= '(' . $this->montarPrefixoCampo(
                                $objInfraDTO,
                                $atributo
                            ) . $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] . ' ' . $operador . ' ' . $valor . ') = ' . $valor;
                    }
                } elseif (is_object($valor) && $valor instanceof InfraAtributoDTO) {
                    $strRet .= $this->montarPrefixoCampo($objInfraDTO, $atributo);
                    $strRet .= $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] . $operador;
                    ...
                    if ($valor === '' && !($this->getObjInfraIBanco(
                            ) instanceof InfraPostgreSql && $atributo[InfraDTO::$POS_ATRIBUTO_PREFIXO] != InfraDTO::$PREFIXO_STR)) {
                        $strRet .= $this->montarPrefixoCampo($objInfraDTO, $atributo);
                        $strRet .= $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] . $operador . '\'\'';
                    } else {
                        $strRet .= $this->montarPrefixoCampo(
                                $objInfraDTO,
                                $atributo
                            ) . $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] . $operador . $this->adicionarBind(
                                $atributo,
                                $valor
                            );
                    }
        ...
        if (is_array($objInfraDTO->getArrFK())) {
            ...
            foreach ($arrNomesTabelasFK as $nomeTabelaFK) {
                ...
                foreach ($arrNomesAtributosFK as $nomeAtributoFK) {
                    ...
                    //Testa se não é join entre outras tabelas, pode ser adicionado no
                    //DTO campos estrangeiros e forçar um join de outras tabelas
                    $strWhere .= $this->montarPrefixoCampo(
                            $objInfraDTO,
                            $arrAtributos[$nomeAtributoFK]
                        ) . $arrAtributos[$nomeAtributoFK][InfraDTO::$POS_ATRIBUTO_CAMPO_SQL];
            ...
            if (!in_array($i, $this->arrCriteriosProcessados)) {
                ...
                if ($this->verificarTabelasCriterios($objInfraDTO, $arrCriterios[$i], $arrTabelas)) {
                    ...
                    for ($j = 0; $j < $numGrupos; $j++) {
                        if (in_array(
                            $arrCriterios[$i][InfraDTO::$POS_CRITERIO_NOME],
                            $arrGrupos[$j][InfraDTO::$POS_GRUPO_CRITERIO_CRITERIOS]
                        )) {
                            break;
                        }
                    }
    ...
    private function montarCriterio($objInfraDTO, $arrCriterio)
    {
        ...
        for ($j = 0; $j < count($arrCriterio[InfraDTO::$POS_CRITERIO_ATRIBUTOS]); $j++) {
            ...
            $strCriterio .= $this->montarCondicao(
                $objInfraDTO,
                $arrAtributos[$arrCriterio[InfraDTO::$POS_CRITERIO_ATRIBUTOS][$j]],
                $arrCriterio[InfraDTO::$POS_CRITERIO_OPERADORES_ATRIBUTOS][$j],
                $arrCriterio[InfraDTO::$POS_CRITERIO_VALORES_ATRIBUTOS][$j]
            );
        }
        $strCriterio .= ')';
        return $strCriterio;
            ...
            foreach ($arrOrdenacao as $atributoOrd) {
                ...
                if ($atributo[InfraDTO::$POS_ATRIBUTO_FLAGS] & InfraDTO::$FLAG_ORD) {
                    if ($atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] !== null) {
                        ...
                        if ($this->objInfraIBanco instanceof InfraOracle || $this->objInfraIBanco instanceof InfraPostgreSql) {
                            if ($atributoOrd[1] == InfraDTO::$TIPO_ORDENACAO_ASC) {
                                $strRet .= ' NULLS FIRST';
                            } else {
                                $strRet .= ' NULLS LAST';
                            }
    ...
    protected function gerarAlias($objInfraDTO)
    {
        ...
        foreach ($arrAtributos as $atributo) {
            ...
            if ($bolMumps) {
                ...
                $this->arrAlias[$atributo[InfraDTO::$POS_ATRIBUTO_NOME]] = strtolower(
                    $atributo[InfraDTO::$POS_ATRIBUTO_CAMPO_SQL]
                );
            } else {
                ...
                if (strlen($strAlias) <= 30) {
                    ...
                    $this->arrAlias[$atributo[InfraDTO::$POS_ATRIBUTO_NOME]] = substr($strAlias, 0, 27) . str_pad(
                            $i,
                            3,
                            '0',
                            STR_PAD_LEFT
    ...
    protected function montarWherePK($objInfraDTO)
    {
        ...
        if ($arrPK != null) {
            ...
            foreach ($arrKeys as $key) {
                ...
                $strRet .= $strSeparador . $this->montarPrefixoCampo(
                        $objInfraDTO,
                        $arrAtributos[$key]
                    ) . $arrAtributos[$key][InfraDTO::$POS_ATRIBUTO_CAMPO_SQL] . '=' . $this->adicionarBind(
                        $arrAtributos[$key],
                        $valor
                    );
                $strSeparador = ' AND ';
        ...
        if ($strRet != '') {
            ...
            throw new InfraException(
                'Erro montando condição de chave-primária para tabela ' . $objInfraDTO->getStrNomeTabela() . '.'
            );
        }
        return $strRet;
    }
        ...
        if (!$this->bolRetornarSql) {
            if ($this->objInfraIBanco->isBolUsarPreparedStatement()) {
                if ($this->objInfraIBanco instanceof InfraMySqli) {
                    ...
                    $this->tipoBind = self::$TB_ORACLE;
                } elseif ($this->objInfraIBanco instanceof InfraSqlServer && $this->objInfraIBanco->getTipoInstalacao(
                    ) == InfraSqlServer::$TI_SQLSRV) {
    ...
    public static function formatarDetalhesSql($sql, $arrBind)
    {
        ...
        if (strlen($sql) < INFRA_TAM_MAX_LOG_SQL) {
            $strDetalhes = $sql;
        } else {
            $strDetalhes = substr($sql, 0, INFRA_TAM_MAX_LOG_SQL) . '...';
        }
            ...
            foreach ($arrBind as $chave => $valor) {
                ...
                if (!is_array($valor)) {
                    ...
                    foreach ($valor as $subchave => $subvalor) {
                        $strBind .= $strSep . $subvalor;
                        $strSep = ', ';
                    }
                }
                $strBind .= "\n";

                if (strlen($strBind) > INFRA_TAM_MAX_LOG_SQL) {
                    $strBind = substr($strBind, 0, INFRA_TAM_MAX_LOG_SQL) . '...';
                    break;
                }
            }
        ...
        if ($this->tipoBind != null) {
            if ($this->tipoBind == self::$TB_MYSQL) {
                ...
                if ($valorFormatado == 'NULL') {
                    ...
                    } elseif ($strPrefixo == InfraDTO::$PREFIXO_DTA || $strPrefixo == InfraDTO::$PREFIXO_DTH) {
                ...
                return '?';
            } elseif ($this->tipoBind == self::$TB_ORACLE) {
                ...
                if ($valorFormatado == 'NULL') {
                    ...
                    if ($strPrefixo == InfraDTO::$PREFIXO_STR) {
                        ...
                        $valorFormatado = substr($valor, 0, 10);
                    } elseif ($strPrefixo == InfraDTO::$PREFIXO_DTH) {
                        $valorFormatado = substr($valor, 0, 19);
                    }
                }
                ...
                return ':' . $strBind;
            } elseif ($this->tipoBind == self::$TB_SQLSERVER) {
                //valida campos
                $valorFormatado = $this->gravarCampo($strPrefixo, $valor);
                ...
                return '?';
            } elseif ($this->tipoBind == self::$TB_POSTGRESQL) {
    ...
    public function formatarIn($strCampo, $arrItens, $strPrefixo, $bolNotIn = false)
    {
        $ret = '';

        if ($this->getObjInfraIBanco() instanceof InfraOracle) {
            $arrPartes = array_chunk($arrItens, 1000);
        } else {
            $arrPartes = array($arrItens);
        }

        $strOr = '';
        foreach ($arrPartes as $arrParte) {
            $ret .= $strOr;
            $ret .= $strCampo . ' ' . ($bolNotIn ? 'not' : '') . ' in (';
            $strVirgula = '';
            foreach ($arrParte as $valor) {
                $ret .= $strVirgula;
                $ret .= $this->gravarCampo($strPrefixo, $valor);
                $strVirgula = ',';
            }
            $ret .= ')';
            $strOr = ' or ';
