    ...
    public function __call($func, $args)
    {
        ...
        try {
            ...
            if (!$bolFlagConectado && !$bolFlagControlado) {
                throw new InfraException('Método [' . get_class($this) . '.' . $func . '] não encontrado.');
            }
            ...
            if ($objInfraIBanco === null) {
                throw new InfraException(
                    'Não foi possível inicializar o banco de dados para a classe ' . get_class($this) . '.'
                );
            }
            ...
            if ($this->getObjInfraIBanco()->getIdConexao() !== null) {
                ...
                if ($this->getObjInfraIBanco()->getIdConexao() !== $this->getObjInfraIBanco()->getIdBanco()) {
                    ...
                    //InfraDebug::getInstance()->gravarInfra('[InfraRN->__call] Tentou abrir conexão com: '.$this->getObjInfraIBanco()->getIdBanco());
                    //throw new InfraException('Tentativa de abertura de conexão com outro banco de dados sem fechar a conexão anterior.');

                    if ($this->getObjInfraIBanco()->isBolProcessandoTransacao()) {
                        throw new InfraException(
                            'Tentativa de abertura de conexão com outro banco de dados sem fechar a transação anterior.'
                        );
                    }
                ...
                //InfraDebug::getInstance()->gravarInfra('[InfraRN->__call] Reutilizando conexão: '.$this->getObjInfraIBanco()->getIdConexao());


                //somente abre transação se: método controlado E o banco não esta em uma transação
                ...
                //Sem conexão aberta abre a conexão
                $this->getObjInfraIBanco()->abrirConexao();

                //abre transação se for um controlado
            ...
            if (count($args) == 0) {
                ...
                if ((is_object($args[1]) && get_class(
                    ...
                    throw new InfraException('Tipo inválido para o segundo parâmetro do método [' . $func . '].');
                }
            } else {
