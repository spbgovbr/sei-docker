...
class InfraLDAP
{
    ...
    public function autenticar($usuario, $senha, $servidor, $porta = 389)
    {
        try {
            ...
            try {
                ldap_bind($this->conexao, $this->usuario, InfraString::toUTF8($this->senha));
            } catch (Exception $e) {
            ...
            if (strpos(strtoupper($e2->__toString()), 'CAN\'T CONTACT LDAP SERVER') !== false) {
                $objInfraException->lancarValidacao(
                    'Não foi possível estabelecer conexão com o servidor de autenticação.'
                );
            }
            ...
            if (ldap_errno($this->conexao) == 0) {
                ...
                if (!empty($entry)) {
                    ...
                    if (!empty($dominio)) {
                        ...
                        try {
                            ldap_bind($this->conexao, $dominio, InfraString::toUTF8($this->senha));
                        ...
                        $objInfraException->adicionarValidacao(
                            'Não foi possível determinar o contexto LDAP do usuário.'
                        );
                    }
                } else {
    ...
    private function validarUsuarioAD()
    {
        ...
        try {
            ...
            try {
                ...
                $pesquisa = ldap_search(
                    $this->conexao,
                    $this->contexto,
                    'userPrincipalName=' . $this->usuario,
                ...
                if (strpos(strtoupper($e->__toString()), 'CAN\'T CONTACT LDAP SERVER') !== false) {
                    $objInfraException->lancarValidacao(
                        'Não foi possível estabelecer conexão com o servidor de autenticação AD.'
                    );
                }
    ...
    public function pesquisaAvancada(
        ...
        try {
            ...
            if ($usuarioPesquisa != null && $senhaPesquisa != null) {
                try {
                    ldap_bind($this->conexao, $usuarioPesquisa, InfraString::toUTF8($senhaPesquisa));
                } catch (Exception $e2) {
                ...
                try {
                    ldap_bind($this->conexao, $usuario, InfraString::toUTF8($senha));
                    $bolAutenticou = true;
                } catch (Exception $e2) {
                    throw $e2;
                }
            ...
            if ($contextoPesquisa == null && $atributoFiltro == null && $atributoRetorno == null) {
                ...
                $pesquisa = @ldap_search(
                    $this->conexao,
                    $contextoPesquisa,
                    $atributoFiltro . '=' . $usuario,
                    array($atributoRetorno)
                );
                ...
                if (!$bolAutenticou) {
                    ldap_bind($this->conexao, $dominio, InfraString::toUTF8($senha));
                }

                $dominio = InfraString::fromUTF8($dominio);
