...
abstract class InfraZabbix implements InfraIMonitoramento
{
    ...
    public function listarHosts(
        ...
        $strLimite = '',
        $strIdsHost = '',
        $bolSomenteAtivos = false,
        $bolBuscarAtributos = false
    ) {
        $arrCriterios = $this->montarArrayCriterios(
            'item',
            $strOrdem,
            $strLimite,
            $strNome,
    ...
    public function listarAtributos(
        ...
        $strLimite = '',
        $bolSomenteAtivos = false,
        $bolSomenteAssociadosTemplate = false,
        $bolBuscarApplications = false
    ) {
        $arrCriterios = $this->montarArrayCriterios(
            'atributo',
            $strOrdem,
            ...
            $strApplicationName,
            $bolSomenteAtivos,
            $bolBuscarApplications,
            $bolSomenteAssociadosTemplate
        );
        $arrResultado = $this->objZabbixApi->itemGet($arrCriterios);
        return $this->montarArrayResultadoFinal($arrResultado);
    }

    public function listarPrototipos(
        $strNome = '',
        $strApplicationName = '',
        $strOrdem = '',
        $strLimite = '',
    ...
    private function montarArrayCriterios(
        $strTipo,
        $strOrdem = '',
        $strLimite = '',
        $strNome = '',
            ...
            for ($i = 0; $i < count($arrResultado); $i++) {
                ...
                if (isset($arrResultado[$i]->applications) && (count($arrResultado[$i]->applications) > 0)) {
                    ...
                    foreach ($arrResultado[$i]->applications as $objApplication) {
                        $arrApplications[] = array(
                            'id_application' => $objApplication->applicationid,
