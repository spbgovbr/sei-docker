...
require_once __DIR__ .'/../../../../../../lib/Diff/Renderer/Text/Onlydiff.php';
...
final class AntiXSS
{
  ...
  public static $_arr_decoded_value=[];
  public static $_arr_decoded_str=[];
  ...
  /**
   * função própria para verificar conteúdo extraído pelo antiXSS
   * @return string
   */
      ...
      if ($strCopy!==$str) {
        ...
        foreach ($matchesTmp as $matches) {
          if (isset($matches['attr'])) {
            $strTmp=(string)$matches['attr'];
            if(strlen($strTmp)<40) {

              if (preg_match('/^(width:\s*\d+px;?)|(lnkSei\d+)$/', $strTmp)) {
                if (!isset(self::$_arr_decoded_value['width/lnksei'])) {
                  self::$_arr_decoded_value['width/lnksei'] = 0;
                }
                self::$_arr_decoded_value['width/lnksei']++;
                continue;
              }
              if (isset(self::$_arr_decoded_value[$strTmp])) {
                self::$_arr_decoded_value[$strTmp]++;
                continue;
              }
            }
            ...
            } elseif(strlen($strTmp)<40) {
              self::$_arr_decoded_value[$strTmp]=1;
            }
          }
        }
      }
    ...
    if (
      ...
      $str = (string)\preg_replace_callback(
          ...
          function ($matches) {
            ...
            if(!isset(self::$_arr_decoded_str[$find])){
              $ret=$this->_decode_entity($matches);
              if($ret==$matches[0]){
                self::$_arr_decoded_str[$find]=1;
              }
              return $ret;
            } else {
              self::$_arr_decoded_str[$find]++;
            }
            return $matches[0];
          },
          $str
...
//      \InfraDebug::getInstance()->gravarInfra('AntiXSS: preparando texto len(' . strlen($str) . ')');
//    }
    ...
    $str_backup2=$this->_debug_infra($str_backup2,$str,'_compact_exploded_javascript');

    // remove disallowed javascript calls in links, images etc.
    $str = $this->_remove_disallowed_javascript($str);
    $str_backup2=$this->_debug_infra($str_backup2,$str,'_remove_disallowed_javascript');

    // remove strings that are never allowed
    $str = $this->_do_never_allowed($str);
    $str_backup2=$this->_debug_infra($str_backup2,$str,'_do_never_allowed');

    // remove evil attributes such as style, onclick and xmlns
    $str = $this->_remove_evil_attributes($str);
    $str_backup2=$this->_debug_infra($str_backup2,$str,'_remove_evil_attributes');

    // sanitize naughty JavaScript elements
    $str = $this->_sanitize_naughty_javascript($str);
    $str_backup2=$this->_debug_infra($str_backup2,$str,'_sanitize_naughty_javascript');

    // sanitize naughty HTML elements
    $str = $this->_sanitize_naughty_html($str);
    $str_backup2=$this->_debug_infra($str_backup2,$str,'_sanitize_naughty_html');
    ...
    $str_backup2=$this->_debug_infra($str_backup2,$str,'_do_never_allowed_afterwards');
    ...
    if ($this->_xss_found===true && $this->_xss_diff_processing===true) {
    ...
    if ($this->_xss_found===true && \InfraDebug::isBolProcessar()) {
      \InfraDebug::getInstance()->gravarInfra('AntiXSS: detecção concluída - len(' . strlen($str) . ')');
    }
    self::$_level--;
    return $str;
  }
  ...
  private function _do_never_allowed($str)
  {
    ...
    foreach ($this->_never_allowed_call_strings as $call) {
      if (stripos($str, $call)!==false) {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('never_allowed_call [ ' . $call . ' ]  PREG_ERROR: ' . preg_last_error());
        }
      }
    }
    ...
    foreach ($this->_never_allowed_regex as $regex => $replacement) {
      if(strpos($regex,'src')!==0 || strpos($str,'.js')!==false) {
        $str = (string)\preg_replace(
            '#' . $regex . '#iUus',
            $replacement,
            $str
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('never_allowed_regex [ ' . $regex . ' ]  PREG_ERROR: ' . preg_last_error());
        }
      }
    }
    ...
    if ($this->_cache_never_allowed_regex_string) {
      ...
      if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
        \InfraDebug::getInstance()->gravarInfra('never_allowed_regex 2 [ ' . $this->_cache_never_allowed_regex_string . ' ]  PREG_ERROR: ' . preg_last_error());
      }
      ...
      foreach ($this->_get_never_allowed_on_events_afterwards_chunks() as $eventNameBeginning => $events) {
        ...
        foreach ($events as $event) {
          ...
          do {
            ...
            if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
              \InfraDebug::getInstance()->gravarInfra('never_allowed_afterwards [ ' . $event . ' ]  PREG_ERROR: ' . preg_last_error());
            }
            $count += $temp_count;
          } while ($count);
  ...
  private function _js_removal_callback($match, $search)
  {
    ...
    if (\stripos($match[1], $search . '=')!==false) {
      ...
      if (\preg_match($pattern, $match[1], $matchInner)) {
        ...
        if ($needProtection) {
          ...
          if ($tmpAntiXss->isXssFound()===true) {
            ...
            if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
              \InfraDebug::getInstance()->gravarInfra('js_removal_callback #2 PREG_ERROR: ' . preg_last_error());
            }
          }
        }
      } elseif (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
        \InfraDebug::getInstance()->gravarInfra('js_removal_callback #1 PREG_ERROR: ' . preg_last_error());
      }
      if (!$foundSomethingBad) {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('js_removal_callback #3 PREG_ERROR: ' . preg_last_error());
        }
      }
    }
  ...
  private function _remove_disallowed_javascript($str)
  {
    do {
      ...
      if (\stripos($str, '<a')!==false) {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('remove_disallowed_javascript a #1 PREG_ERROR: ' . preg_last_error());
        }
        if ($strTmp===null) {
          $strTmp = \preg_replace_callback(
              '#<a[^\p{L}@>]+([^>]*)(?:>|$)#iu',
          ...
          if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
            \InfraDebug::getInstance()->gravarInfra('remove_disallowed_javascript a #2 PREG_ERROR: ' . preg_last_error());
          }
        }
        $str = (string)$strTmp;
      }
      ...
      if (\stripos($str, '<img')!==false) {
        $strTmp = \preg_replace_callback(
            ...
            function ($matches) {
              ...
              if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
                \InfraDebug::getInstance()->gravarInfra('remove_disallowed_javascript img #2 PREG_ERROR: ' . preg_last_error());
              }
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('remove_disallowed_javascript img  PREG_ERROR: ' . preg_last_error());
        }
        if ($strTmp===null) {
      ...
      if (\stripos($str, '<audio')!==false) {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('remove_disallowed_javascript audio  PREG_ERROR: ' . preg_last_error());
        }
        if ($strTmp===null) {
          $strTmp = (string)\preg_replace_callback(
              '#<audio[^\p{L}@]+([^>]*)(?:\s?/?>|$)#iu',
      ...
      if (\stripos($str, '<video')!==false) {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('remove_disallowed_javascript video  PREG_ERROR: ' . preg_last_error());
        }
        if ($strTmp===null) {
          $strTmp = \preg_replace_callback(
              '#<video[^\p{L}@]+([^>]*)(?:\s?/?>|$)#iu',
      ...
      if (\stripos($str, '<source')!==false) {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('remove_disallowed_javascript source  PREG_ERROR: ' . preg_last_error());
        }
      }
      ...
      if (\stripos($str, 'script')!==false) {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('remove_disallowed_javascript script  PREG_ERROR: ' . preg_last_error());
        }
      }
      ...
      if (\stripos($str, 'script')!==false) {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('remove_disallowed_javascript script 2  PREG_ERROR: ' . preg_last_error());
        }
      }
    } while ($original!==$str);
  ...
  private function _remove_evil_attributes($str)
  {
    ...
    if (
      ...
      do {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('_remove_evil_attributes style  PREG_ERROR: ' . preg_last_error());
        }
        $count += $temp_count;
      } while ($count);
    }
    ...
    do {
      ...
      $strTmp = \preg_replace($regex, '$1$2' . $this->_replacement . '$3$4', $str, - 1, $temp_count);
      if(preg_last_error()!=0){
        $arrRegex=explode('|',$this->_cache_evil_attributes_regex_string);
        foreach ($arrRegex as $atributo) {
          if(strpos($str,$atributo)!==false) {
            $regex = '/(.*)((?:<[^>]+)(?<!\p{L}))(?:' . $atributo . ')(?:\s*=\s*)(?:\'(?:.*?)\'|"(?:.*?)")(.*)/ius';
            $strTmp = \preg_replace($regex, '$1$2' . $this->_replacement . '$3$4', $str, - 1, $temp_count);
            if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
              \InfraDebug::getInstance()->gravarInfra('_remove_evil_attributes #1  PREG_ERROR: ' . preg_last_error() . ' REGEX: ' . $regex . ' LEN:' . strlen($str));
            }
          }
        }
      }
      ...
      if ($strTmp===null) {
        ...
        $strTmp = \preg_replace($regex, $this->_replacement, $str, - 1, $temp_count);
        if(preg_last_error()!=0){
          $arrRegex=explode('|',$this->_cache_evil_attributes_regex_string);
          foreach ($arrRegex as $atributo) {
            if(strpos($str,$atributo)!==false) {
              $regex = '/(?:' . $atributo . ')(?:\s*=\s*)(?:\'(?:.*?)\'|"(?:.*?)")/ius';
              $strTmp = \preg_replace($regex, $this->_replacement, $str, - 1, $temp_count);
              if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
                \InfraDebug::getInstance()->gravarInfra('_remove_evil_attributes #2  PREG_ERROR: ' . preg_last_error() . ' REGEX: ' . $regex . ' LEN:' . strlen($str));
              }
            }
          }
        }
      ...
      $strTmp = \preg_replace($regex, '$1$2' . $this->_replacement . '$3', $str, - 1, $temp_count);
      if(preg_last_error()!=0){
        $arrRegex=explode('|',$this->_cache_evil_attributes_regex_string);
        foreach ($arrRegex as $atributo) {
          if(strpos($str,$atributo)!==false) {
            $regex = '/(.*?)(<[^>]+)(?<!\p{L})(?:' . $atributo. ')\s*=\s*(?:[^\s>]*)/ius';
            $strTmp = \preg_replace($regex, '$1$2' . $this->_replacement . '$3', $str, - 1, $temp_count);
            if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
              \InfraDebug::getInstance()->gravarInfra('_remove_evil_attributes #3  PREG_ERROR: ' . preg_last_error() . ' REGEX: ' . $regex . ' LEN:' . strlen($str));
            }
          }
        }
      }
        ...
        if(preg_last_error()!=0){
          ...
          foreach ($arrRegex as $atributo) {
            if(strpos($str,$atributo)!==false) {
              ...
              $strTmp = \preg_replace($regex, '$1$2' . $this->_replacement . '$3', $str, - 1, $temp_count);
              if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
                \InfraDebug::getInstance()->gravarInfra('_remove_evil_attributes #4  PREG_ERROR: ' . preg_last_error() . ' REGEX: ' . $regex . ' LEN:' . strlen($str));
              }
            }
          }
        }
  ...
  private function _sanitize_naughty_html($str)
  {
    ...
    do {
      ...
      if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
        \InfraDebug::getInstance()->gravarInfra('_sanitize_naughty_html  PREG_ERROR: ' . preg_last_error());
      }
      if ($str!=$str_backup && \InfraDebug::isBolProcessar()) {
        \InfraDebug::getInstance()->gravarInfra('_sanitize_naughty_html   **Limpo**');
        $str_backup = $str;
      }
      ...
      if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
        \InfraDebug::getInstance()->gravarInfra('_sanitize_naughty_html #2  PREG_ERROR: ' . preg_last_error());
      }
  ...
  private function _close_html_callback($matches)
  {
    if (empty($matches['closeTag'])) {
      ...
      if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
        \InfraDebug::getInstance()->gravarInfra('_close_html_callback  PREG_ERROR: ' . preg_last_error());
      }
      return '&lt;' . \str_replace(['>', '<'], ['&gt;', '&lt;'], $matches[1]);
    }
  ...
  private function _sanitize_naughty_javascript($str)
  {
    if (\strpos($str, '(')!==false) {
      ...
      if ($found===true) {
        ...
        if (preg_last_error()!=0 && \InfraDebug::isBolProcessar()) {
          \InfraDebug::getInstance()->gravarInfra('_sanitize_naughty_javascript    PREG_ERROR: ' . preg_last_error());
        }
      }
    }
  ...
  public function xss_clean($str)
  {
    ...
    $debug = \InfraDebug::isBolProcessar();
    ...
    if (\is_array($str)) {
      ...
      \InfraDebug::getInstance()->setBolLigado($debug);
    ...
    do {
      $old_str = $str;
      $str = $this->_do($str);
      if ($str!=$old_str && \InfraDebug::isBolProcessar()) {
        \InfraDebug::getInstance()->gravarInfra('AntiXSS: _cleanXSS alteração identificada, executando novamente');
      }
    } while ($old_str!==$str);
  ...
  public function _debug_infra($strOriginal,$strFinal,$strMetodo)
  {
    if (!$this->_debug || $strOriginal==$strFinal || !InfraDebug::isBolProcessar()) {
      return $strFinal;
    }

    InfraDebug::getInstance()->gravarInfra('AntiXSS: ' . $strMetodo . ' modificou string');
    if (strlen($strOriginal)<500) {
      InfraDebug::getInstance()->gravarInfra('String inicial= ' . $strOriginal);
      InfraDebug::getInstance()->gravarInfra('String final= ' . $strFinal);
    }
    return $strFinal;
  }

  public function prepareText(string $str): string
  {
    $str = UTF8::remove_bom($str);
