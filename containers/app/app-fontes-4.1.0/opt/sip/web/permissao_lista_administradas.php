...
try {
  ...
  PaginaSip::getInstance()->salvarCamposPost(array(
      'selOrgaoSistema', 'selSistema', 'selOrgaoUnidade', 'selUnidade', 'selOrgaoUsuario', 'txtUsuario', 'hdnIdUsuario', 'hdnSiglaUsuario', 'hdnNomeUsuario', 'selPerfil'
    ));
  ...
  if ($numRegistros > 0) {
    ...
    for ($i = 0; $i < $numRegistros; $i++) {
      $bolVencida = InfraData::compararDatas($arrObjPermissaoDTO[$i]->getDtaDataInicio(), $dtaAtual) < 0 || ($arrObjPermissaoDTO[$i]->getDtaDataFim() != null && InfraData::compararDatas($dtaAtual,
            $arrObjPermissaoDTO[$i]->getDtaDataFim()) < 0);
...
if ($bolAcaoCopiar) { ?>
  ...
  function bloquearCopiaSelecionado(){

  var naoCopiar = document.getElementById('hdnIdNaoCopiar').value;
  ...
  if (selecionados!='') {
  bolErro = false;
  naoCopiar = naoCopiar.split(',');
  selecionados = selecionados.split(',');
  n = 0;

  for (var i = 0; i
  <naoCopiar.length; i++) {
  var chk = document.getElementById('chkInfraItem'+naoCopiar[i]);
  if (chk!=null){
  for (var j = 0; j
  <selecionados.length; j++) {
  if (chk.value==selecionados[j]) {
  chk.checked = false;
  infraFormatarTrDesmarcada(chk);
  infraSelecionarItens(null,'Infra');
  bolErro = true;
  n++;
  }
  }
  ...
  if (bolErro){
  msg = 'Operação não aplicável em permissões fora do período de validade.\n\n';
  if ( n == 1  ){
  msg += '1 registro foi desmarcado.';
  }else{
  msg += n + ' registros foram desmarcados.';
  }
  alert(msg);
  }
  }
...
?>
  <form id="frmPermissaoListaAdministradas" method="post" action="<?=SessaoSip::getInstance()->assinarLink('permissao_lista_administradas.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    ...
    <label id="lblOrgaoUnidade" for="selOrgaoUnidade" accesskey="g" class="infraLabelOpcional">Ór<span
    ...
    <label id="lblUnidade" for="selUnidade" accesskey="U" class="infraLabelOpcional"><span
    ...
    <select id="selOrgaoUsuario" name="selOrgaoUsuario" onchange="objAjaxUsuario.limpar();this.form.submit();"
            class="infraSelect"
            tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$strDesabilitarUsuario?>>
      <?=$strItensSelOrgaoUsuario?>
    </select>

    <label id="lblUsuario" for="txtUsuario" accesskey="i" class="infraLabelOpcional">Usuár<span
        class="infraTeclaAtalho">i</span>o:</label>
    ...
    <label id="lblPerfil" for="selPerfil" accesskey="P" class="infraLabelOpcional"><span
        class="infraTeclaAtalho">P</span>erfil:</label>
    <select id="selPerfil" name="selPerfil" onchange="this.form.submit();" class="infraSelect"
            tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$strDesabilitarPerfil?>>
      <?=$strItensSelPerfil?>
    </select>
    ...
    <input type="hidden" id="hdnIdNaoCopiar" name="hdnIdNaoCopiar" value="<?=$strIdNaoCopiar?>"/>
    <?
