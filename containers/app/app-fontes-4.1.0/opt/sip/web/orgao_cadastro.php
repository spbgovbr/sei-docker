...
?>
  ...
  #selServidoresAutenticacao {position:absolute;left:0%;top:11%;width:50%;}
  #divOpcoesServidoresAutenticacao {position:absolute;left:51%;top:11%;}
...
?>
  <form id="frmOrgaoCadastro" method="post" onsubmit="return OnSubmitForm();"
        action="<?=SessaoSip::getInstance()->assinarLink('orgao_cadastro.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    <?
    ...
    <div id="divGeral" class="infraAreaDados" style="height:15em;">
      ...
      <input type="text" id="txtSigla" name="txtSigla" class="infraText"
             value="<?=PaginaSip::tratarHTML($objOrgaoDTO->getStrSigla());?>" maxlength="30"
      ...
      <input type="text" id="txtDescricao" name="txtDescricao" class="infraText"
             value="<?=PaginaSip::tratarHTML($objOrgaoDTO->getStrDescricao());?>" maxlength="250"
             tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>

      <label id="lblOrdem" for="txtOrdem" class="infraLabelObrigatorio">Ordem:</label>
      <input type="text" id="txtOrdem" name="txtOrdem" onkeypress="return infraMascaraNumero(this, event)"
             class="infraText" value="<?=PaginaSip::tratarHTML($objOrgaoDTO->getNumOrdem());?>"
             tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>
      <a href="javascript:void(0);" id="ancAjudaOrdem" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"
         onmouseover="return infraTooltipMostrar('<?=PaginaSip::tratarHTML(PaginaSip::formatarParametrosJavaScript('Permite alterar a posição do órgão na lista da tela de login. Órgãos compartilhando o mesmo valor para este campo serão ordenados alfabeticamente pela sigla.'))?>','',200);"
         onmouseout="return infraTooltipOcultar();"><img
          src="<?=PaginaSip::getInstance()->getIconeAjuda()?>" class="infraImg"/></a>
    ...
    <div id="divServidoresAutenticacao" class="infraAreaDados" style="height:15em;">
      <label id="lblServidoresAutenticacao" for="selServidoresAutenticacao" class="infraLabelOpcional">Servidores de
        Autenticação Associados:</label>
      <select id="selServidoresAutenticacao" name="selServidoresAutenticacao" class="infraSelect" multiple="multiple"
              tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>">
        <?=$strItensSelServidoresAutenticacao?>
      </select>
      <div id="divOpcoesServidoresAutenticacao">
        <img id="imgPesquisarServidoresAutenticacao" onclick="objLupaServidoresAutenticacao.selecionar(700,500);"
             src="<?=PaginaSip::getInstance()->getIconePesquisar()?>" alt="Pesquisa de Servidores de Autenticação"
             title="Pesquisa de Servidores de Autenticação" class="infraImg"
             tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>
        <img id="imgRemoverServidoresAutenticacao" onclick="objLupaServidoresAutenticacao.remover();"
             src="<?=PaginaSip::getInstance()->getIconeRemover()?>"
             alt="Remover Servidores de Autenticação Selecionados"
             title="Remover Servidores de Autenticação Selecionados" class="infraImg"
             tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>
        <br/>
        <img id="imgServidoresAutenticacaoAcima" onclick="objLupaServidoresAutenticacao.moverAcima();"
             src="<?=PaginaSip::getInstance()->getIconeMoverAcima()?>"
             alt="Mover Acima Servidor de Autenticação Selecionado"
             title="Mover Acima Servidor de Autenticação Selecionado" class="infraImg"
             tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>
        <img id="imgServidoresAutenticacaoAbaixo" onclick="objLupaServidoresAutenticacao.moverAbaixo();"
             src="<?=PaginaSip::getInstance()->getIconeMoverAbaixo()?>"
             alt="Mover Abaixo Servidor de Autenticação Selecionado"
             title="Mover Abaixo Servidor de Autenticação Selecionado" class="infraImg"
             tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>
      </div>
    </div>

    <input type="hidden" name="hdnIdOrgao" value="<?=$objOrgaoDTO->getNumIdOrgao();?>"/>
    <input type="hidden" id="hdnServidoresAutenticacao" name="hdnServidoresAutenticacao"
           value="<?=PaginaSip::tratarHTML($_POST['hdnServidoresAutenticacao'])?>"/>
    <input type="hidden" id="hdnSinAutenticar" name="hdnSinAutenticar"
           value="<?=PaginaSip::tratarHTML($objOrgaoDTO->getStrSinAutenticar())?>"/>
    <?
