...
?>
  <form id="frmCoordenadorUnidadeCadastro" method="post" onsubmit="return OnSubmitForm();"
        action="<?=SessaoSip::getInstance()->assinarLink('coordenador_unidade_cadastro.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    <?
    ...
    <input type="text" id="txtUsuario" name="txtUsuario" class="infraText"
           value="<?=PaginaSip::tratarHTML($objCoordenadorUnidadeDTO->getStrSiglaUsuario())?>" maxlength="100"
           tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$strDesabilitar?> />

    <input type="hidden" id="hdnIdUsuario" name="hdnIdUsuario"
           value="<?=$objCoordenadorUnidadeDTO->getNumIdUsuario();?>"/>
