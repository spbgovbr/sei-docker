...
try {
  ...
  $arrComandos[] = '<input type="submit" id="sbmPesquisar" name="sbmPesquisar" value="Pesquisar" class="infraButton" />';
  ...
  if (isset($_POST['sbmPesquisar']) || $numIdUnidadeRamificacao != null || in_array($_GET['acao_origem'], array(
      'rel_hierarquia_unidade_listar', 'rel_hierarquia_unidade_excluir', 'rel_hierarquia_unidade_desativar', 'rel_hierarquia_unidade_reativar', 'rel_hierarquia_unidade_cadastrar', 'rel_hierarquia_unidade_alterar'
    ))) {
    $objRelHierarquiaUnidadeRN = new RelHierarquiaUnidadeRN();
  ...
  if ($numRegistros > 0) {
    ...
    $strResultado .= '<th class="infraTh" width="12%">' . PaginaSip::getInstance()->getThOrdenacao($objRelHierarquiaUnidadeDTO, 'Unidade', 'SiglaUnidade', $arrObjRelHierarquiaUnidadeDTO) . '</th>';
    $strResultado .= '<th class="infraTh" width="12%">' . PaginaSip::getInstance()->getThOrdenacao($objRelHierarquiaUnidadeDTO, 'Órgão', 'SiglaOrgaoUnidade', $arrObjRelHierarquiaUnidadeDTO) . '</th>';
    //$strResultado .= '<th class="infraTh">Superiores</th>';
    //$strResultado .= '<th class="infraTh">Inferiores</th>';
    $strResultado .= '<th class="infraTh" width="10%">' . PaginaSip::getInstance()->getThOrdenacao($objRelHierarquiaUnidadeDTO, 'Data&nbsp;Inicial', 'DataInicio', $arrObjRelHierarquiaUnidadeDTO) . '</th>';
    $strResultado .= '<th class="infraTh" width="10%">' . PaginaSip::getInstance()->getThOrdenacao($objRelHierarquiaUnidadeDTO, 'Data&nbsp;Final', 'DataFim', $arrObjRelHierarquiaUnidadeDTO) . '</th>';
    ...
    for ($i = 0; $i < $numRegistros; $i++) {
      ...
      if ($bolCheck) {
        $strResultado .= '<td valign="center">' . PaginaSip::getInstance()->getTrCheck($i, $arrObjRelHierarquiaUnidadeDTO[$i]->getNumIdUnidade() . '-' . $arrObjRelHierarquiaUnidadeDTO[$i]->getNumIdHierarquia(),
            $arrObjRelHierarquiaUnidadeDTO[$i]->getStrSiglaUnidade()) . '</td>';
      }
      $strResultado .= '<td valign="center">' . PaginaSip::tratarHTML($arrObjRelHierarquiaUnidadeDTO[$i]->getStrRamificacao()) . '</td>';
      /*
...
?>
  <form id="frmRelHierarquiaUnidadeLista" method="post" onsubmit="return OnSubmitForm();"
    ...
    <select id="selHierarquia" name="selHierarquia" class="infraSelect" onchange="trocarHierarquia()"
            tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>">
      <?=$strItensSelHierarquia?>
    </select>

    <label id="lblUnidadeRamificacao" for="txtUnidadeRamificacao" class="infraLabelOpcional">Unidade da
      Ramificação:</label>
    <input type="text" id="txtUnidadeRamificacao" name="txtUnidadeRamificacao" class="infraText"
           value="<?=PaginaSip::tratarHTML($strUnidadeRamificacao)?>"
           tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>
    <input type="hidden" id="hdnIdUnidadeRamificacao" name="hdnIdUnidadeRamificacao"
           value="<?=PaginaSip::tratarHTML($numIdUnidadeRamificacao)?>"/>
