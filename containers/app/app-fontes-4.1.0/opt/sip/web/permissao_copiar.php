...
?>
  ...
  #fldUnidade {position:absolute;left:38%;top:5%;height:85%;width:30%;}
  #divOptMesmaUnidade {position:absolute;left:5%;top:<?=PaginaSip::getInstance()->isBolAjustarTopFieldset() ? '17%' : '27%'?>;}
  #divOptUnidadeInformada {position:absolute;left:5%;top:<?=PaginaSip::getInstance()->isBolAjustarTopFieldset() ? '57%' : '67%'?>;}

  #lblOrgaoUnidade {position:absolute;left:71%;top:0%;width:25%;visibility:hidden;}
  #selOrgaoUnidade {position:absolute;left:71%;top:20%;width:25%;visibility:hidden;}

  #lblUnidade {position:absolute;left:71%;top:50%;width:25%;visibility:hidden;}
  #selUnidade {position:absolute;left:71%;top:70%;width:25%;visibility:hidden;}
...
?>
  <form id="frmPermissaoCopiar" method="post" onsubmit="return OnSubmitForm();"
        action="<?=SessaoSip::getInstance()->assinarLink('permissao_copiar.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    <?
    ...
    <input type="text" id="txtUsuarioCopia" name="txtUsuarioCopia" class="infraText"
    ...
    <fieldset id="fldUnidade" class="infraFieldset">
      ...
      <div id="divOptMesmaUnidade" class="infraDivRadio">
        <input type="radio" name="rdoTipoUnidadeDestino" id="optMesmaUnidade" value="M" onclick="tipoUnidadeDestino()"
               class="infraRadio" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$_POST['rdoTipoUnidadeDestino'] == 'M' ? 'checked=checked' : ''?> />
        <label id="lblMesmaUnidade" for="optMesmaUnidade" class="infraLabelRadio">Mesma da permissão original</label>
        <br/>
      </div>

      <div id="divOptUnidadeInformada" class="infraDivRadio">
        <input type="radio" name="rdoTipoUnidadeDestino" id="optUnidadeInformada" value="I"
               onclick="tipoUnidadeDestino()" class="infraRadio"
               tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$_POST['rdoTipoUnidadeDestino'] == 'I' ? 'checked=checked' : ''?> />
