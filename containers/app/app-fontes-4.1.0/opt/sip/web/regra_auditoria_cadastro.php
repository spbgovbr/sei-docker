...
?>
  <form id="frmRegraAuditoriaCadastro" method="post" onsubmit="return OnSubmitForm();"
        action="<?=SessaoSip::getInstance()->assinarLink('regra_auditoria_cadastro.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    <?
    ...
    <div id="divCabecalho" class="infraAreaDados" style="height:15em;">
      ...
      <label id="lblOrgaoSistema" for="selOrgaoSistema" accesskey="" class="infraLabelObrigatorio">Órgão do
        Sistema:</label>
      <select id="selOrgaoSistema" name="selOrgaoSistema" onchange="trocarOrgaoSistema();" class="infraSelect"
      ...
      <input type="text" id="txtDescricao" name="txtDescricao" class="infraText"
             ...
             value="<?=PaginaSip::tratarHTML($objRegraAuditoriaDTO->getStrDescricao());?>"/>
    </div>
    ...
    <div id="divRecursos" class="infraAreaDados" style="height:37em;">
      ...
      <select id="selRecursos" name="selRecursos"
              size="<?=PaginaSip::getInstance()->isBolNavegadorChrome() ? '19' : '20'?>" multiple="multiple"
              class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>">
        <?=$strItensSelRecursos?>
      </select>
      <img id="imgLupaRecursos" onclick="objLupaRecursos.selecionar(700,500);"
           src="<?=PaginaSip::getInstance()->getIconePesquisar()?>" alt="Selecionar Recursos"
           title="Selecionar Recursos" class="infraImg" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>
      <img id="imgExcluirRecursos" onclick="objLupaRecursos.remover();"
           src="<?=PaginaSip::getInstance()->getIconeRemover()?>" alt="Remover Recursos Selecionados"
           title="Remover Recursos Selecionados" class="infraImg"
           tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>
    </div>

    <input type="hidden" id="hdnIdRegraAuditoria" name="hdnIdRegraAuditoria"
           value="<?=$objRegraAuditoriaDTO->getNumIdRegraAuditoria();?>"/>
    <input type="hidden" id="hdnIdOrgaoSistema" name="hdnIdOrgaoSistema"
           value="<?=$objRegraAuditoriaDTO->getNumIdOrgaoSistema();?>"/>
    <input type="hidden" id="hdnIdSistema" name="hdnIdSistema"
           value="<?=$objRegraAuditoriaDTO->getNumIdSistema();?>"/>
