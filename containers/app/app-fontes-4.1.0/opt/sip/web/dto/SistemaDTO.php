...
class SistemaDTO extends InfraDTO {
  ...
  public function montar() {
    ...
    $this->adicionarAtributoTabela(InfraDTO::$PREFIXO_STR, 'PaginaInicial', 'pagina_inicial');

    $this->adicionarAtributoTabela(InfraDTO::$PREFIXO_STR, 'WebService', 'web_service');

    $this->adicionarAtributoTabela(InfraDTO::$PREFIXO_STR, 'Logo', 'logo');

    $this->adicionarAtributoTabela(InfraDTO::$PREFIXO_STR, 'ServicosLiberados', 'servicos_liberados');

    $this->adicionarAtributoTabela(InfraDTO::$PREFIXO_STR, 'ChaveAcesso', 'chave_acesso');

    $this->adicionarAtributoTabela(InfraDTO::$PREFIXO_STR, 'Crc', 'crc');

    $this->adicionarAtributoTabela(InfraDTO::$PREFIXO_STR, 'Sta2Fatores', 'sta_2_fatores');

    $this->adicionarAtributoTabela(InfraDTO::$PREFIXO_STR, 'EsquemaLogin', 'esquema_login');
