...
?>
  <form id="frmPerfilListaCoordenados" method="post" action="<?=SessaoSip::getInstance()->assinarLink('controlador.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    ...
    <div id="divSinCoordenadoPeloUsuario" class="infraDivCheckbox">
      <input type="checkbox" id="chkCoordenadoPeloUsuario" name="chkCoordenadoPeloUsuario" onclick='this.form.submit();'
             class="infraCheckbox" <?=PaginaSip::getInstance()->setCheckBox($objPerfilDTO->getStrSinCoordenadoPeloUsuario())?> <?=$strDesabilitar?>/>
      <label id="lblCoordenadoPeloUsuario" for="chkCoordenadoPeloUsuario" class="infraLabelCheckbox">Visualizar somente
        coordenados por mim</label>
    </div>
