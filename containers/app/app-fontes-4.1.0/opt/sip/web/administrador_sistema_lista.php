...
try {
  ...
  if ($numRegistros > 0) {
    ...
    for ($i = 0; $i < $numRegistros; $i++) {
      ...
      if ($bolCheck) {
        $strResultado .= '<td valign="center">' . PaginaSip::getInstance()->getTrCheck($i, $arrObjAdministradorSistemaDTO[$i]->getNumIdSistema() . '-' . $arrObjAdministradorSistemaDTO[$i]->getNumIdUsuario(),
            $arrObjAdministradorSistemaDTO[$i]->getStrSiglaSistema()) . '</td>';
      }
