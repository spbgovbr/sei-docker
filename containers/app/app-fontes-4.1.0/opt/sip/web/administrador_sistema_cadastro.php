...
?>
  <form id="frmAdministradorSistemaCadastro" method="post" onsubmit="return OnSubmitForm();"
        action="<?=SessaoSip::getInstance()->assinarLink('administrador_sistema_cadastro.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    <?
    ...
    <input type="text" id="txtUsuario" name="txtUsuario" class="infraText"
           value="<?=PaginaSip::tratarHTML($objAdministradorSistemaDTO->getStrSiglaUsuario())?>" maxlength="100"
           tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$strDesabilitar?> />

    <input type="hidden" id="hdnIdUsuario" name="hdnIdUsuario"
           value="<?=$objAdministradorSistemaDTO->getNumIdUsuario();?>"/>
    <input type="hidden" id="hdnIdSistema" name="hdnIdSistema"
           value="<?=$objAdministradorSistemaDTO->getNumIdSistema();?>"/>
    <?
