...
try {
  ...
  if (isset($_GET['id_orgao_sistema']) && isset($_GET['id_sistema']) && isset($_GET['id_perfil'])) {
    ...
    PaginaSip::getInstance()->salvarCamposPost(array(
        'selOrgaoSistema', 'selSistema', 'selPerfil', 'selOrgaoUsuario', 'hdnIdUsuario', 'txtUsuario', 'hdnNomeUsuario'
      ));
  }
...
?>
  <form id="frmCoordenadorPerfilLista" method="post" action="<?=SessaoSip::getInstance()->assinarLink('coordenador_perfil_lista.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    ...
    <label id="lblUsuario" for="txtUsuario" accesskey="u" class="infraLabelOpcional"><span
        class="infraTeclaAtalho">U</span>suário:</label>
    <input type="text" id="txtUsuario" name="txtUsuario" class="infraText" value="<?=$strSiglaUsuario?>"
           maxlength="100" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$strDesabilitar?> />
