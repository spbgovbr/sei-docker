    ...
    try {
      ...
      if ($objLoginDTO != null) {
        ...
        if (InfraData::compararDataHora($dthLimite, InfraData::getStrDataHoraAtual()) > 0) {
          throw new InfraException('Tempo limite para validação do login esgotado.', null, 'Sistema: ' . $objLoginDTO->getStrSiglaSistema() . "\n" . 'Usuário: ' . $objLoginDTO->getStrSiglaUsuario(), false);
        }
  ...
  private function validar2FatoresObrigatorio(LoginDTO $objLoginDTO) {
    try {
      ...
      if ($objSistemaDTO->getStrSta2Fatores() == SistemaRN::$T2E_OBRIGATORIA) {
        $objInfraException->lancarValidacao('O sistema ' . $objLoginDTO->getStrSiglaSistema() . ' requer o uso da autenticação em dois fatores.');
      }
      ...
      if (count($arrObjPermissaoDTO)) {
        $objInfraException->lancarValidacao('As permissões do usuário no sistema ' . $objLoginDTO->getStrSiglaSistema() . ' requerem o uso da autenticação em dois fatores.');
      }
    } catch (Exception $e) {
      throw new InfraException('Erro validando obrigatoriedade do uso da autenticação em dois fatores.', $e);
    }
  }
}
