...
class AcessoRN extends InfraRN {
  ...
  protected function obterAcessosConectado(AcessoDTO $objAcessoDTO) {
    try {
      ...
      $objPermissaoDTOCriterios->adicionarCriterio(array('DataFim', 'DataFim'), array(InfraDTO::$OPER_MAIOR_IGUAL, InfraDTO::$OPER_IGUAL), array(InfraData::getStrDataAtual(), null), InfraDTO::$OPER_LOGICO_OR);

      if (($objAcessoDTO->getNumTipo() & AcessoDTO::$ADMINISTRADOR) or ($objAcessoDTO->getNumTipo() & AcessoDTO::$TODOS)) {
      ...
      if (($objAcessoDTO->getNumTipo() & AcessoDTO::$COORDENADOR_PERFIL) or ($objAcessoDTO->getNumTipo() & AcessoDTO::$TODOS)) {
      ...
      if (($objAcessoDTO->getNumTipo() & AcessoDTO::$COORDENADOR_UNIDADE) or ($objAcessoDTO->getNumTipo() & AcessoDTO::$TODOS)) {
        ...
        if (count($arrObjCoordenadorUnidadeDTO) > 0) {
          ...
          foreach ($arrObjCoordenadorUnidadeDTO as $objCoordenadorUnidadeDTO) {
            if (isset($arrObjPerfisCoordenados[$objCoordenadorUnidadeDTO->getNumIdSistema()])) {
              foreach (
                $arrObjPerfisCoordenados[$objCoordenadorUnidadeDTO->getNumIdSistema()] as $objPerfilDTO
              ) {
      ...
      if (($objAcessoDTO->getNumTipo() & AcessoDTO::$PERMISSAO) or ($objAcessoDTO->getNumTipo() & AcessoDTO::$TODOS)) {
