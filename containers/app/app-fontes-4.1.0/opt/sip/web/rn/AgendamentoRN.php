...
class AgendamentoRN extends InfraRN {
  ...
  public function testarAgendamento() {
    try {
      ...
      InfraMail::enviarConfigurado(ConfiguracaoSip::getInstance(), $objInfraParametro->getValor('SIP_EMAIL_SISTEMA'), $objInfraParametro->getValor('SIP_EMAIL_ADMINISTRADOR'), null, null, 'Teste Agendamento SIP',
        'Agendamento SIP executado com sucesso.');
    } catch (Throwable $e) {
  ...
  private function obterTextoLog(Exception $e) {
    if ($e instanceof InfraException) {
      if ($e->contemValidacoes()) {
        ...
        if ($e->getObjException() instanceof SoapFault && InfraException::getTipoInfraException($e->getObjException()) == 'INFRA_VALIDACAO') {
