...
?>
  <form id="frmTipoPermissaoCadastro" method="post" onsubmit="return OnSubmitForm();"
        action="<?=SessaoSip::getInstance()->assinarLink('tipo_permissao_cadastro.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    <?
    ...
    <input type="text" id="txtDescricao" name="txtDescricao" class="infraText"
           value="<?=PaginaSip::tratarHTML($objTipoPermissaoDTO->getStrDescricao());?>" maxlength="50"
           tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>

    <input type="hidden" id="hdnIdTipoPermissao" name="hdnIdTipoPermissao"
           value="<?=$objTipoPermissaoDTO->getNumIdTipoPermissao();?>"/>
    <?
