...
?>
  <form id="frmPerfilClonar" method="post" onsubmit="return OnSubmitForm();"
        action="<?=SessaoSip::getInstance()->assinarLink('perfil_clonar.php?acao=' . $_GET['acao'] . '&acao_origem=' . $_GET['acao'])?>">
    <?
    ...
    <label id="lblPerfilOrigem" for="selPerfilOrigem" accesskey="O" class="infraLabelObrigatorio">Perfil <span
        class="infraTeclaAtalho">O</span>rigem:</label>
    <select id="selPerfilOrigem" name="selPerfilOrigem" onchange="this.form.submit();" class="infraSelect"
            tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>">
      <?=$strItensSelPerfilOrigem?>
    </select>

    <label id="lblPerfilDestino" for="txtPerfilDestino" accesskey="D" class="infraLabelObrigatorio">Perfil <span
        class="infraTeclaAtalho">D</span>estino:</label>
    <input type="text" id="txtPerfilDestino" name="txtPerfilDestino" class="infraText"
           value="<?=PaginaSip::tratarHTML($objClonarPerfilDTO->getStrPerfilDestino());?>" maxlength="50"
           tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>"/>
