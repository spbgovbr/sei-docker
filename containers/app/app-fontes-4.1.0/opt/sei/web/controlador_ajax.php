...
try{
  ...
  switch($_GET['acao_ajax']){
    ...
    case 'texto_padrao_auto_completar':
      $arrObjTextoPadraoInternoDTO=TextoPadraoInternoINT::autoCompletarNome(utf8_decode(urldecode($_POST['palavras_pesquisa'])));
      $xml = InfraAjax::gerarXMLItensArrInfraDTO($arrObjTextoPadraoInternoDTO,'IdTextoPadraoInterno', 'Nome');
      break;

    case 'texto_padrao_editor_registrar':
      $objDocumentoGeracaoDTO = new DocumentoGeracaoDTO();
      $objDocumentoGeracaoDTO->setDblIdDocumento($_POST['idDOcumento']);
      $objDocumentoGeracaoDTO->setNumIdTextoPadraoInterno($_POST['idTextoPadrao']);

      $objDocumentoGeracaoRN = new DocumentoGeracaoRN();
      $objDocumentoGeracaoRN->registrarTextoPadrao($objDocumentoGeracaoDTO);
      break;
   ...
   case 'uf_montar_select_sigla':
    ...
    case 'assinatura_verificar_confirmacao':
      ...
      $xml = InfraAjax::gerarXMLComplementosArray(array('assinaturaConfirmada' => ($objAssinaturaDTO!=null && $objAssinaturaDTO->getNumIdAtividade()!=null?'S':'N')));
      break;
   ...
   case 'assunto_auto_completar_RI1223':
    ...
    case 'serie_dados':
      ...
      $objSerieDTO->setNumIdSerie($_POST['id_serie']);

      $objSerieRN = new SerieRN();
      $objSerieDTO = $objSerieRN->consultarRN0644($objSerieDTO);

      if ($objSerieDTO!=null){
        $arr = array('IdSerie' => $objSerieDTO->getNumIdSerie(),
          'Nome' => $objSerieDTO->getStrNome(),
          'StaAplicabilidade' => $objSerieDTO->getStrStaAplicabilidade(),
          'StaNumeracao' => $objSerieDTO->getStrStaNumeracao());
   ...
   case 'contato_auto_completar_contexto_substituicao':
    ...
    case 'contato_auto_completar_associado':
      $arrObjContatoDTO = ContatoINT::autoCompletarAssociado($_POST['palavras_pesquisa'],$_POST['id_tipo_contato']);
      $xml = InfraAjax::gerarXMLItensArrInfraDTO($arrObjContatoDTO,'IdContato', 'Nome');
      break;
   ...
   case 'unidade_auto_completar_envio_processo':
      $arrObjUnidadeDTO = UnidadeINT::autoCompletarEnvioProcesso($_POST['palavras_pesquisa'],$_POST['id_orgao'],$_POST['unidade_atual']);
      $xml = InfraAjax::gerarXMLItensArrInfraDTO($arrObjUnidadeDTO,'IdUnidade', 'Sigla');
      break;
   ...
   case 'upload_buscar':
    ...
    case 'relatorio_atividade_unidade_usuario':
      $strOptions = UsuarioINT::montarSelectPorUnidadeRI0811('null',' ',null,$_POST['idUnidade']);
      $xml = InfraAjax::gerarXMLSelect($strOptions);
      break;

    case 'etapa_trabalho_salvar_visualizacao':
      SessaoSEI::getInstance()->setAtributo('ETAPA_TRABALHO_VISUALIZACAO_' . $_POST['id_plano_trabalho'], $_POST['etapas_trabalho']);
      $xml = InfraAjax::gerarXMLComplementosArray(array());
      break;
