    ...
    case 'procedimento_gerar_zip':
      ...
      if ($_POST['hdnFlagGerar']=='1'){
        try{
          ...
          $objAnexoDTO = $objDocumentoRN->gerarZip($arrObjDocumentoDTO);
          $strNomeDownload = 'SEI-'.$objProcedimentoDTO->getStrProtocoloProcedimentoFormatado().'.zip';
...
?>
  ...
  function inicializar(){
    ...
    objLupaDocumentosExceto	= new infraLupaSelect('selDocumentosExceto','hdnDocumentosExceto','<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao=documento_selecionar&tipo_selecao=2&id_object=objLupaDocumentosExceto&id_procedimento='.$_GET['id_procedimento'].'&tipo_selecao_documento='.DocumentoINT::$TSD_ZIP)?>');
    objLupaDocumentosApenas	= new infraLupaSelect('selDocumentosApenas','hdnDocumentosApenas','<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao=documento_selecionar&tipo_selecao=2&id_object=objLupaDocumentosApenas&id_procedimento='.$_GET['id_procedimento'].'&tipo_selecao_documento='.DocumentoINT::$TSD_ZIP)?>');
  ...
  function gerar() {
    ...
    document.getElementById('frmProcedimentoZip').submit();
  }
...
?>
  ...
  <form id="frmProcedimentoZip" method="post" onsubmit="return OnSubmitForm();" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'].'&acao_retorno='.PaginaSEI::getInstance()->getAcaoRetorno())?>">
    <?
