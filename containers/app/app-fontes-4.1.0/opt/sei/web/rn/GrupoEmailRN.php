...
class GrupoEmailRN extends InfraRN {
  ...
  protected function listarInstitucionaisConectado(GrupoEmailDTO $parObjGrupoEmailDTO) {
    try {

      //Valida Permissao
      SessaoSEI::getInstance()->validarAuditarPermissao('grupo_email_listar',__METHOD__,$parObjGrupoEmailDTO);
      ...
      $ret = $this->listar($parObjGrupoEmailDTO);

      $objWS = RH::getInstance()->getWebService('GruposEmail');

      if ($objWS != null){

        $retWS = $objWS->listarGrupos();

        if (is_array($retWS)){
          foreach($retWS as $item){
            $objGrupoEmailDTO = new GrupoEmailDTO();
            $objGrupoEmailDTO->setNumIdGrupoEmail('RH-'.$item['Id']);
            $objGrupoEmailDTO->setStrNome($item['Nome']);
            $objGrupoEmailDTO->setStrDescricao(null);
            $ret[] = $objGrupoEmailDTO;
          }
        }
      }

      InfraArray::ordenarArrInfraDTO($ret, 'Nome', InfraArray::$TIPO_ORDENACAO_ASC);

      return $ret;

    }catch(Exception $e){
      throw new InfraException('Erro listando Grupos de E-mail para seleção.',$e);
    }
  }
