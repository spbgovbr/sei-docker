  ...
  private function obterServico(InstalacaoFederacaoDTO $parObjInstalacaoFederacaoDTO){
    try {
      ...
      if (strpos(strtoupper($e->__toString()),'CERTIFICATE VERIFY FAILED')!==false){
        $objInfraException->lancarValidacao('Não foi possível validar o certificado da instalação'.$strIdentificacao.'.');
      }

      if (strpos(strtoupper($e->__toString()),'FAILED TO LOAD EXTERNAL ENTITY')!==false){
        $objInfraException->lancarValidacao('Problema no certificado ou bloqueio no acesso ao serviço da instalação'.$strIdentificacao.'.');
      }

      if (strpos(strtoupper($e->__toString()),'COULDN\'T LOAD FROM')!==false){
        $objInfraException->lancarValidacao('Não foi possível carregar o serviço no endereço da instalação'.$strIdentificacao.'.');
      }

      if (strpos(strtoupper($e->__toString()),'FAILED TO OPEN STREAM')!==false){
        $objInfraException->lancarValidacao('Não foi possível acessar o serviço no endereço da instalação'.$strIdentificacao.'.');
      }

      throw new InfraException('Falha na conexão com a Instalação'.$strIdentificacao.'.', $e);
    }
  }
    ...
    try {
      ...
      if ($dthTimeStamp_Recebido === false) {
        ...
        if ($numSegundosDiferenca > $numSegundosSincronizacao) {
          throw new InfraException('Dados de criptografia expirados para a instalação '.$strSiglaInstalacao.' ('.InfraUtil::formatarCnpj($dblCnpjInstalacao).').');
        }
      }

    }catch(Exception $e){
      throw new InfraException('Erro nos dados de criptografia para a instalação '.$strSiglaInstalacao.' ('.InfraUtil::formatarCnpj($dblCnpjInstalacao).').', $e);
    }
  }
  ...
  public function executar($func, $strIdInstalacaoFederacao, ...$params) {
    try {
      ...
      if (strpos(strtoupper($e->__toString()),'HANDSHAKE TIMED OUT')!==false){
        throw new InfraException('Tempo esgotado para resposta do serviço solicitado na instalação '.$objInstalacaoFederacaoDTO->getStrSigla().'.', $e);
      }
    ...
    try {
      ...
      if ($objProcedimentoDTO->getStrIdProtocoloFederacaoProtocolo()!=null) {
        ...
        if (count($arrObjAcessoFederacaoDTO)) {
          ...
          //remove orgao origem NO FEDERAÇÃO da lista para deixar no topo
          ...
          //remove sinalizações de instalações sem acesso
