  ...
  public function enviar(EmailDTO $objEmailDTO) {
    try{
      ...
      try {
        ...
        if (strpos(strtoupper($e->__toString()), strtoupper('Não foi possível enviar para o(s) destinatário(s):'))!==false) {
      ...
      foreach ($arrStrIds as $strIdDocumento) {
        ...
        if (!isset($arrObjDocumentoDTO[$strIdDocumento])) {
          ...
          if ($objDocumentoDTO2->getDblIdProcedimento() != $objEmailDTO->getDblIdProtocolo()) {
            throw new InfraException('Documento '.$objDocumentoDTO2->getStrProtocoloDocumentoFormatado().' não pertence ao processo.');
          }

          $objInfraException->adicionarValidacao('Documento '.$objDocumentoDTO2->getStrProtocoloDocumentoFormatado().' não pode ser enviado por e-mail.');
        }
      }
