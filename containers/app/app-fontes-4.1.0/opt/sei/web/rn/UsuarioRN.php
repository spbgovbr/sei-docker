...
class UsuarioRN extends InfraRN {
  ...
  protected function excluirRN0491Controlado($arrObjUsuarioDTO){
    try {
      ...
      for ($i=0;$i<count($arrObjUsuarioDTO);$i++){
        ...
        if (($objAssinaturaDTORet = $objAssinaturaRN->consultarRN1322($objAssinaturaDTO))!=null){
          $objInfraException->lancarValidacao('Existem assinaturas associadas com o usuário '.$objAssinaturaDTORet->getStrSiglaUsuario().'.');
        }
  ...
  protected function pesquisarConectado(UsuarioDTO $objUsuarioDTO) {
    try {
      ...
      $objUsuarioDTO = InfraString::tratarPalavrasPesquisaDTO($objUsuarioDTO,"Sigla");
      $objUsuarioDTO = InfraString::tratarPalavrasPesquisaDTO($objUsuarioDTO,"Nome");
      $objUsuarioDTO = InfraString::tratarPalavrasPesquisaDTO($objUsuarioDTO,"NomeRegistroCivil");
      $objUsuarioDTO = InfraString::tratarPalavrasPesquisaDTO($objUsuarioDTO,"NomeSocial");
  ...
  protected function desativarRN0695Controlado($arrObjUsuarioDTO){
    try {
      ...
      */
      ...
      foreach($SEI_MODULOS as $seiModulo){
        $seiModulo->executar('desativarUsuario', $arrObjUsuarioAPI);
      }
  ...
  protected function reativarRN0696Controlado($arrObjUsuarioDTO){
    try {
      ...
      if (count($arrObjUsuarioDTO)) {
        ...
        foreach($SEI_MODULOS as $seiModulo){
          $seiModulo->executar('reativarUsuario', $arrObjUsuarioAPI);
        }
      }
    ...
    try {
      ...
      $objInfraException = new InfraException();  
        ...
        if ($objEmailSistemaDTO != null) {
          ...
          $strConteudo = str_replace('@email_usuario_externo@', $objUsuarioDTO->getStrSigla(), $strConteudo);
          $strConteudo = str_replace('@link_login_usuario_externo@', ConfiguracaoSEI::getInstance()->getValor('SEI', 'URL') . '/controlador_externo.php?acao=usuario_externo_logar&id_orgao_acesso_externo=' . $objUsuarioDTO->getNumIdOrgao(),
            $strConteudo);
    ...
    if ($objUsuarioDTO!=null) {
      ...
      if ($objUsuarioDTO->getStrStaTipo() == UsuarioRN::$TU_SIP) {
        ...
        if ($objUsuarioDTO->getStrIdOrigem() != null) {
          ...
          $objWS = RH::getInstance()->getWebService('CargoFuncao');
