...
class AgendamentoRN extends InfraRN {
  ...
  public function reabrirProcessos(){
    try{
      ...
      $bolSessaoHabilitada = SessaoSEI::getInstance()->isBolHabilitada();

      if ($bolSessaoHabilitada) {
        SessaoSEI::getInstance()->setBolHabilitada(false);
      }

      $objReaberturaProgramadaRN = new ReaberturaProgramadaRN();
      $objReaberturaProgramadaRN->processarAgendamento();

      //LogSEI::getInstance()->gravar('Agendamento Reabertura Processos: '.InfraDebug::getInstance()->getStrDebug());
