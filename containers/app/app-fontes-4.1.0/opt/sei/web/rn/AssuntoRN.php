...
class AssuntoRN extends InfraRN {
  ...
  protected function excluirRN0248Controlado($arrObjAssuntoDTO){
    try {
      ...
      $objRelTipoProcedimentoAssuntoRN = new RelTipoProcedimentoAssuntoRN();
      $objRelSerieAssuntoRN = new RelSerieAssuntoRN();
      $objAvaliacaoDocumental = new AvaliacaoDocumentalRN();
      ...
      for ($i=0;$i<count($arrObjAssuntoDTO);$i++){
        ...
        $dto = new AvaliacaoDocumentalDTO();
        $dto->retNumIdAvaliacaoDocumental();
        $dto->setNumIdAssuntoOriginal($arrObjAssuntoDTO[$i]->getNumIdAssunto());
        $dto->setNumMaxRegistrosRetorno(1);
        if ($objAvaliacaoDocumental->consultar($dto)!=null){
          $objInfraException->adicionarValidacao('Assunto '.$strCodigoEstruturado.' foi utilizado em avaliação documental.');
        }
