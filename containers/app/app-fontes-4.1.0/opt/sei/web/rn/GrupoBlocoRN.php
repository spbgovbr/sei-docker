...
class GrupoBlocoRN extends InfraRN {
  ...
  protected function excluirControlado($arrObjGrupoBlocoDTO){
    try {
      ...
      foreach($arrObjGrupoBlocoDTO as $objGrupoBlocoDTO){
        ...
        $objGrupoBlocoDTOBanco->retNumIdUnidade();
        ...
        if ($objGrupoBlocoDTOBanco->getNumIdUnidade() != SessaoSEI::getInstance()->getNumIdUnidadeAtual()) {
          $objInfraException->adicionarValidacao('Grupo de Blocos "'.$objGrupoBlocoDTOBanco->getStrNome().'" não pertence à unidade '.SessaoSEI::getInstance()->getStrSiglaUnidadeAtual().'.');
        }
      }
  ...
  protected function listarUnidadeConectado(GrupoBlocoDTO $objGrupoBlocoDTO) {
    try {
      ...
      $objGrupoBlocoBD = new GrupoBlocoBD($this->getObjInfraIBanco());
      $ret = $objGrupoBlocoBD->listarUnidade($objGrupoBlocoDTO);
