 ...
 */
    ...
    case 'tarefa_configurar_historico':
      ...
      if ($_POST['hdnCompletosItens']!=''){
        ...
        foreach($arrItens as $item){
          ...
          $objTarefaDTO->setStrSinConsultaProcessual('N');
          $arrObjTarefaDTO[] = $objTarefaDTO;
        }
        ...
        $arrConsultaProcessual = PaginaSEI::getInstance()->getArrStrItensSelecionados('ConsultaProcessual');
        foreach($arrConsultaProcessual as $consultaProcessual){
          foreach($arrObjTarefaDTO as $objTarefaDTO){
            if ($objTarefaDTO->getNumIdTarefa()==$consultaProcessual){
              $objTarefaDTO->setStrSinConsultaProcessual('S');
            }
          }
        }
        try{
  ...
  $objTarefaDTO->retStrSinConsultaProcessual();
  ...
  if ($numRegistros > 0){
    ...
    $strResultado .= '<th class="infraTh" width="20%">'.PaginaSEI::getInstance()->getThCheck('Consulta&nbsp;Processual externa','ConsultaProcessual').'</th>'."\n";
    $strResultado .= '<th class="infraTh">'.PaginaSEI::getInstance()->getThOrdenacao($objTarefaDTO,'Nome','Nome',$arrObjTarefaDTO).'</th>'."\n";
    $strResultado .= '<th class="infraTh">'.PaginaSEI::getInstance()->getThOrdenacao($objTarefaDTO,'ID','IdTarefa',$arrObjTarefaDTO).'</th>'."\n";
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      $strResultado .= '<td>'.PaginaSEI::getInstance()->getTrCheck($i,$arrObjTarefaDTO[$i]->getNumIdTarefa(),$arrObjTarefaDTO[$i]->getStrNome(),$arrObjTarefaDTO[$i]->getStrSinConsultaProcessual(),'ConsultaProcessual').'</td>';

      if ($arrObjTarefaDTO[$i]->getNumIdTarefa()==TarefaRN::$TI_ATUALIZACAO_ANDAMENTO){
        $strResultado .= '<td>'.$arrObjTarefaDTO[$i]->getStrNome().' (atualização de andamento)</td>';
      }else{
        $strResultado .= '<td>'.$arrObjTarefaDTO[$i]->getStrNome().'</td>';
      }


      $strResultado .= '<td align="center">'.$arrObjTarefaDTO[$i]->getNumIdTarefa().'</td>';
