  ...
  if ($numRegistros > 0){
    ...
    */
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td valign="center">'.PaginaSEI::getInstance()->getTrCheck($i,$arrObjModeloDTO[$i]->getNumIdModelo(),$arrObjModeloDTO[$i]->getStrNome()).'</td>';
      }
      ...
      if($bolAcaoEditorSimular){
        $strResultado .= '<a onclick="infraLimparFormatarTrAcessada(this.parentNode.parentNode);infraAbrirJanelaModal(\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=editor_simular&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_modelo='.$arrObjModeloDTO[$i]->getNumIdModelo()).'\',800,600);" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.Icone::PRE_VISUALIZAR.'" title="Visualizar Modelo" alt="Visualizar Modelo" class="infraImg" /></a>&nbsp;';
      }
