    ...
    var msoListsDetected = Boolean(html.match(/mso-list:\s*l\d+\s+level\d+\s+lfo\d+/)),
        ...
        rules = {
          ...
          attributes: {
            ...
            // 'align': remove,
            'width': function(styles, element){
              return Style.mapCommonStyles( element ) || false;
            },
            'height': remove,
            ...
            'class': function (classes, element) {
              if (element.name!=='p') return false;
              var vs = CKEDITOR.currentInstance.config.stylesheetParser_validSelectors;
              var reg = new RegExp(vs);
              if (!reg.test(element.name + '.' + classes)) return false;
    ...
    numbering: {
      ...
      toNumber: function (marker, markerType) {
        ...
        if (markerType=='decimal') {
          ...
          if (markerType=='upper-roman' || markerType=='lower-roman') {
            return fromRoman(marker.toUpperCase());
          } else
            if (markerType=='lower-alpha' || markerType=='upper-alpha') {
