...
try {
  ...
  if ($objEstatisticasInspecaoDTORet!=null){
    ...
    $strCssTr='';
    $totalTiposTramitacao = 0;
    $totalGeral = 0;
    $strTabelaTiposTramitacao = '';

    if ($objEstatisticasInspecaoDTO->getStrStaTipo()==EstatisticasRN::$TIPO_INSPECAO_TIPOS_TRAMITACAO){
      foreach ($objEstatisticasInspecaoDTORet->getArrTiposProcessosTramitacao() as $keyOrgaoTipoProcedimento => $numRegistros){
        ...
        $strCssTr = ($strCssTr=='<tr class="infraTrClara">')?'<tr class="infraTrEscura">':'<tr class="infraTrClara">';
        $strTabelaTiposTramitacao .= $strCssTr;
        $strTabelaTiposTramitacao .= '<td align="center"><a alt="'.PaginaSEI::tratarHTML($strDescricaoOrgao).'" title="'.PaginaSEI::tratarHTML($strDescricaoOrgao).'" class="ancoraSigla">'.PaginaSEI::tratarHTML($strSiglaOrgao).'</a></td>';
        $strTabelaTiposTramitacao .= '<td align="center"><a alt="'.PaginaSEI::tratarHTML($strNomeTipoProcedimento).'" title="'.PaginaSEI::tratarHTML($strNomeTipoProcedimento).'" class="ancoraSigla">'.PaginaSEI::tratarHTML($strNomeTipoProcedimento).'</a></td>';
        //$strTabelaTiposTramitacao .= '<td align="center"><a href="javascript:void(0);" onclick="abrirDetalhe(\''.$strLink.'\');" class="linkFuncionalidade">'.$numRegistros.'</a></td>';
        $strTabelaTiposTramitacao .= '<td align="center">'.InfraUtil::formatarMilhares($numRegistros).'</td>';
        $strTabelaTiposTramitacao .= '</tr>'."\n";

        $arrayGraficoTramitacaoTipoProcedimento[] = array($strNomeTipoProcedimento.' / '.$strSiglaOrgao, InfraUtil::formatarMilhares($numRegistros),$numRegistros);

        $totalGeral += $numRegistros;

        $totalTiposTramitacao++;
        //}
      }

      $strTabelaTiposTramitacaoCompleta = '';
      $strTabelaTiposTramitacaoCompleta .= '<table width="70%" class="infraTable" summary="Tabela de '.EstatisticasRN::$TITULO_INSPECAO_TIPOS_TRAMITACAO.'">'."\n";
      $strTabelaTiposTramitacaoCompleta .= '<caption class="infraCaption">'.EstatisticasRN::$TITULO_INSPECAO_TIPOS_TRAMITACAO.':</caption>';
      $strTabelaTiposTramitacaoCompleta .= '<tr>';
      $strTabelaTiposTramitacaoCompleta .= '<th class="infraTh" width="25%">'.PaginaSEI::getInstance()->getThOrdenacao($objEstatisticasInspecaoDTO,'Órgão','SiglaOrgao',$arrTemp).'</th>'."\n";
      $strTabelaTiposTramitacaoCompleta .= '<th class="infraTh" width="">'.PaginaSEI::getInstance()->getThOrdenacao($objEstatisticasInspecaoDTO,'Tipo de Processo','NomeTipoProcedimento',$arrTemp).'</th>'."\n";
      $strTabelaTiposTramitacaoCompleta .= '<th class="infraTh" width="15%">'.PaginaSEI::getInstance()->getThOrdenacao($objEstatisticasInspecaoDTO,'Quantidade','Quantidade',$arrTemp).'</th>'."\n";
      $strTabelaTiposTramitacaoCompleta .= '</tr>'."\n";
      $strTabelaTiposTramitacaoCompleta .= $strTabelaTiposTramitacao;
      if ($totalTiposTramitacao > 1){
        $strTabelaTiposTramitacaoCompleta .= '<tr class="totalEstatisticas"><td align="right" colspan="2"><b>TOTAL:</b></td><td align="center">'.InfraUtil::formatarMilhares($totalGeral).'</td></tr>';
      }
      $strTabelaTiposTramitacaoCompleta .= '</table>';


      $strResultadoGraficoTiposTramitacao = '';
      if ($objEstatisticasInspecaoDTO->getNumIdTipoProcedimento()==null && $totalTiposTramitacao > 1){
        $strResultadoGraficoTiposTramitacao .= '<div id="divGrf'.(++$numGrafico).'" class="divAreaGrafico">';
        $strResultadoGraficoTiposTramitacao .= $objEstatisticasRN->gerarGraficoBarrasSimples($numGrafico,EstatisticasRN::$TITULO_ESTATISTICAS_TRAMITACAO,null,$arrayGraficoTramitacaoTipoProcedimento, 300, $arrCores[0]);
        $strResultadoGraficoTiposTramitacao .= '</div><br />';
...
function alterarTipo(bolLimpar){
  ...
  }else if (tipo == '<?=EstatisticasRN::$TIPO_INSPECAO_TIPOS_TRAMITACAO?>'){
...
<form id="frmInspecaoAdministrativa"  method="post" onsubmit="return onSubmitForm();" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  <div id="divGeral" class="infraAreaDados" style="height:10em;">
    ...
    <select id="selTipo" name="selTipo" onchange="alterarTipo(true)" class="infraSelect" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>">
    <?=$strItensSelTipo?>
    </select>
  </div>
  ...
  if ($objEstatisticasInspecaoDTO->getStrStaTipo()!=EstatisticasRN::$TIPO_INSPECAO_MOVIMENTACAO){
    ...
    if ($totalTiposTramitacao > 0) {
      PaginaSEI::getInstance()->montarAreaTabela($strTabelaTiposTramitacaoCompleta,$totalTiposTramitacao);
      if ($strResultadoGraficoTiposTramitacao != '') {
        EstatisticasINT::montarGrafico('TiposTramitacao',$strResultadoGraficoTiposTramitacao);
      }
    }
