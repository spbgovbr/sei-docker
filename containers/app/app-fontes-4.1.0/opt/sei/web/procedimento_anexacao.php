...
function inicializar(){	
  ...
  <?if (($_GET['acao_origem']=='procedimento_anexar' || $_GET['acao_origem']=='procedimento_desanexar') && $_GET['arvore']=='1') { ?>
...
<form id="frmProcedimentoAnexar" method="post" onsubmit="return OnSubmitForm();"  action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'].$strParametros)?>">
  ...
  <button type="button" accesskey="P" name="btnPesquisar" id="btnPesquisar" onclick="pesquisar();" value="Pesquisar" class="infraButton" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>"><span class="infraTeclaAtalho">P</span>esquisar</button>
