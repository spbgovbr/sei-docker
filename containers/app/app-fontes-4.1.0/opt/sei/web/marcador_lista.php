...
try {
  ...
  PaginaSEI::getInstance()->salvarCamposPost(array('hdnMarcadoresFiltro'));
  ...
  $strFiltroMarcadores = PaginaSEI::getInstance()->recuperarCampo('hdnMarcadoresFiltro','S');
  $objMarcadorDTO->setStrSinAtivo($strFiltroMarcadores);
  ...
  */
  ...
  if ($numRegistros > 0){
    ...
    if ($_GET['acao']=='marcador_selecionar'){
      ...
      $bolAcaoReativar = ($strFiltroMarcadores == 'N') && SessaoSEI::getInstance()->verificarPermissao('marcador_reativar');
      $bolAcaoConsultar = false; //SessaoSEI::getInstance()->verificarPermissao('marcador_consultar');
      $bolAcaoAlterar = SessaoSEI::getInstance()->verificarPermissao('marcador_alterar');
      $bolAcaoImprimir = true;
      //$bolAcaoGerarPlanilha = SessaoSEI::getInstance()->verificarPermissao('infra_gerar_planilha_tabela');
      $bolAcaoExcluir = SessaoSEI::getInstance()->verificarPermissao('marcador_excluir');
      $bolAcaoDesativar = ($strFiltroMarcadores == 'S') && SessaoSEI::getInstance()->verificarPermissao('marcador_desativar');
    }
    ...
    */
    ...
    if ($objMarcadorDTO->getStrSinAtivo()=='S'){
...
function filtrar(sinAtivos){
  document.getElementById('hdnMarcadoresFiltro').value = sinAtivos;
  document.getElementById('frmMarcadorLista').action='<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao=marcador_listar&acao_origem='.$_GET['acao'])?>';
  document.getElementById('frmMarcadorLista').submit();
}
...
<form id="frmMarcadorLista" method="post" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  <div id="divFiltro" class="infraAreaDados">
    <br>
    <a  href="#" class="ancoraPadraoPreta" onclick="filtrar('<?=$strFiltroMarcadores=='S'?'N':'S'?>')" ><?=$strFiltroMarcadores == 'N' ? "Ver ativos" : "Ver inativos" ?></a>
  </div>
  <input type="hidden" id="hdnMarcadoresFiltro" name="hdnMarcadoresFiltro" value="<?=$strFiltroMarcadores?>" />
  <br>
  <?
