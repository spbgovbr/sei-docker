...
class PaginaSEI extends InfraPaginaEsquema3
{
  ...
  public function getStrLogoSistema(){
    $strRet = '<img src="svg/sei_barra.svg?'.$this->getNumVersao().'" title="Sistema Eletrônico de Informações - Versão ' . SEI_VERSAO . '"/>';
                 ...
                 <form class="form-inline align-self-center w-100" id="frmProtocoloPesquisaRapida" method="post" action="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=protocolo_pesquisa_rapida').'">
                  <div class="input-group">
                    ...
                    <span class="input-group-btn">
                      ...
                      <img src="svg/pesquisa_rapida.svg?'.$this->getNumVersao().'" width="20" height="20" onclick="document.getElementById(\'frmProtocoloPesquisaRapida\').submit();" title="Pesquisa Rápida" alt="Pesquisa Rápida" tabindex="' . $this->getProxTabBarraSistema() . '" class="infraImg" />
                      </span>
                    </span>
                  </div>
                 </form>
  ...
  public function montarBarraComandosSuperior($arrComandos) {
    array_unshift($arrComandos, '<a id="ancVoltarArvoreSuperior" href="javascript:seiVoltarArvoreProcesso()" class="btn" style="display:none;" title="Voltar para a Árvore do Processo" tabindex="'.InfraPagina::$TAB_INI_BARRA_COMANDOS_SUPERIOR.'">
  ...
  public function montarBarraComandosInferior($arrComandos, $bolForcarMontagem = false) {
    array_unshift($arrComandos, '<a id="ancVoltarArvoreInferior" href="javascript:seiVoltarArvoreProcesso()" class="btn" style="display:none;" title="Voltar para a Árvore do Processo" tabindex="'.InfraPagina::$TAB_INI_BARRA_COMANDOS_SUPERIOR.'">
          <img src="'.$this->getDiretorioSvgGlobal() .'/voltar.svg?'.$this->getNumVersao().'">
        </a>');
  ...
  public function getArrStrAcessibilidade(){
    ...
    $arr['Árvore de Processo'] = array(
      'CTRL + ALT + R' => 'posiciona no item raiz da árvore que representa o número do processo',
      'CTRL + ALT + S' => 'posiciona no item de protocolo selecionado atualmente na árvore',
      'CTRL + ALT + F' => 'posiciona na barra de funcionalidades associadas com o protocolo selecionado (usar TAB para navegar entre as funcionalidades disponíveis)',
      'CTRL + ALT + V' => 'posiciona na área de visualização de conteúdo associada com protocolo selecionado ou funcionalidade escolhida',
      'CTRL + ALT + U' => 'posiciona no último item de protocolo da árvore',
      'ALT + Seta acima' => 'seleciona o item anterior (pasta ou protocolo)',
      'ALT + Seta abaixo' => 'seleciona o próximo item (pasta ou protocolo)',
      'TAB' => 'navega entre todos os itens da árvore (pastas, protocolos e sinalizações)'
    );
    ...
    $arr['Editor'] = array(
      ...
      'CTRL + SHIFT + X' => 'Inserir o conteúdo de um texto padrão (AutoTexto)',
      'CTRL + B' => 'Negrito',
      'CTRL + I' => 'Itálico',
      'CTRL + U' => 'Sublinhado',
      'CTRL + X' => 'Recortar',
      'CTRL + C' => 'Copiar',
      'CTRL + V' => 'Colar',
      'CTRL + SHIFT + V' => 'Colar como texto sem formatação',
      'CTRL + Z' => 'Desfazer',
      'CTRL + Y' => 'Refazer',
      'ALT + 0' => 'Exibe instruções de acessibilidade do editor'
    );
