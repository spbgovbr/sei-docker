...
try {
  ...
  if ($numRegistros > 0){
    ...
    $strResultado .= '<th class="infraTh" width="5%">'.PaginaSEI::getInstance()->getThOrdenacao($objOrgaoDTO,'ID','IdOrgao',$arrObjOrgaoDTO).'</th>'."\n";
    $strResultado .= '<th class="infraTh" width="10%">'.PaginaSEI::getInstance()->getThOrdenacao($objOrgaoDTO,'Sigla','Sigla',$arrObjOrgaoDTO).'</th>'."\n";
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td>'.PaginaSEI::getInstance()->getTrCheck($i,$arrObjOrgaoDTO[$i]->getNumIdOrgao(),$arrObjOrgaoDTO[$i]->getStrSigla()).'</td>';
      }
