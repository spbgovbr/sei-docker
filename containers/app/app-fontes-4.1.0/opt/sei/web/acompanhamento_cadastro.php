...
try {
  ...
  switch($_GET['acao']){
    ...
    case 'acompanhamento_alterar':
      ...
      $arrComandos[] = '<button type="button" accesskey="C" name="btnCancelar" id="btnCancelar" value="Cancelar" onclick="location.href=\''.SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.PaginaSEI::getInstance()->getAcaoRetorno().'&acao_origem='.$_GET['acao'].PaginaSEI::getInstance()->montarAncora($objAcompanhamentoDTO->getNumIdAcompanhamento())).'\';" class="infraButton"><span class="infraTeclaAtalho">C</span>ancelar</button>';
...
<? if (SessaoSEI::getInstance()->verificarPermissao('grupo_acompanhamento_cadastrar')) { ?>
  function cadastrarGrupoAcompanhamento(){
    infraAbrirJanelaModal('<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao=grupo_acompanhamento_cadastrar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&pagina_simples=1')?>', 700, 300);
  }
<?}?>
...
<form id="frmAcompanhamentoCadastro" method="post" onsubmit="return OnSubmitForm();" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  if (SessaoSEI::getInstance()->verificarPermissao('grupo_acompanhamento_cadastrar')) {
    echo '<img id="imgNovoGrupoAcompanhamento" onclick="cadastrarGrupoAcompanhamento();" src="'.PaginaSEI::getInstance()->getIconeMais().'" alt="Novo Grupo de Acompanhamento" title="Novo Grupo de Acompanhamento" class="infraImg" tabindex="'.PaginaSEI::getInstance()->getProxTabDados().'"/>'."\n";
  }
  ?>
