...
try {
  ...
  switch($_GET['acao']){
    ...
    case 'documento_conferir':
      ...
      CaptchaSEI::getInstance()->configurarCaptcha('Validação de Documentos');
                ...
                foreach ($arrObjAssinaturaDTO as $objAssinaturaDTO) {
                  ...
                  if ($objDocumentoDTORet->getStrStaDocumento()!=DocumentoRN::$TD_EDITOR_EDOC && $numAssinaturasDigitais) {
                    ...
                    if ($objAssinaturaDTO->getStrStaFormaAutenticacao() == AssinaturaRN::$TA_CERTIFICADO_DIGITAL) {
                      $strResultado .= '<a href="' . 'controlador_externo.php?acao=' . $_GET['acao'] . '&codigo_verificador=' . $_POST['txtCodigoVerificador'] . '&codigo_crc=' . $_POST['txtCrc'] . '&id_assinatura=' . $objAssinaturaDTO->getNumIdAssinatura() . '&hash_download=' . $strHashDownload . '&id_orgao_acesso_externo=' . $_GET['id_orgao_acesso_externo'] . '" target="_blank"><img src="'.PaginaSEIExterna::getInstance()->getIconeDownload().'" title="'._('Download do arquivo PKCS #7 para validação da assinatura digital').'" alt="'._('Download do arquivo PKCS #7 para validação da assinatura digital').'" class="infraImg" /></a>&nbsp;';
                    }
                    $strResultado .= '</td>' . "\n";
                  }
                  $strResultado .= '</tr>' . "\n";
