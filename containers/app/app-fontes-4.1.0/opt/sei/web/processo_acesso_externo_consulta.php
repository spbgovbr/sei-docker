...
try {
  ...
  $objProcedimentoDTO->setArrObjParticipanteDTO($objInteressadosParticipanteDTO);
  ...
  */
  ...
  if ($numProtocolos) {
    $strCssMostrarAcoes = '.colunaAcoes {display:none !important;}' . "\n";
    ...
    foreach ($arrObjRelProtocoloProtocoloDTO as $objRelProtocoloProtocoloDTO) {
      ...
      if ($objRelProtocoloProtocoloDTO->getStrStaAssociacao() == RelProtocoloProtocoloRN::$TA_DOCUMENTO_ASSOCIADO) {
        ...
        $strLinkDocumento = '<a ';
        if ($objRelProtocoloProtocoloDTO->getStrSinAcessoBasico()=='S'){
          $strLinkDocumento .= ' class="ancoraPadraoAzul" onclick="infraLimparFormatarTrAcessada(this.parentNode.parentNode);" href="' . SessaoSEIExterna::getInstance()->assinarLink('documento_consulta_externa.php?'.$strParametro.'&id_documento=' . $objDocumentoDTO->getDblIdDocumento()) . '" target="_blank"';
        ...
        if ($objDocumentoDTO->getDblIdDocumento() == $objAcessoExternoDTO->getDblIdDocumento() && $objDocumentoDTO->getStrStaEstadoProtocolo() != ProtocoloRN::$TE_DOCUMENTO_CANCELADO) {
          ...
          if (!$bolFlagAssinou) {
            $strLinkAssinar = '<a href="javascript:void(0);" onclick="infraLimparFormatarTrAcessada(this.parentNode.parentNode);infraAbrirJanelaModal(\'' . SessaoSEIExterna::getInstance()->assinarLink('controlador_externo.php?'.$strParametro.'&acao=usuario_externo_assinar&id_documento=' . $objDocumentoDTO->getDblIdDocumento()) . '\',450,330);" tabindex="' . PaginaSEIExterna::getInstance()->getProxTabTabela() . '"><img src="'.Icone::DOCUMENTO_ASSINAR.'" title="Assinar Documento" alt="Assinar Documento" class="infraImg" /></a>&nbsp;';
            $strCssMostrarAcoes = '';
          }
        }
        ...
        if ($objRelProtocoloProtocoloDTO->getStrSinAcessoBasico()=='S'){
          $strLinkProcessoAnexado .= ' class="ancoraPadraoAzul" onclick="infraLimparFormatarTrAcessada(this.parentNode.parentNode);" href="' . SessaoSEIExterna::getInstance()->assinarLink('processo_acesso_externo_consulta.php?'.$strParametro.'&id_procedimento_anexado=' . $objProcedimentoDTOAnexado->getDblIdProcedimento()) . '" target="_blank"';
        }else{
  ...
  if ($numDocumentosCheck > 0 && $objProcedimentoDTO->getStrSinEliminadoProtocolo()!='S'){
