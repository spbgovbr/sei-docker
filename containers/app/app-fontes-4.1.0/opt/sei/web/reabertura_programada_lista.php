...
try {
  ...
  switch($_GET['acao']){
    ...
    case 'reabertura_programada_listar':
      $strTitulo = 'Reaberturas Programadas';
      break;
  ...
  $arrComandos = array();
  $arrComandos[] = '<button type="submit" accesskey="P" name="sbmPesquisar" id="sbmPesquisar" value="Pesquisar" class="infraButton"><span class="infraTeclaAtalho">P</span>esquisar</button>';
  ...
  $objReaberturaProgramadaDTO->setDtaInicio($dtaPeriodoDe);

  $dtaPeriodoA = $_POST['txtPeriodoA'];
  $objReaberturaProgramadaDTO->setDtaFim($dtaPeriodoA);

  $strProtocoloPesquisa = $_POST['txtProtocolo'];
  $objReaberturaProgramadaDTO->setStrProtocoloFormatadoProtocolo($strProtocoloPesquisa);
  ...
  if ($numRegistros > 0){
    ...
    */
    ...
    $strResultado .= '<th class="infraTh">'.PaginaSEI::getInstance()->getThOrdenacao($objReaberturaProgramadaDTO,'Processo','IdProtocolo',$arrObjReaberturaProgramadaDTO).'</th>'."\n";
    $strResultado .= '<th class="infraTh">Tipo</th>'."\n";
    $strResultado .= '<th class="infraTh">'.PaginaSEI::getInstance()->getThOrdenacao($objReaberturaProgramadaDTO, 'Data Programada', 'Programada', $arrObjReaberturaProgramadaDTO).'</th>'."\n";
    $strResultado .= '<th class="infraTh">'.PaginaSEI::getInstance()->getThOrdenacao($objReaberturaProgramadaDTO,'Usuário','SiglaUsuario',$arrObjReaberturaProgramadaDTO).'</th>'."\n";
    if ($strSinAgendadas=='N'){
      $strResultado .= '<th class="infraTh">'.PaginaSEI::getInstance()->getThOrdenacao($objReaberturaProgramadaDTO, 'Processamento', 'Processamento', $arrObjReaberturaProgramadaDTO).'</th>'."\n";
...
/* #frmReaberturaProgramadaLista{max-width: 1200px;} */

#lblPeriodoDe {position:absolute;left:0%;top:10%;width:14%;}
#txtPeriodoDe {position:absolute;left:12%;top:0%;width:9%;}
#imgCalPeriodoD {position:absolute;left:21.5%;top:5%;}

#lblPeriodoA 	{position:relative;left:24.5%;top:10%;width:9%;}
#txtPeriodoA 	{position:absolute;left:26%;top:0%;width:9%;}
#imgCalPeriodoA {position:absolute;left:35.5%;top:5%;}

#lblProtocolo {position:absolute;left:0%;top:10%;width:14%}
#txtProtocolo {position:absolute;left:12%;top:0%;width:23%;}

#ancFiltro {position:absolute;left:12%;top:0%;}
...
function acaoExcluir(id,processo,data){
  if (confirm("Confirma exclusão da Reabertura Programada do processo " + processo + " em " + data + "?")){
    document.getElementById('hdnInfraItemId').value=id;
    document.getElementById('frmReaberturaProgramadaLista').action='<?=$strLinkExcluir?>';
    document.getElementById('frmReaberturaProgramadaLista').submit();
  }
}
...
?>
<form id="frmReaberturaProgramadaLista" onsubmit="return validarFormulario();" method="post" action="<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  <?
  PaginaSEI::getInstance()->montarBarraComandosSuperior($arrComandos);
  PaginaSEI::getInstance()->abrirAreaDados('3em');
  ?>
  <label id="lblPeriodoDe" for="txtPeriodoDe" accesskey="" class="infraLabelOpcional">Data Programada:</label>
  ...
  PaginaSEI::getInstance()->abrirAreaDados('3em');
  ?>
  <label id="lblProtocolo" for="txtProtocolo" accesskey="" class="infraLabelOpcional">Processo:</label>
  <input type="text" id="txtProtocolo" name="txtProtocolo" class="infraText" value="<?=PaginaSEI::tratarHTML($strProtocoloPesquisa)?>" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>" />
  <?
  PaginaSEI::getInstance()->fecharAreaDados();
  PaginaSEI::getInstance()->abrirAreaDados('3em');
  ?>
  <a id="ancFiltro" href="#" class="ancoraPadraoPreta" onclick="filtrarReaberturas('<?=$strSinAgendadas=='S'?'N':'S'?>')" tabindex="<?=PaginaSEI::getInstance()->getProxTabDados()?>"><?=$strSinAgendadas == 'S' ? "Ver processadas" : "Ver agendadas" ?></a>
  <?
  PaginaSEI::getInstance()->fecharAreaDados();
