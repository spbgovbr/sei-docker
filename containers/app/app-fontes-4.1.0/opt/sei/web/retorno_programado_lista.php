  ...
  $strResultado = '';
    ...
    for ($i = 0; $i < $numRegistros; $i++) {
      ...
      if ($arrObjRetornoProgramadoDTO[$i]->getNumIdUnidadeEnvio() == SessaoSEI::getInstance()->getNumIdUnidadeAtual()) {
        ...
        $strCssTrAguardando = ($strCssTrAguardando=='<tr class="infraTrClara">')?'<tr class="infraTrEscura">':'<tr class="infraTrClara">';

        $strAguardando .= $strCssTrAguardando;
        ...
        if ($arrObjRetornoProgramadoDTO[$i]->getNumDiasPrazo() < 0) {
          $strAguardando .= ' class="tdVermelha"';
        }

        $strAguardando .= '>'.$arrObjRetornoProgramadoDTO[$i]->getNumDiasPrazo();
        $strAguardando .= '</td>';
        ...
        $strCssTrDevolver = ($strCssTrDevolver=='<tr class="infraTrClara">')?'<tr class="infraTrEscura">':'<tr class="infraTrClara">';

        $strDevolver .= $strCssTrDevolver;
        ...
        $strDevolver .= '<td align="center"';
        if ($arrObjRetornoProgramadoDTO[$i]->getNumDiasPrazo() < 0) {
          $strDevolver .= ' class="tdVermelha"';
        }

        $strDevolver .= '>'.$arrObjRetornoProgramadoDTO[$i]->getNumDiasPrazo().'</td>';
        $strDevolver .= '</tr>'."\n";
      }
    }
    ...
    if ($contadorAguardando) {
      $strResultado .= '<label class="textoSubtitulo">Processos aguardando retorno de outras unidades</label>'."\n".
          ...
          '<table width="100%" class="infraTable" summary="Tabela de processos aguardando retorno de outras unidades">'."\n".
    ...
    if ($contadorDevolver) {
      $strResultado .= '<label class="textoSubtitulo">Processos para devolver</label>'.
          '<hr class="linhaSubtitulo" />'.
          '<table width="100%" class="infraTable" summary="Tabela de processos para devolver">'."\n".
...
#divCalendario {float:left;border:2px solid #BBBBBB;width:16em;border-radius:.4rem;padding:.2em;}
...
hr.linhaSubtitulo{width:100%;color:var(--infra-esquema-cor-clara);border-bottom-color:unset;margin-top:.4em;}
