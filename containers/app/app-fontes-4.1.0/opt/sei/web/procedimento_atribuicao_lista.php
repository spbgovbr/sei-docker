...
try {
  ...
  if ($numRegistros > 0){
    ...
    $strResultado .= '<table width="99%" id="tblAtribuicoes" class="infraTable" summary="'.$strSumarioTabela.'">'."\n"; //80
    $strResultado .= '<caption class="infraCaption">'.PaginaSEI::getInstance()->gerarCaptionTabela($strCaptionTabela,$numRegistros).'</caption>';
    $strResultado .= '<tr>';
    $strResultado .= '<th class="infraTh" width="1%">'.PaginaSEI::getInstance()->getThCheck('','Infra','',$numTabIndexAtribuicoes).'</th>'."\n";
    $strResultado .= '<th class="infraTh" width="10%">&nbsp;</th>'."\n";
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      $strResultado .= '<td align="center"> <a href="'.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=procedimento_trabalhar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_procedimento='.$arrObjProcedimentoDTO[$i]->getDblIdProcedimento()).'" target="_blank" tabindex="'.$numTabIndexAtribuicoes.'" alt="'.PaginaSEI::tratarHTML($arrObjProcedimentoDTO[$i]->getStrNomeTipoProcedimento()).'" title="'.PaginaSEI::tratarHTML($arrObjProcedimentoDTO[$i]->getStrNomeTipoProcedimento()).'" class="protocoloNormal" style="font-size:1em !important;">'.PaginaSEI::tratarHTML($arrObjProcedimentoDTO[$i]->getStrProtocoloProcedimentoFormatado()).'</a></td>'."\n";
      $strResultado .= '<td align="left">'.PaginaSEI::tratarHTML($arrObjProcedimentoDTO[$i]->getStrNomeTipoProcedimento()).'</td>'."\n";
      
      /*
      $strResultado .= '<td align="center">';
      if ($bolAcaoAlterar){
        $strResultado .= '<a href="#ID-'.$arrObjProcedimentoDTO[$i]->getDblIdProcedimento().'"  onclick="acaoAlterar(\''.$arrObjProcedimentoDTO[$i]->getDblIdProcedimento().'\',\''.$arrObjProcedimentoDTO[$i]->getStrProtocoloProcedimentoFormatado().'\');" tabindex="'.$numTabIndexAtribuicoes.'"><img src="'.PaginaSEI::getInstance()->getIconeAlterar().'" title="Alterar Atribuição" alt="Alterar Atribuição" class="infraImg" /></a>&nbsp;';
      }
      $strResultado .= '</td>'."\n";
      */
...
function inicializar(){
  ...
  seiConfigurarTabIndexSinalizacoes('tblAtribuicoes','<?=$numTabIndexAtribuicoes?>');
