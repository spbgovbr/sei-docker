...
?>
<form id="frmRecursoLista" method="post" onsubmit="return OnSubmitForm();" action="<?=SessaoSip::getInstance()->assinarLink('hierarquia_clonar.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  <?
  ...
  <label id="lblHierarquiaOrigem" for="selHierarquiaOrigem" accesskey="H" class="infraLabelObrigatorio"><span class="infraTeclaAtalho">H</span>ierarquia Origem:</label>
  <select id="selHierarquiaOrigem" name="selHierarquiaOrigem" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>">
  ...
  <input type="text" id="txtNomeDestino" name="txtNomeDestino" class="infraText" value="<?=PaginaSip::tratarHTML($objClonarHierarquiaDTO->getStrNomeDestino());?>" maxlength="50" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
