...
class RecursoRN extends InfraRN {
  ...
  protected function gerarControlado(RecursoPadraoDTO $objRecursoPadraoDTO) {
    try{
      ...
      if ($objRecursoPadraoDTO->getStrSinAcaoCadastrar()!='S' && 
          $objRecursoPadraoDTO->getStrSinAcaoAlterar()!='S' &&
          $objRecursoPadraoDTO->getStrSinAcaoConsultar()!='S' &&
          $objRecursoPadraoDTO->getStrSinAcaoListar()!='S' &&
          $objRecursoPadraoDTO->getStrSinAcaoSelecionar()!='S' &&
          $objRecursoPadraoDTO->getStrSinAcaoExcluir()!='S' &&
          $objRecursoPadraoDTO->getStrSinAcaoDesativar()!='S' &&
          $objRecursoPadraoDTO->getStrSinAcaoReativar()!='S'){
      ...
      if ($objRecursoPadraoDTO->getStrSinAcaoCadastrar()=='S'){
        foreach ($arrStrEntidade as $strEntidade) {
          if(InfraString::isBolVazia($strEntidade)){continue;}
  ...
  private function validarStrNome(RecursoDTO $objRecursoDTO, InfraException $objInfraException){
    ...
    if (strlen($objRecursoDTO->getStrNome())>50){
