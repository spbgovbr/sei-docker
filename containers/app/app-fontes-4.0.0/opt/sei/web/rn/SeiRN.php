    ...
    try{
      ...
      if (is_array($objEntradaEnviarEmailAPI->getArquivos())){
        foreach($objEntradaEnviarEmailAPI->getArquivos() as $strNome => $strNomeUpload){
          ...
          $arrArquivos[] = array($strNomeUpload, $strNome, InfraData::getStrDataHoraAtual(), $numTamanhoArquivo, InfraUtil::formatarTamanhoBytes($numTamanhoArquivo));
        }
      }
      ...
      if (SessaoSEI::getInstance()->getObjServicoDTO()!=null && SessaoSEI::getInstance()->getObjServicoDTO()->getStrSinLinkExterno() == 'S') {
        $objAcessoExternoDTO = $this->obterAcessoExternoSistema($objProcedimentoDTO->getDblIdProcedimento());
        $objSaidaEnviarEmailAPI->setLinkAcesso(SessaoSEIExterna::getInstance($objAcessoExternoDTO->getNumIdAcessoExterno())->assinarLink(ConfiguracaoSEI::getInstance()->getValor('SEI', 'URL') . '/documento_consulta_externa.php?id_acesso_externo=' . $objAcessoExternoDTO->getNumIdAcessoExterno() . '&id_documento=' . $objDocumentoDTOGerado->getDblIdDocumento()));
      }else{
        $objSaidaEnviarEmailAPI->setLinkAcesso(ConfiguracaoSEI::getInstance()->getValor('SEI', 'URL') . '/controlador.php?acao=procedimento_trabalhar&id_procedimento=' . $objProcedimentoDTO->getDblIdProcedimento() . '&id_documento=' . $objDocumentoDTOGerado->getDblIdDocumento());
      }
