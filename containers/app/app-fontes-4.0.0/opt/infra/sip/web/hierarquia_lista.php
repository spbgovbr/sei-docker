...
try {
  ...
  if ($numRegistros > 0){
    ...
    }else if ($_GET['acao']=='hierarquia_reativar'){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td valign="top">'.PaginaSip::getInstance()->getTrCheck($i,$arrObjHierarquiaDTO[$i]->getNumIdHierarquia(),$arrObjHierarquiaDTO[$i]->getStrNome()).'</td>';
      }
