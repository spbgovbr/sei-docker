...
try {
  ...
  SessaoSip::getInstance()->validarPermissao($_GET['acao']);
	...
	if (isset($_GET['id_menu_superior']) && 
	    isset($_GET['id_item_menu_superior']) &
	    isset($_GET['id_sistema']) && 
	    isset($_GET['id_orgao_sistema'])){
      ...
      if (isset($_GET['id_menu']) && isset($_GET['id_item_menu'])){
        ...
        if (count($arr)==0){
          throw new InfraException("Registro não encontrado.");
				...
				if ($objItemMenuDTO->getNumIdMenuPai()==null &&
				    $objItemMenuDTO->getNumIdItemMenuPai()==null){
				  $bolRaiz = 'true';
			  }
			} else {
...
?>
<form id="frmItemMenuCadastro" method="post" onsubmit="return OnSubmitForm();" action="<?=SessaoSip::getInstance()->assinarLink(basename(__FILE__).'?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
<?
...
?>
  ...
  <select id="selSistema" name="selSistema" onchange="trocarSistema(this);" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$strDesabilitar?>>
  <?=$strItensSelSistema?>
  </select>
  <?
  PaginaSip::getInstance()->fecharAreaDados();
  ...
  <label id="lblMenu" for="selMenu" accesskey="M" class="infraLabelObrigatorio"><span class="infraTeclaAtalho">M</span>enu:</label>
  <select id="selMenu" name="selMenu" onchange="trocarMenu(this);" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" <?=$strDesabilitar?>>
  <?=$strItensSelMenu?>
  </select>
  <?
  PaginaSip::getInstance()->fecharAreaDados();
  ...
  <label id="lblItemMenuSuperior" for="selItemMenuSuperior" accesskey="I" class="infraLabelObrigatorio"><span class="infraTeclaAtalho">I</span>tem Menu Superior:</label>
  <select id="selItemMenuSuperior" name="selItemMenuSuperior" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" >
  ...
  <label id="lblRecurso" for="selRecurso" accesskey="R" class="infraLabelOpcional"><span class="infraTeclaAtalho">R</span>ecurso:</label>
  <select id="selRecurso" name="selRecurso" class="infraSelect" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" >
  <?=$strItensSelRecurso?>
  </select>
  <?
  PaginaSip::getInstance()->fecharAreaDados();
  ...
  <input type="text" id="txtRotulo" name="txtRotulo" class="infraText" value="<?=PaginaSip::tratarHTML($objItemMenuDTO->getStrRotulo())?>" maxlength="50" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
  <?
  PaginaSip::getInstance()->fecharAreaDados();
  PaginaSip::getInstance()->abrirAreaDados('5em');
  ?>
  ...
  <input type="text" id="txtDescricao" name="txtDescricao" class="infraText" value="<?=PaginaSip::tratarHTML($objItemMenuDTO->getStrDescricao())?>" maxlength="200" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
  <?
  PaginaSip::getInstance()->fecharAreaDados();
  PaginaSip::getInstance()->abrirAreaDados('5em');
  ?>
  ...
  <input type="text" id="txtIcone" name="txtIcone" class="infraText" value="<?=PaginaSip::tratarHTML($objItemMenuDTO->getStrIcone())?>" maxlength="250" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
  <?
  PaginaSip::getInstance()->fecharAreaDados();
  PaginaSip::getInstance()->abrirAreaDados('5em');
  ?>

  <label id="lblSequencia" for="txtSequencia" accesskey="c" class="infraLabelObrigatorio">Sequên<span class="infraTeclaAtalho">c</span>ia:</label>
  <input type="text" id="txtSequencia" name="txtSequencia" onkeypress="return infraMascaraNumero(this, event)" class="infraText" value="<?=PaginaSip::tratarHTML($objItemMenuDTO->getNumSequencia())?>"  maxlength="10" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
  <?
  PaginaSip::getInstance()->fecharAreaDados();
  ...
  <div id="divSinNovaJanela" class="infraDivCheckbox">
    <input type="checkbox" id="chkSinNovaJanela" name="chkSinNovaJanela" <?=PaginaSip::getInstance()->setCheckbox($objItemMenuDTO->getStrSinNovaJanela())?> class="infraCheckbox" />
  	<label id="lblSinNovaJanela" accesskey="" for="chkSinNovaJanela" class="infraLabelCheckbox">Abrir em uma nova janela</label>			
  </div>
  <?
  PaginaSip::getInstance()->fecharAreaDados();
  ?>
