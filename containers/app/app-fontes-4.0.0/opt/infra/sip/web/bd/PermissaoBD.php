  ...
  public function __construct(InfraIBanco $objInfraIBanco){
	...
	public function carregar(LoginDTO $objLoginDTO){
		try{
		  ...
		  $sql = 'SELECT DISTINCT unidade.id_unidade,'. 
		                          'unidade.sigla as sigla_unidade, '.
		                          'unidade.descricao as descricao_unidade, '.
		                          'unidade.id_orgao as id_orgao_unidade, '.
					                    'unidade.id_origem as id_origem_unidade, '.
					                    'unidade.sin_global as sin_global_unidade, '.
		                          'permissao.sin_subunidades, '.
		                          'recurso.nome as nome_recurso, '.
		                          'rel_perfil_item_menu.id_item_menu '. 
             'FROM permissao '.
             'INNER JOIN  (perfil INNER JOIN (rel_perfil_recurso INNER JOIN recurso '. 
                                                                    'ON rel_perfil_recurso.id_sistema=recurso.id_sistema '. 
                                                                    'AND rel_perfil_recurso.id_recurso=recurso.id_recurso '. 
                                                                    'AND recurso.sin_ativo=\'S\' '.
                                                                    'LEFT JOIN (rel_perfil_item_menu INNER JOIN item_menu '. 
                                                                                                    'ON item_menu.id_menu=rel_perfil_item_menu.id_menu '.
                                                                                                    'AND item_menu.id_item_menu=rel_perfil_item_menu.id_item_menu '.
                                                                                                    'AND item_menu.sin_ativo=\'S\') '.
                                                                    'ON rel_perfil_recurso.id_perfil=rel_perfil_item_menu.id_perfil '. 
                                                                    'AND rel_perfil_recurso.id_sistema=rel_perfil_item_menu.id_sistema '.
                                                                    'AND rel_perfil_recurso.id_recurso=rel_perfil_item_menu.id_recurso) '.
                                     'ON rel_perfil_recurso.id_sistema=perfil.id_sistema '. 
                                     'AND rel_perfil_recurso.id_perfil=perfil.id_perfil) '.
                 'ON perfil.id_sistema=permissao.id_sistema '.
                 'AND perfil.id_perfil=permissao.id_perfil '.        
                 'AND perfil.sin_ativo=\'S\' '.
              'INNER JOIN (unidade INNER JOIN orgao ON unidade.id_orgao=orgao.id_orgao AND orgao.sin_ativo=\'S\') ON permissao.id_unidade=unidade.id_unidade AND unidade.sin_ativo=\'S\' '.
              'INNER JOIN usuario ON permissao.id_usuario=usuario.id_usuario AND usuario.sin_ativo=\'S\' '. 
              'INNER JOIN sistema ON perfil.id_sistema=sistema.id_sistema AND sistema.sin_ativo=\'S\' '. 
              'WHERE (permissao.id_usuario = '.$this->getObjInfraIBanco()->formatarGravacaoNum($objLoginDTO->getNumIdUsuario()).') '. 
              'AND (permissao.id_sistema='.$this->getObjInfraIBanco()->formatarGravacaoNum($objLoginDTO->getNumIdSistema()).') '. 
              'AND (permissao.dta_inicio) <= '.$this->getObjInfraIBanco()->formatarGravacaoDta(InfraData::getStrDataAtual()).' AND (permissao.dta_fim >= '.$this->getObjInfraIBanco()->formatarGravacaoDta(InfraData::getStrDataAtual()).' OR permissao.dta_fim IS NULL) '. 
              'ORDER BY unidade.id_unidade,  permissao.sin_subunidades';
      ...
      $arrOrgaos = array();
        ...
        if (!isset($arrPermissoes[$numIdUnidade])){
          ...
          $arrPermissoes[$numIdUnidade] = array();
					$arrPermissoes[$numIdUnidade][InfraSip::$WS_LOGIN_PERMISSAO_UNIDADES] = array(array($numIdUnidade,
					                                                                              $recurso['sigla_unidade'],
					                                                                              $recurso['descricao_unidade'],
					                                                                              $recurso['id_orgao_unidade'],
							                                                                          $recurso['id_origem_unidade']));
    	      ...
    	      for($j=0;$j<$numFilhas;$j++){
    	        ...
    	        if (!isset($arrPermissoes[$numIdUnidadeFilha])){
    	          ...
    	          if (!in_array($arrFilhas[$j]->getNumIdOrgaoUnidade(), $arrOrgaos)){
    	            $arrOrgaos[] = $arrFilhas[$j]->getNumIdOrgaoUnidade();
    						...
    						$arrPermissoes[$numIdUnidadeFilha][InfraSip::$WS_LOGIN_PERMISSAO_UNIDADES] = array(array($numIdUnidadeFilha,
          						                                                                             $arrFilhas[$j]->getStrSiglaUnidade(),
          						                                                                             $arrFilhas[$j]->getStrDescricaoUnidade(),
          						                                                                             $arrFilhas[$j]->getNumIdOrgaoUnidade(),
										                                                                               $arrFilhas[$j]->getStrIdOrigemUnidade()));
                ...
                $arrPermissoes[$numIdUnidadeFilha][InfraSip::$WS_LOGIN_PERMISSAO_MENU] = array_keys($arrReplicarItensMenu[$numIdUnidadePai]);
      	        ...
      	        //Complementa recursos
   						  $arrPermissoes[$numIdUnidadeFilha][InfraSip::$WS_LOGIN_PERMISSAO_RECURSOS] = array_unique(array_merge(array_keys($arrReplicarRecursos[$numIdUnidadePai]),$arrPermissoes[$numIdUnidadeFilha][InfraSip::$WS_LOGIN_PERMISSAO_RECURSOS]));
					...
					foreach ($arrHierarquia as $unidadeHierarquia) {
						...
						if ($unidadeHierarquia->getNumIdOrgaoUnidade() == $arrObjUnidadeDTO[$numIdUnidadeGlobal]->getNumIdOrgao()) {
							...
							if (!isset($arrPermissoes[$numIdUnidadeHierarquia])) {
								...
								$arrPermissoesGlobais[] = array($numIdUnidadeHierarquia,
										$unidadeHierarquia->getStrSiglaUnidade(),
										$unidadeHierarquia->getStrDescricaoUnidade(),
										$unidadeHierarquia->getNumIdOrgaoUnidade(),
										$unidadeHierarquia->getStrIdOrigemUnidade());
							} else {
								//Complementa recursos
								$arrPermissoes[$numIdUnidadeHierarquia][InfraSip::$WS_LOGIN_PERMISSAO_RECURSOS] = array_unique(array_merge($arrPermissoes[$numIdUnidadeGlobal][InfraSip::$WS_LOGIN_PERMISSAO_RECURSOS], $arrPermissoes[$numIdUnidadeHierarquia][InfraSip::$WS_LOGIN_PERMISSAO_RECURSOS]));
								$arrPermissoes[$numIdUnidadeHierarquia][InfraSip::$WS_LOGIN_PERMISSAO_MENU] = array_unique(array_merge($arrPermissoes[$numIdUnidadeGlobal][InfraSip::$WS_LOGIN_PERMISSAO_MENU], $arrPermissoes[$numIdUnidadeHierarquia][InfraSip::$WS_LOGIN_PERMISSAO_MENU]));
							}
						}
					}
      ...
      if (count($arrOrgaos)){
        ...
        foreach($arrObjOrgaoDTO as $objOrgaoDTO){
          $arrOrgaos[] = array(InfraSip::$WS_LOGIN_ORGAO_ID => $objOrgaoDTO->getNumIdOrgao(),
                               InfraSip::$WS_LOGIN_ORGAO_SIGLA => $objOrgaoDTO->getStrSigla(),
                               InfraSip::$WS_LOGIN_ORGAO_DESCRICAO => $objOrgaoDTO->getStrDescricao());
        }
	...
	public function carregarUsuarios(PermissaoDTO $objPermissaoDTO){
		try{
		  ...
		  $sql = ' SELECT DISTINCT usuario.id_usuario, usuario.id_origem, usuario.id_orgao, usuario.sigla, usuario.nome, usuario.sin_ativo, unidade.id_unidade, permissao.sin_subunidades'.
						  ' FROM permissao, perfil, sistema, rel_perfil_recurso, recurso, unidade, usuario'.
						  ' WHERE (permissao.id_sistema='.$this->getObjInfraIBanco()->formatarGravacaoNum($objPermissaoDTO->getNumIdSistema()).')'.
						  ' and (permissao.dta_inicio) <= '.$this->getObjInfraIBanco()->formatarGravacaoDta(InfraData::getStrDataAtual()).
						  ' and (permissao.dta_fim >= '.$this->getObjInfraIBanco()->formatarGravacaoDta(InfraData::getStrDataAtual()).' or permissao.dta_fim is null)'.
						  ' and (permissao.id_usuario=usuario.id_usuario)'.
						  ' and (permissao.id_perfil=perfil.id_perfil)'.
						  ' and (permissao.id_sistema=perfil.id_sistema)'.
						  ' and (permissao.id_unidade=unidade.id_unidade)'.
		          ' and unidade.sin_ativo=\'S\''.
					    ' and unidade.sin_global=\'N\''.
						  ' and (perfil.id_perfil=rel_perfil_recurso.id_perfil)'.
						  ' and (perfil.id_sistema=rel_perfil_recurso.id_sistema)'.
						  ' and perfil.sin_ativo=\'S\''.
						  ' and (perfil.id_sistema=sistema.id_sistema)'.
						  ' and sistema.sin_ativo=\'S\''.
						  ' and (rel_perfil_recurso.id_sistema=recurso.id_sistema)'.
						  ' and (rel_perfil_recurso.id_recurso=recurso.id_recurso)';
     ...
     if ($objPermissaoDTO->isSetNumIdUnidade()){
		 ...
		 $sql .= ' and recurso.sin_ativo=\'S\''.
						 ' order by id_usuario, id_unidade, sin_subunidades';
      ...
      if (count($rs)>0){
        ...
        foreach($rs as $item){
          ...
          if (!isset($ret[$numIdUsuario])){
            ...
            $ret[$numIdUsuario][InfraSip::$WS_USUARIO_NOME] = $item['nome'];
	...
	public function listarUnidades(PermissaoDTO $objPermissaoDTO){
		try{
		  ...
		  $sql = ' SELECT DISTINCT permissao.id_unidade, permissao.sin_subunidades'.
						  ' FROM permissao, perfil, sistema, rel_perfil_recurso, recurso, unidade, usuario'.
						  ' WHERE (permissao.id_sistema='.$this->getObjInfraIBanco()->formatarGravacaoNum($objPermissaoDTO->getNumIdSistema()).')'.
						  ' and (permissao.dta_inicio) <= '.$this->getObjInfraIBanco()->formatarGravacaoDta(InfraData::getStrDataAtual()).
						  ' and (permissao.dta_fim >= '.$this->getObjInfraIBanco()->formatarGravacaoDta(InfraData::getStrDataAtual()).' or permissao.dta_fim is null)'.
	...
	public function carregarUsuario(PermissaoDTO $objPermissaoDTO){
	  try{
	    ...
	    $sql = ' SELECT DISTINCT unidade.id_unidade, unidade.sigla as siglaunidade, unidade.descricao as descricaounidade, permissao.sin_subunidades, perfil.id_perfil, perfil.nome as nomeperfil'.
	        ' FROM permissao, perfil, sistema, unidade, usuario'.
	        ' WHERE (permissao.id_sistema='.$this->getObjInfraIBanco()->formatarGravacaoNum($objPermissaoDTO->getNumIdSistema()).')'.
	        ' and (permissao.dta_inicio) <= '.$this->getObjInfraIBanco()->formatarGravacaoDta(InfraData::getStrDataAtual()).
	        ' and (permissao.dta_fim >= '.$this->getObjInfraIBanco()->formatarGravacaoDta(InfraData::getStrDataAtual()).' or permissao.dta_fim is null)'.
	        
	        ' and (permissao.id_usuario='.$this->getObjInfraIBanco()->formatarGravacaoNum($objPermissaoDTO->getNumIdUsuario()).')'.
	        ' and (permissao.id_usuario=usuario.id_usuario)'.
	        ' and usuario.sin_ativo=\'S\''.
	    ...
	    if ($numRegistros){
	      ...
	      for($i=0; $i<$numRegistros;$i++){
            ...
            $ret[$numIdUnidade] = array('IdUnidade' => $rs[$i]['id_unidade'],
                                        'SiglaUnidade' => $rs[$i]['siglaunidade'],
                                        'DescricaoUnidade' => $rs[$i]['descricaounidade'],
                                        'Perfis' => array());
	        }
	        ...
	        for($j=$i;$j<$numRegistros;$j++){
	          if ($rs[$i]['id_unidade']==$rs[$j]['id_unidade']){
	            $arrPerfis[] = array('IdPerfil' => $rs[$j]['id_perfil'],
	                                 'NomePerfil' => $rs[$j]['nomeperfil']); 
	          }else{
	            $j--;
	            break;
	          }
					...
					if ($rs[$i]['sin_subunidades']=='S'){
						...
						foreach($arrFilhas as $filha){
							...
							if (!isset($ret[$numIdUnidadeFilha])){
								$ret[$numIdUnidadeFilha] = array('IdUnidade' => $filha->getNumIdUnidade(),
																								 'SiglaUnidade' => $filha->getStrSiglaUnidade(),
																								 'DescricaoUnidade' => $filha->getStrDescricaoUnidade(),
																								 'Perfis' => array());
							}
	    ...
	    foreach($ret as $arrUnidade){
	      $retFormatado[] = array('IdUnidade' => $arrUnidade['IdUnidade'],
	                              'SiglaUnidade' => $arrUnidade['SiglaUnidade'],
	                              'DescricaoUnidade' => $arrUnidade['DescricaoUnidade'],
	                              'Perfis' => array_values($arrUnidade['Perfis']));
	    }
	    return $retFormatado;
