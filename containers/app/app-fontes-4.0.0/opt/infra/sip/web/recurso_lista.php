...
try {
  ...
  if ($numRegistros > 0){
    ...
    if ($_GET['acao']=='recurso_selecionar'){
      ...
      $bolCheck = true;
    }else if ($_GET['acao']=='recurso_reativar'){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td valign="top">'.PaginaSip::getInstance()->getTrCheck($i,$arrObjRecursoDTO[$i]->getNumIdSistema().'-'.$arrObjRecursoDTO[$i]->getNumIdRecurso(),$arrObjRecursoDTO[$i]->getStrNome()).'</td>';
      }
...
<form id="frmRecursoLista" method="post" action="<?=SessaoSip::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  ...
  <label id="lblNomeRecurso" for="txtNomeRecurso" accesskey="N" class="infraLabelOpcional"><span class="infraTeclaAtalho">N</span>ome:</label>
  <input type="text" id="txtNomeRecurso" name="txtNomeRecurso" class="infraText" value="<?=$strNomePesquisa?>" maxlength="50" tabindex="<?=PaginaSip::getInstance()->getProxTabDados()?>" />
