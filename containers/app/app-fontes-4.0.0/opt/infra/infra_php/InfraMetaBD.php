...
class InfraMetaBD {
  ...
  public function obterColunasConstraints($strNomeTabela=null){
    ...
    if ($this->objInfraIBanco instanceof InfraSqlServer){
      ...
      $sql = 'SELECT lower(tc.table_name) as table_name, lower(tc.constraint_name) as constraint_name, lower(tc.constraint_type) as constraint_type, string_agg(distinct kcu.column_name, \',\') AS column_names, ccu.table_name AS referenced_table_name, string_agg(distinct ccu.column_name, \',\') AS foreign_column_names   
              ...
              WHERE lower(tc.constraint_catalog) = \''.strtolower($this->objInfraIBanco->getBanco()).'\' and lower(tc.constraint_type) in (\'primary key\',\'foreign key\')';
    ...
    foreach($rs as $arrConstraint){
      ...
      $ret[$arrConstraint['table_name']][$arrConstraint['constraint_name']][$arrConstraint['column_name']] = array($arrConstraint['referenced_table_name'], $arrConstraint['referenced_column_name']);
    }
  ...
  public function obterSequencias($arrTabelasIgnorar = null){
    ...
    if ($this->objInfraIBanco instanceof InfraSqlServer){
      ...
      $sql .= str_replace(' and ', ' where ',$this->formatarTabelasIgnorar('lower(sequencename)',$arrTabelasIgnorar));
  ...
  public function obterMaxIdTabelaSequencia($strNomeSequencia){
    ...
    try{
      if ($this->objInfraIBanco instanceof InfraSqlServer){
        ...
        $ret = $this->objInfraIBanco->consultarSql('SELECT COALESCE(last_value, start_value) as ultimo FROM pg_sequences WHERE lower(sequencename) = \''.$strNomeSequencia.'\'');
        $ret = $ret[0]['ultimo'];
      }
  ...
  public function alterarColuna($strTabela, $strColuna, $strTipo, $strNull){
    ...
    if ($this->objInfraIBanco instanceof InfraPostgreSql){
      ...
      $sql .= ' alter column '.$this->validarIdentificador($strColuna).' '.$this->getOptionNull($strTabela, $strColuna, $strNull);
    }else {
      if ($this->objInfraIBanco instanceof InfraMySql) {
        $sql .= 'modify column';
      } else if ($this->objInfraIBanco instanceof InfraSqlServer) {
        ...
        if ($strTipo == 'clob') {
          $rs = $this->objInfraIBanco->consultarSql('SELECT lower(data_type) as data_type FROM all_tab_columns WHERE lower(table_name)=\''.strtolower($strTabela).'\' AND lower(owner)=\''.strtolower($this->objInfraIBanco->getUsuario()).'\' and lower(column_name)=\''.strtolower($strColuna).'\'');
          if ($rs[0]['data_type'] == 'varchar' || $rs[0]['data_type'] == 'varchar2') {
            $this->objInfraIBanco->executarSql('alter table '.$strTabela.' rename column '.$strColuna.' to x'.$strColuna);
            $this->objInfraIBanco->executarSql('alter table '.$strTabela.' add '.$strColuna.' clob null');
            $this->objInfraIBanco->executarSql('update '.$strTabela.' set '.$strColuna.' = x'.$strColuna);
            $this->objInfraIBanco->executarSql('alter table '.$strTabela.' drop column x'.$strColuna);

            if ($this->normalizarOpcao($strNull) == 'not null'){
              $this->objInfraIBanco->executarSql('alter table '.$strTabela.' modify '.$strColuna.' not null');
            }
  ...
  public function renomearIndice($strTabela, $strIndexAntigo, $strIndexNovo, $arrColunas){
    if ($this->objInfraIBanco instanceof InfraMySql){
      ...
      if (in_array(substr($strVersao,0,3), array('5.0', '5.1', '5.2', '5.3', '5.4', '5.5', '5.6')) || strpos($strVersao, 'MariaDB')!==false){
        $this->objInfraIBanco->executarSql('SET FOREIGN_KEY_CHECKS = 0');
        $this->objInfraIBanco->executarSql('alter table '.$strTabela.' drop index '.$strIndexAntigo.', add index '.$strIndexNovo.' ('.implode(',',$arrColunas).')');
        $this->objInfraIBanco->executarSql('SET FOREIGN_KEY_CHECKS = 1');
      }else{
        $this->objInfraIBanco->executarSql('alter table '.$strTabela.' rename index '.$strIndexAntigo.' to '.$strIndexNovo);
      }
    }else if ($this->objInfraIBanco instanceof InfraSqlServer){
      try {
        $this->objInfraIBanco->executarSql('EXEC sp_rename \''.$strTabela.'.'.$strIndexAntigo.'\', \''.$strIndexNovo.'\', \'INDEX\'');
      }catch(Exception $e){
    ...
    foreach($arrConstraints as $arrConstraint) {
      ...
      if ($arrTabelas == null || in_array($strNomeTabela, $arrTabelas)) {
        ...
        if ($arrConstraint['constraint_type'] == 'primary key') {
          ...
          $arrColunasPK = array_keys($arrColunasConstraints[$strNomeTabela][$arrConstraint['constraint_name']]);
      ...
      if ($bolPKs) {
        ...
        for ($i = 0; $i < $numConstraints; $i++) {
          if ($arrConstraints[$i]['constraint_type'] == 'primary key') {
            $arrColunasPK = array_keys($arrColunasContraints[$arrConstraints[$i]['table_name']][$arrConstraints[$i]['constraint_name']]);
            $sqlPKs .= 'alter table '.$arrConstraints[$i]['table_name'].' add constraint '.$arrConstraints[$i]['constraint_name'].' primary key ('.implode(',',$arrColunasPK).');'."\n";
          }
        }
        $ret .= $sqlPKs;
      }
      ...
      if ($bolFKs) {
        ...
        for ($i = 0; $i < $numConstraints; $i++) {
          if ($arrConstraints[$i]['constraint_type'] == 'foreign key') {
            ...
            $sqlFKs .= 'alter table '.$arrConstraints[$i]['table_name'].' add constraint '.$arrConstraints[$i]['constraint_name'].' foreign key ('.implode(',',$arrCampos).') references '.$strTabelaOrigem.' ('.implode(',',$arrCamposOrigem).');'."\n";
          }
        }
        $ret .= $sqlFKs;
      }
      ...
      if ($bolIndices) {
        ...
        foreach ($arrIndices as $strTabela => $arrIndicesTabela) {
          foreach ($arrIndicesTabela as $strIndice => $arrColunasIndice) {
            $sqlIndices .= 'create index '.$strIndice.' on '.$strTabela.' ('.implode(',', $arrColunasIndice).');'."\n";
          }
        }
        ...
        foreach ($arrIndices as $strTabela => $arrIndicesTabela) {
          foreach ($arrIndicesTabela as $strIndice => $arrColunasIndice) {
            $sqlIndices .= 'create unique index '.$strIndice.' on '.$strTabela.' ('.implode(',', $arrColunasIndice).');'."\n";
          }
        }
        $ret .= $sqlIndices;
      }
