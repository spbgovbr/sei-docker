...
abstract class InfraPagina {
  ...
  public function isBolAjustarTopFieldset(){
    return $this->isBolNavegadorFirefox() || $this->isBolNavegadorSafari() || intval(self::getNumVersaoChrome()) >= 88;
  }
  ...
  public function montarStyle() {
    ...
    }else if ($this->isBolNavegadorSafariIpad()){
  ...
  public function montarJavaScript() {
    ...
    $numVersao = $this->getNumVersao();
      ...
      echo '
<script type="text/javascript">var INFRA_PATH_IMAGENS=\''.$this->getDiretorioImagensGlobal().'\',INFRA_PATH_SVG=\''.$this->getDiretorioSvgGlobal().'\',INFRA_PATH_JS=\''.$this->getDiretorioJavaScriptGlobal().'\',INFRA_PATH_CSS=\''.$this->getDiretorioCssGlobal().'\';</script>
<script type="text/javascript" charset="utf-8" src="' . $strDiretorioJavascriptGlobal . '/jquery/jquery-'.$this->getVersaoJQuery().'.min.js?'.$this->getVersaoJQuery().'"></script>
...
<script type="text/javascript" charset="utf-8" src="' . $strDiretorioJavascriptGlobal . '/smartmenu/addons/keyboard/jquery.smartmenus.keyboard.js"></script>
    ...
    echo '<script type="text/javascript" charset="iso-8859-1" src="' . $this->getDiretorioJavaScriptGlobal() . '/'.$this->getArquivoJavaScriptPagina().'?' . $this->getNumVersao() . '"></script>'."\n";
...
<script type="text/javascript" charset="iso-8859-1" src="' . $strDiretorioJavascriptGlobal . '/calendario/v'.$this->getNumVersaoCalendario().'/InfraCalendario.js?' . $numVersao . '"></script>
...
';
  ...
  protected function montarBarraAcesso() {
    if ($this->isBolMontarBarraAcesso() && $this->getObjInfraSessao()!=null && $this->getObjInfraSessao()->getDthUltimoAcesso()!=null && $this->getObjInfraSessao()->getAtributo('infra_ultimo_acesso')==null) {
      ...
      $this->getObjInfraSessao()->setAtributo('infra_ultimo_acesso', $this->getObjInfraSessao()->getDthUltimoAcesso());
    }
  }
  ...
  public function montarAreaTabela($strResultado, $numRegistros, $bolExecutou = false, $strAtributos = '', $arrSelecoes = null, $strSelecaoPrincipal = 'Infra', $bolForcarMontagemConteudo = false, $strCustomCallbackJs = null) {
    ...
    if ($this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PREPARAR] && $this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PROCESSAR]) {
      $this->montarAreaPaginacao('Superior', $strSelecaoPrincipal, $this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_TAB_INDEX], $strCustomCallbackJs);
    }
    ...
    if (isset($this->arrSelecoes[$strSelecaoPrincipal])) {
      ...
      if ($this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PREPARAR] && $this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_PAG_PROCESSAR]) {
        $this->montarAreaPaginacao('Inferior', $strSelecaoPrincipal, $this->arrSelecoes[$strSelecaoPrincipal][self::$POS_SEL_TAB_INDEX], $strCustomCallbackJs);
  ...
  public function montarAreaPaginacao($strTipo, $strSelecao, $bolMontarTabIndexPaginacao = true, $strCustomCallbackJs = null) {
  ...
  public function getThCheck($strRotulo = '', $strNomeSelecao = 'Infra', $strAtributos = '', $bolGerarTabIndex = true) {
    ...
    $strAnchor .= '<a href="javascript:void(0);" id="lnkInfraCheck" ' . $strAtributos . ' ' . ($bolGerarTabIndex ? 'tabindex="' . $this->getProxTabTabela() . '"' : '') . '><img src="' . $this->getIconeCheck() . '" id="img' . $strNomeSelecao . 'Check" title="Selecionar Tudo" alt="Selecionar Tudo" class="infraImg" /></a>';
  ...
  public function getTrCheck($numItem, $strId, $strTitulo, $strValor = 'N', $strNomeSelecao = 'Infra', $strAtributos = '') {
    ...
    $hasTabIndex = $this->arrSelecoes[$strNomeSelecao][self::$POS_SEL_TAB_INDEX];

    return $this->getInfraPaginaRendererFactory()->createTrCheck($strId,
          $strNomeSelecao,
          $strAtributos,
          $strTitulo,
          $isChecked,
                  ...
                  if (progressArray[name].excecao!='') {
                    ...
                    if (progressArray[name].validacao!='') {
                      ...
                      if (progressArray[name].redirect) {
                        this.stop();
                        var janelaPai = (window.opener != null) ? window.opener : window.parent;
...
//                      alert('close');
  ...
  public function prepararPaginacao($objInfraDTO, $numRegistrosPorPagina = 50, $bolIgnorarSessao = false, $numPaginaAtual = null, $strSelecao = 'Infra') {
    ...
    if ($this->getObjInfraSessao()==null) {
      ...
      $strHashAtual = md5($objInfraDTO->__toString() . $numRegistrosPorPagina. $this->getObjInfraSessao()->getNumIdUnidadeAtual());
    }
   ...
   * @throws InfraException
   */
  public function getThOrdenacao($objInfraDTO, $strRotulo, $strCampo, &$arrObjInfraDTO, $bolForcarOrdenacaoMemoria = false, $strSelecao = 'Infra', $strCustomCallbackJs = null) {

    $hasTabIndex = (bool)($this->arrSelecoes[$strSelecao][self::$POS_SEL_TAB_INDEX]);
  ...
  public function montarMenuSessao($strNomeMenu) {
    if ($this->getObjInfraSessao()!==null) {
      ...
      return ($this->tipoMenu==self::$MENU_NORMAL ||  $this->tipoMenu==self::$MENU_BOOTSTRAP ? $this->montarMenuArray($arrMenu) : $this->montarSmartMenuArray($arrMenu));
    }
    return '';
  }
    ...
    if ($objInfraSessao!==null) {
      ...
      if ($objInfraSessao->getObjInfraSessaoDTO()!==null) {
        ...
        foreach ($arrIdUnidades as $numIdUnidade) {
          ...
          $str .= '>' . self::tratarHTML($objInfraSessao->getStrSiglaUnidade($numIdUnidade)) . '</option>' . "\n";
        }
      }
      $str .= '</select>' . "\n";
      $str .= '</form>' . "\n";
  ...
  public static function montarHeaderDownload($strNomeArquivo = null, $strContentDisposition = 'inline', $strContentType = null, $bolXSS = false, $bolPermitirIndexacaoRobos = false) {
