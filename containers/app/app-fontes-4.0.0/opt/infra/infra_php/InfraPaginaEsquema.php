...
echo '<link href="'.$this->getDiretorioEsquemasGlobal().'/'.$this->getStrEsquemaCores().'/'.$this->getArquivoCssEsquema().'?'.$numVersao.'" rel="stylesheet" type="text/css" media="all" />
<link href="'.$this->getDiretorioEsquemasGlobal().'/'.$this->getStrEsquemaCores().'/'.$this->getArquivoCssEsquemaN().'?'.$numVersao.'" rel="stylesheet" type="text/css" media="all" />
...
';
	...
	private function montarBarraSistema() {
		...
		if ( ($this->getTipoPagina()==self::$TIPO_PAGINA_COMPLETA && $this->getStrMenuSistema() != null) || $this->getTipoPagina()==self::$TIPO_PAGINA_SEM_MENU){
			...
			echo '<div id="divInfraBarraSistema" class="infraBarraSistema" style="height:4em;background: url('.$this->getDiretorioEsquemasGlobal().'/'.$this->getStrEsquemaCores().'/bg_barra_sistema_conteudo.png'.');">'."\n".
			...
			echo '</div>'."\n".
					...
					'<img src="'.$this->getDiretorioEsquemasGlobal().'/'.$this->getStrEsquemaCores().'/bg_barra_sistema_curva.png" />'.
					'</div>';
		}
	}
