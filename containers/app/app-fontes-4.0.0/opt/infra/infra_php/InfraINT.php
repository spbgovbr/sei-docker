...
abstract class InfraINT {
    ...
    private static function montarItensIniciais($strPrimeiroItemValor, $strPrimeiroItemDescricao, $varValorItemSelecionado, $bolPrimeiroItemDesabilitado = false)
    {
        $strRet = '';
 ...
 */
    public static function montarSelectArrInfraDTO($strPrimeiroItemValor, $strPrimeiroItemDescricao, $varValorItemSelecionado, $arrObjInfraDTO, $varAtributoChave, $strAtributoDescricao, $bolPrimeiroItemDesabilitado = false, $arrAtributosAdicionais = array())
    {
        $strRet = '';
        ...
        if (InfraArray::contar($arrObjInfraDTO)) {
            ...
            $strRet .= self::montarItensIniciais($strPrimeiroItemValor, $strPrimeiroItemDescricao, $varValorItemSelecionado, $bolPrimeiroItemDesabilitado);
            ...
            foreach ($arrObjInfraDTO as $dto) {
                ...
                if (is_array($arrAtributosAdicionais) && count($arrAtributosAdicionais) > 0) {
                    foreach ($arrAtributosAdicionais as $strAtributo) {
                        $strValorAtributo = '';
                        if (!empty($strAtributo) && $dto->isBolExisteAtributo($strAtributo) && $dto->isSetAtributo($strAtributo)) {
                            $strValorAtributo = $dto->get($strAtributo);
                        }
                        $strAtributosAdicionais .= ' '.$strAtributo.'="'.InfraString::formatarXML($strValorAtributo).'"';
                    }
                }
                
                $strRet .= '<option value="' . $strAtributoChave . '"' . $strSelected .  $strAtributosAdicionais . '>' . InfraString::formatarXML($dto->get($strAtributoDescricao)) . '</option>' . "\n";
            }
        }
        return $strRet;
    }

    public static function montarSelectArray($strPrimeiroItemValor, $strPrimeiroItemDescricao, $varValorItemSelecionado, $arrOption, $bolPrimeiroItemDesabilitado = false, $arrAtributosAdicionais = array())
    {
        $varValorItemSelecionado = (! is_array($varValorItemSelecionado) && $varValorItemSelecionado !== null) ? array(
            $varValorItemSelecionado
        ) : $varValorItemSelecionado; // se não for array e não for null: transforma em array
    ...
    public static function montarItemSelect($strValor, $strDescricao, $bolSelecionado, $arrAtributosAdicionais = null)
    {
        ...
        return '<option value="' . $strValor . '" ' . $strSelecionado . $strAtributos . '>' . InfraString::formatarXML($strDescricao) . '</option>' . "\n";
    }
  ...
  public static function montarCheckboxArray($arr, $strValorItensSelecionados){
    ...
    if (is_array($arr)) {
     foreach($arr as $valor=>$descricao){
        $strRet .= self::montarItemCheckbox($valor,$descricao,InfraUtil::inArray($valor, $strValorItensSelecionados));
     }
    }
    return $strRet;
  }
