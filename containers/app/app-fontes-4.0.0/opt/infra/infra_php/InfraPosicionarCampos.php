...
class InfraPosicionarCampos {
  ...
  public function getDblQuantidadeLinhas(){
    $this->obterTabelaEstruturada(); //roda a análise estrutural apenas para atualizar a quantidade de linhas no objeto
  ...
  private function obterTabelaEstruturada(){
    ...
    foreach($this->arrCamposEstrutura as $arrAtributos) {
      ...
      $dblEspacoNecessario = bccomp($arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR], $arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR], 2) > 0 ? $arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR] : $arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR];
    ...
    foreach($arrTabelaEstruturada as $numLinha => $arrCamposLinha){
      if(count($arrCamposLinha) == 0){ //se for linha em branco, é dupla
        ...
        if(!isset($this->arrLinhasSimples[$numLinha])){
          ...
          $this->dblQuantidadeLinhas = bcadd($this->dblQuantidadeLinhas, 0.5, 1); //adiciona linha como simples
        }
      }
    }
  ...
  private function listarValoresCampos(){
    ...
    $dblPercentuaTamanholLinhaSimples = bcdiv($dblPercentuaTamanholLinhaDupla, 2, 2); //(100% / quantidade de linhas) / 2
      ...
      foreach($arrCamposLinha as $arrAtributosCampo){
        ...
        if(!empty($arrAtributosCampo[self::$POS_ID_CAMPO_SUPERIOR])){ //se foi setado o campo superior, faz
          if(!$arrAtributosCampo[self::$POS_BOL_VISIVEL_SUPERIOR]){ //se for invisível
            ...
            $strLargura = (!empty($arrAtributosCampo[self::$POS_ID_CAMPO_COMPLEMENTAR_SUPERIOR]))? bcsub($arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR],2,2): $arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR];
            ...
            if(!InfraString::isBolVazia(trim($arrAtributosCampo[self::$POS_CSS_ADICIONAL_SUPERIOR]))){
              $arrAtributosCssCampo = explode(';', trim($arrAtributosCampo[self::$POS_CSS_ADICIONAL_SUPERIOR]));
            ...
            if(!empty($arrAtributosCampo[self::$POS_ID_CAMPO_COMPLEMENTAR_SUPERIOR])){
              ...
              $dblInicioHorizontalComplemento = bcadd($dblInicioHorizontal, $arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR], 2);
        ...
        if(!empty($arrAtributosCampo[self::$POS_ID_CAMPO_INFERIOR])){ //se foi setado o campo superior, faz
          if(!$arrAtributosCampo[self::$POS_BOL_VISIVEL_INFERIOR]){ //se for invisível
            ...
            $dblAjusteVertical = bcdiv($arrAtributosCampo[self::$POS_DISTANCIA_VERTICAL_INFERIOR], 100, 2); //transforma "60%" em "0,6"
            $dblAjusteVertical = bcmul($dblPercentuaTamanholLinha, $dblAjusteVertical,2); //descobre quanto é o percentual em relação ao tamanho da linha
            $arrResultadoAtributosCssCampo['top'] = bcadd($dblInicioVertical, $dblAjusteVertical, 2).'%'; //adiciona o ajuste sobre o top original (início da linha)

            $strLargura = (!empty($arrAtributosCampo[self::$POS_ID_CAMPO_COMPLEMENTAR_INFERIOR]))? bcsub($arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR],2,2): $arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR];
            $strLargura = (!empty($arrAtributosCampo[self::$POS_ID_CAMPO_COMPLEMENTAR_INFERIOR]))? bcsub($arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR],2,2): $arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR];
            $arrResultadoAtributosCssCampo['width'] = $strLargura.'%';
            ...
            if(!InfraString::isBolVazia(trim($arrAtributosCampo[self::$POS_CSS_ADICIONAL_INFERIOR]))){
              $arrAtributosCssCampo = explode(';', trim($arrAtributosCampo[self::$POS_CSS_ADICIONAL_INFERIOR]));
        ...
        $dblMaiorLargura = bccomp($arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR], $arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR], 2) > 0? $arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR] : $arrAtributosCampo[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR];
  ...
  public function obterJquery(){
    ...
    foreach($arrCamposAtributos as $strIdCampo => $arrAtributosCampo){
      ...
      //associa campo e atributos
      $strResultado .= '     $("#'.$strIdCampo.'").css({'.implode(', ', $arrStrParesAtributos).'});'."\r\n";
    }
   ...
   * @return void
   */
  public function adicionarCampoEstrutura($strIdCampoSuperior=null, $strIdCampoInferior=null, $bolVisivelCampoSuperior=true, $bolVisivelCampoInferior=true,
                                          $dblPercentualHorizontalCampoSuperior=null, $dblPercentualHorizontalCampoInferior=null,
                                          $bolForcarNovaLinha=false, $strIdCampoComplementarCampoSuperior=null, $strIdCampoComplementarCampoInferior=null,
                                          $strCssAdicionalCampoSuperior='position:absolute', $strCssAdicionalCampoInferior='position:absolute',
                                          $numLinhasEmBrancoAbaixo=0, $dblDistanciaVerticalCampoInferior=null, $bolLinhaSimples=false){

    //seta atributos
    $arrAtributos = array();
    $arrAtributos[self::$POS_ID_CAMPO_SUPERIOR] = $strIdCampoSuperior;
    $arrAtributos[self::$POS_ID_CAMPO_INFERIOR] = $strIdCampoInferior;
    $arrAtributos[self::$POS_BOL_VISIVEL_SUPERIOR] = InfraString::isBolVazia($arrAtributos[self::$POS_ID_CAMPO_SUPERIOR])? false : $bolVisivelCampoSuperior;
    $arrAtributos[self::$POS_BOL_VISIVEL_INFERIOR] = InfraString::isBolVazia($arrAtributos[self::$POS_ID_CAMPO_INFERIOR])? false : $bolVisivelCampoInferior;
    ...
    if($arrAtributos[self::$POS_BOL_VISIVEL_SUPERIOR]){ //se o campo SUPERIOR está visível, seta o percentual horizontal ocupado
      $arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR] = InfraString::isBolVazia($dblPercentualHorizontalCampoSuperior)? $this->getDblPercentualHorizontalDefault() : $dblPercentualHorizontalCampoSuperior;
      //garante que o valor não extrapola o limite máximo
      $arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR] = bccomp($arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR], $this->dblPercentualHorizontalMaximo, 2) > 0? $this->dblPercentualHorizontalMaximo : $arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_SUPERIOR];
    ...
    if($arrAtributos[self::$POS_BOL_VISIVEL_INFERIOR]){ //se o campo INFERIOR está visível, seta o percentual horizontal ocupado
      $arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR] = InfraString::isBolVazia($dblPercentualHorizontalCampoInferior)? $this->getDblPercentualHorizontalDefault() : $dblPercentualHorizontalCampoInferior;
      //garante que o valor não extrapola o limite máximo
      $arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR] = bccomp($arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR], $this->dblPercentualHorizontalMaximo, 2) > 0? $this->dblPercentualHorizontalMaximo : $arrAtributos[self::$POS_PERCENTUAL_HORIZONTAL_INFERIOR];
    ...
    if($arrAtributos[self::$POS_BOL_VISIVEL_INFERIOR]){ //se o campo INFERIOR está visível, seta o percentual vertical de início
      $arrAtributos[self::$POS_DISTANCIA_VERTICAL_INFERIOR] = InfraString::isBolVazia($dblDistanciaVerticalCampoInferior)? $this->dblPercentualVerticalLocalCampoInferiorDefault : $dblDistanciaVerticalCampoInferior;
