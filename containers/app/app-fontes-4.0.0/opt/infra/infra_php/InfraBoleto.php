<?
  class InfraBoleto {
    ...
    public static $arrMascara = array('00110', '10001', '01001', '11000', '00101', '10100', '01100', '00011', '10010', '01010');
    ...
    public static function gerarBoleto($strBanco, $strCedente, $strAgenciaCodigoCedente, $strConvenio, $strNossoNumero, $strNumeroDocumento, $dtaVencimento, $strValorDocumento, $strValorCobrado, $strSacado, $strInformacoes, $strCarteira) {
      self::$numXAtual = 0;
      self::$numCor = 0;
      self::$hndImagem = null;
      ...
      $numCodigoBarras = self::montarCodigoBarras($strBanco, $dtaVencimento, $strValorDocumento, $strConvenio, $strNossoNumero, $strCarteira);
...
<div id="ficha_compensacao">
  ...
  <div class="linha_corte"><label>Corte na linha pontilhada</label></div>
    ...
    public static function montarCodigoBarras($strBanco, $dtaVencimento, $strValorDocumento, $strConvenio, $strNossoNumero, $strCarteira) {
      $numValorFormatadoDocumento = substr('0000000000'.str_replace('.', '', str_replace(',', '', $strValorDocumento)), -10);
      if (strlen($strConvenio) == 7) {
        $numValorFormatadoNossoNumero = '000000'.$strConvenio.substr('0000000000'.$strNossoNumero, -10);
      }  
      $numCodigoBarras = $strBanco.self::$numMoeda.$numDVGeral.self::calcularFatorVencimento($dtaVencimento).$numValorFormatadoDocumento.$numValorFormatadoNossoNumero.$strCarteira;
    ...
    public function processarCodigoNumerico($strCodigo) {
      ...
      for ($i=0; $i<$n; $i+=2) {
        ...
        if ($i == 0) $strValorAtual = self::$strInicioCodigo.$strValorAtual;
        if ($i == ($n-2)) $strValorAtual .= self::$strFimCodigo;
