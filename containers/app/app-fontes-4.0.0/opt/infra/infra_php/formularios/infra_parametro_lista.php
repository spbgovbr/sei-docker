 ...
 */
  ...
  /* if ($_GET['acao'] == 'infra_parametro_listar' || $_GET['acao'] == 'infra_parametro_selecionar'){ */
    ...
    if ($bolAcaoCadastrar){
      $arrComandos[] = '<button type="button" accesskey="N" id="btnNovo" value="Novo" onclick="location.href=\''.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_parametro_cadastrar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao']).'\'" class="infraButton"><span class="infraTeclaAtalho">N</span>ovo</button>';
    }
  /* } */
 ...
 */
  ...
  if ($numRegistros > 0){
    ...
    if ($_GET['acao']=='infra_parametro_selecionar'){
      ...
      $bolCheck = true;
/*     }else if ($_GET['acao']=='infra_parametro_reativar'){
 ...
 */    }else{
    ...
    if ($bolAcaoExcluir){
      ...
      $strLinkExcluir = SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_parametro_excluir&acao_origem='.$_GET['acao']);
    }
    ...
    $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraParametroDTO,'Nome','Nome',$arrObjInfraParametroDTO).'</th>'."\n";
    $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraParametroDTO,'Valor','Valor',$arrObjInfraParametroDTO).'</th>'."\n";
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td valign="top">'.PaginaInfra::getInstance()->getTrCheck($i,$arrObjInfraParametroDTO[$i]->getStrNome(),$arrObjInfraParametroDTO[$i]->getStrNome()).'</td>';
      }

      $strResultado .= '<td valign="top">'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraParametroDTO[$i]->getStrNome()).'</td>';

      if (substr($arrObjInfraParametroDTO[$i]->getStrValor(), 0, strlen('data:image'))=='data:image'){
        $strResultado .= '<td valign="top"><img alt="'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraParametroDTO[$i]->getStrNome()).'" src="'.$arrObjInfraParametroDTO[$i]->getStrValor().'" /></td>';
      }else{
        $strResultado .= '<td valign="top">'.nl2br(PaginaInfra::getInstance()->tratarHTML($arrObjInfraParametroDTO[$i]->getStrValor())).'</td>';
      }

      $strResultado .= '<td valign="top" align="center">';

      $strResultado .= PaginaInfra::getInstance()->getAcaoTransportarItem($i,$arrObjInfraParametroDTO[$i]->getStrNome());

      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_parametro_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&nome='.$arrObjInfraParametroDTO[$i]->getStrNome()).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getIconeConsultar().'" title="Consultar Parâmetro" alt="Consultar Parâmetro" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_parametro_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&nome='.$arrObjInfraParametroDTO[$i]->getStrNome()).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getIconeAlterar().'" title="Alterar Parâmetro" alt="Alterar Parâmetro" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoDesativar || $bolAcaoReativar || $bolAcaoExcluir){
        $strId = $arrObjInfraParametroDTO[$i]->getStrNome();
        $strDescricao = PaginaInfra::getInstance()->formatarParametrosJavaScript($arrObjInfraParametroDTO[$i]->getStrNome());
      }
/* 
      if ($bolAcaoDesativar){
 ...
 */
      ...
      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaInfra::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getIconeExcluir().'" title="Excluir Parâmetro" alt="Excluir Parâmetro" class="infraImg" /></a>&nbsp;';
      }
...
?>
<form id="frmInfraParametroLista" method="post" action="<?=SessaoInfra::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  <?
