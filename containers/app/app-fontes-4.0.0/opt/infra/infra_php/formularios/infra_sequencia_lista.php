 ...
 */
  ...
  /* if ($_GET['acao'] == 'infra_sequencia_listar' || $_GET['acao'] == 'infra_sequencia_selecionar'){ */
    ...
    if ($bolAcaoCadastrar){
      $arrComandos[] = '<button type="button" accesskey="N" id="btnNova" value="Nova" onclick="location.href=\''.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_sequencia_cadastrar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao']).'\'" class="infraButton"><span class="infraTeclaAtalho">N</span>ova</button>';
    }
  /* } */
 ...
 */
  ...
  if ($numRegistros > 0){
    ...
    if ($_GET['acao']=='infra_sequencia_selecionar'){
      ...
      $bolCheck = true;
/*     }else if ($_GET['acao']=='infra_sequencia_reativar'){
 ...
 */    }else{
    ...
    if ($bolAcaoExcluir){
      ...
      $strLinkExcluir = SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_sequencia_excluir&acao_origem='.$_GET['acao']);
    }
    ...
    $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSequenciaDTO,'Nome','Nome',$arrObjInfraSequenciaDTO).'</th>'."\n";
    $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSequenciaDTO,'Incremento','QtdIncremento',$arrObjInfraSequenciaDTO).'</th>'."\n";
    $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSequenciaDTO,'Valor Atual','NumAtual',$arrObjInfraSequenciaDTO).'</th>'."\n";
    $strResultado .= '<th class="infraTh">'.PaginaInfra::getInstance()->getThOrdenacao($objInfraSequenciaDTO,'Valor Máximo','NumMaximo',$arrObjInfraSequenciaDTO).'</th>'."\n";
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolCheck){
        $strResultado .= '<td valign="top">'.PaginaInfra::getInstance()->getTrCheck($i,$arrObjInfraSequenciaDTO[$i]->getStrNome(),$arrObjInfraSequenciaDTO[$i]->getStrNome()).'</td>';
      }
      $strResultado .= '<td>'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSequenciaDTO[$i]->getStrNome()).'</td>';
      $strResultado .= '<td>'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSequenciaDTO[$i]->getDblQtdIncremento()).'</td>';
      $strResultado .= '<td>'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSequenciaDTO[$i]->getDblNumAtual()).'</td>';
      $strResultado .= '<td>'.PaginaInfra::getInstance()->tratarHTML($arrObjInfraSequenciaDTO[$i]->getDblNumMaximo()).'</td>';
      $strResultado .= '<td align="center">';

      $strResultado .= PaginaInfra::getInstance()->getAcaoTransportarItem($i,$arrObjInfraSequenciaDTO[$i]->getStrNome());

      if ($bolAcaoConsultar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_sequencia_consultar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&nome='.$arrObjInfraSequenciaDTO[$i]->getStrNome()).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getIconeConsultar().'" title="Consultar Sequência" alt="Consultar Sequência" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoAlterar){
        $strResultado .= '<a href="'.SessaoInfra::getInstance()->assinarLink('controlador.php?acao=infra_sequencia_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&nome='.$arrObjInfraSequenciaDTO[$i]->getStrNome()).'" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getIconeAlterar().'" title="Alterar Sequência" alt="Alterar Sequência" class="infraImg" /></a>&nbsp;';
      }

      if ($bolAcaoDesativar || $bolAcaoReativar || $bolAcaoExcluir){
        $strId = $arrObjInfraSequenciaDTO[$i]->getStrNome();
        $strDescricao = PaginaInfra::getInstance()->formatarParametrosJavaScript($arrObjInfraSequenciaDTO[$i]->getStrNome());
      }
/* 
      if ($bolAcaoDesativar){
 ...
 */
      ...
      if ($bolAcaoExcluir){
        $strResultado .= '<a href="'.PaginaInfra::getInstance()->montarAncora($strId).'" onclick="acaoExcluir(\''.$strId.'\',\''.$strDescricao.'\');" tabindex="'.PaginaInfra::getInstance()->getProxTabTabela().'"><img src="'.PaginaInfra::getInstance()->getIconeExcluir().'" title="Excluir Sequência" alt="Excluir Sequência" class="infraImg" /></a>&nbsp;';
      }
...
?>
<form id="frmInfraSequenciaLista" method="post" action="<?=SessaoInfra::getInstance()->assinarLink('controlador.php?acao='.$_GET['acao'].'&acao_origem='.$_GET['acao'])?>">
  <?
