...
try {
  ...
  switch($_GET['acao']){
    ...
    case 'procedimento_gerar_pdf':
      ...
      if ($dblIdDocumento!=null){
        $objProcedimentoDTO->setArrDblIdProtocoloAssociado(array($dblIdDocumento));
      }
      ...
      if (InfraArray::contar($objProcedimentoDTO->getArrObjDocumentoDTO())){
        ...
        foreach($objProcedimentoDTO->getArrObjDocumentoDTO() as $objDocumentoDTO){
          ...
          if(isset($arrObjProtocoloDTO[$objDocumentoDTO->getDblIdDocumento()]) && $objDocumentoDTO->getStrSinPdf()=='S'){
            $strResultadoDocumentos .= PaginaSEI::getInstance()->getTrCheck($numDocumentos++, $objDocumentoDTO->getDblIdDocumento(), $objDocumentoDTO->getStrNomeSerie(), $strMarcado, 'Infra', ($dblIdDocumento==null?'':'disabled="disabled"'));
          }else{
          ...
          $strResultadoDocumentos .= PaginaSEI::tratarHTML($objDocumentoDTO->getStrNomeSerie().' '.$objDocumentoDTO->getStrNumero().(InfraString::isBolVazia($objDocumentoDTO->getStrNomeArvore()) ? "" : " - ".$objDocumentoDTO->getStrNomeArvore()));
          $strResultadoDocumentos .= '</td>';
        ...
        if ($numDocumentos){
          ...
          $strResultadoDocumentos = '<table id="tblDocumentos" width="85%" class="infraTable" summary="Lista de documentos disponíveis para geração">'.
              ($dblIdDocumento==null?'<caption class="infraCaption" >'.PaginaSEI::getInstance()->gerarCaptionTabela("documentos disponíveis para geração", $numDocumentos).'</caption>':'').
          '<tr>
      ...
      if ($_POST['hdnFlagGerar']=='1' || $dblIdDocumento!=null){
        try{

          if ($dblIdDocumento==null){
            $objAnexoDTO = $objDocumentoRN->gerarPdf(InfraArray::gerarArrInfraDTO('DocumentoDTO','IdDocumento',PaginaSEI::getInstance()->getArrStrItensSelecionados()));
            $strNomeDownload = 'SEI-'.$objProcedimentoDTO->getStrProtocoloProcedimentoFormatado().'.pdf';
          }else{
            $arrObjDocumentoDTO = $objProcedimentoDTO->getArrObjDocumentoDTO()[0];
          ...
          $strNomeDownload=InfraString::excluirAcentos($strNomeDownload);
...
function inicializar(){   
  ...
  <?if ($bolGeracaoOK){ ?>  
    window.open('<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao=exibir_arquivo&nome_arquivo='.InfraUtil::formatarNomeArquivo($objAnexoDTO->getStrNome()).'&nome_download='.InfraUtil::formatarNomeArquivo($strNomeDownload).'&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao']);?>');
  <?}?>
...
<?
 ...
 if ($numDocumentos){
   ...
   if ($dblIdDocumento==null) {
     ...
     echo '<label>Documento não está disponível para geração.</label>';
   }
 }
